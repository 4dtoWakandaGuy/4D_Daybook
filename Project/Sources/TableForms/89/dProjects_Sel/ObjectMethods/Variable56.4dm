If (False:C215)
	//object Name: [PROJECTS]dProjects_Sel.Variable56
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>PER_l_CurrentUserID; $_l_event; $_l_ProjectsTable; cNAA; cNAL; cSIS; SM_l_QueryMode; SM_l_ReturnQueryID)
	C_TEXT:C284($_t_ContextName; $_t_oldMethodName; $_t_SearchName; vCompCode; vCOmpName; vCOntCode; vForename; vJob; vJobcode; vSales; vSUrname)
	C_TEXT:C284(vType)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [PROJECTS].dProjects_Sel.Variable56"; Form event code:C388)
$_l_event:=Form event code:C388
$_l_ProjectsTable:=Table:C252(->[PROJECTS:89])
$_t_ContextName:="Projects"
...
Case of 
	: ($_l_event=On Clicked:K2:4)
			...
			Case of 
				: (SM_al_SearchIDs{SM_at_SearchNames}>0)
					...
					If (SM_l_QueryMode#0)
						...
						End if 
						Gen_Set_enterable($_bo_Enterable; ->vJobcode; ->vJob; ->vCompCode; ->vCOmpName; ->vCOntCode; ->vForename; ->vSUrname; ->vSales; ->vType; ->vsDateF; ->vsDateT; ->SRCH_d_EntryDateFrom; ->SRCH_d_EntryDateTo; ->MOD_al_ModuleAccessStatus; ->SYS_at_RecordStateNames; ->cSIS; ->cNAA; ->cNAL)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}=-99998)  //Set user default
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							Sm_SetUserSearchPrefs(True:C214; $_l_ProjectsTable; <>PER_l_CurrentUserID; Abs:C99(SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}))
					...
					Else 
						...
						If (ok=0)
							Sm_SetUserSearchPrefs(True:C214; $_l_ProjectsTable; <>PER_l_CurrentUserID; 0)
					...
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							...
							Else 
								...
								SM_LoadSearchParameterNames($_l_ProjectsTable)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}<0)  //Resave
					...
					SM_SaveSearchParameters(Abs:C99(SM_al_SearchIDs{SM_at_SearchNames}); ""; $_l_ProjectsTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
				...
				Else 
					...
					If ($_t_SearchName#"")
						...
						SET BLOB SIZE:C606(SM_blb_SearchBlob; 0)
						SM_SearchVarsToBlob(->SM_blb_SearchBlob; ->vJobcode; ->vJob; ->vCompCode; ->vCOmpName; ->vCOntCode; ->vForename; ->vSUrname; ->vSales; ->vType; ->vsDateF; ->vsDateT; ->SRCH_d_EntryDateFrom; ->SRCH_d_EntryDateTo; ->MOD_al_ModuleAccessStatus; ->SYS_at_RecordStateNames; ->cSIS; ->cNAA; ->cNAL)
						SM_SaveSearchParameters(0; $_t_SearchName; $_l_ProjectsTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
						SM_LoadSearchParameterNames($_l_ProjectsTable)
		...
		End if 
End case 
ERR_MethodTrackerReturn("OBJ [PROJECTS].dProjects_Sel.Variable56"; $_t_oldMethodName)
