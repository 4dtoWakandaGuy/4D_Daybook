...
If (False:C215)
	//Form Name:      PRODUCTS.Products_Out.Variable17
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_TEXT:C284($_t_Keystroke; $_t_oldMethodName; vGroup; vSales; $_t_EditedText; $_t_Keystroke; $_t_oldMethodName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [PRODUCTS].Products_Out.Variable17"; Form event code:C388)
...
Case of 
		...
		If (cNAL=0)
			...
			If ((Character code:C91($_t_Keystroke)#Backspace:K15:36) & (Length:C16($_t_EditedText)>0))
				If (Character code:C91($_t_EditedText[[Length:C16($_t_EditedText)]])#64)
					$_l_ArrayRow:=Find in array:C230(PROD_at_GroupCodes; $_t_EditedText)
					$_l_EditedTextLength:=Length:C16($_t_EditedText)
					If ($_l_ArrayRow<0)
						$_l_ArrayRow:=Find in array:C230(PROD_at_GroupNames; $_t_EditedText)
						If ($_l_ArrayRow<0)
							For ($_l_Index; 1; Size of array:C274(PROD_at_GroupCodes))
								If (Position:C15($_t_EditedText; PROD_at_GroupCodes{$_l_Index})=1)
									$_l_ArrayRow:=$_l_Index
									$_l_Index:=99999
								End if 
							End for 
							If ($_l_ArrayRow<0)
								For ($_l_Index; 1; Size of array:C274(PROD_at_GroupNames))
									If (Position:C15($_t_EditedText; (PROD_at_GroupNames{$_l_Index}))=1)
										$_l_ArrayRow:=$_l_Index
										$_l_Index:=99999
									End if 
								End for 
					...
					End if 
					If ($_l_ArrayRow>0)
						Self:C308->:=PROD_at_GroupCodes{$_l_ArrayRow}
	...
	: ($_l_event=On Data Change:K2:15)
		//$_t_oldMethodName:=ERR_MethodTracker ("OBJ [PRODUCTS].Products_Out.Variable17";Form event)
		If (cNAL=0)
...
End case 
ERR_MethodTrackerReturn("OBJ [PRODUCTS].Products_Out.Variable17"; $_t_oldMethodName)
$_t_oldMethodName:=ERR_MethodTracker("OBJ [PRODUCTS].Products_Out.Variable17"; Form event code:C388)
$_l_event:=Form event code:C388
Case of 
	: ($_l_event=On After Keystroke:K2:26)
