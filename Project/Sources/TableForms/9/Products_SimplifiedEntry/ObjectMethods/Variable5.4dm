If (False:C215)
	//object Name: [PRODUCTS]Products_SimplifiedEntry.Variable5
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	ARRAY TEXT:C222($_at_ExpectedNames; 0)
	...
	C_LONGINT:C283($_l_CatalogueCount; $_l_CatalogueIndex; $_l_CatalogueRow; $_l_CatImageIndex; $_l_Clash; $_l_ConventionRow; $_l_DestinationRow; $_l_ImageNameRow; CAT_l_PageNumber; CAT_l_PictureSource; CAT_l_SaveSettings)
	C_LONGINT:C283(WIN_l_CurrentWinRef; WIN_l_CurrentWinRefOLD)
	C_PICTURE:C286(PRD_Pic_Graphic)
	C_TEXT:C284($_t_CatalogueNames; $_t_CurrentImageName; $_t_ExpectedImageName; $_t_NameWithoutExtension; $_t_oldMethodName; $_t_PictureCode; $_t_SaveImageName; CAT_t_PictureNaming; CAT_T_PicturePath; DB_t_CurrentFormUsage; PROD_t_GraphicPath)
End if 

//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("OBJ [PRODUCTS]Products_SimplifiedEntry.Variable5"; Form event code:C388)
...
If (Picture size:C356(PRD_Pic_Graphic)>0) & (OK=1)
	...
	If (Size of array:C274(PRD_at_CatalogueNames)>0)
		...
		For ($_l_CatalogueIndex; 1; Size of array:C274(PRD_al_CatalogueSource))
			...
			End case 
			$_l_ImageNameRow:=Find in array:C230(PRD_al_ImageNamePrefs; PRD_al_CatItemNamePREF{$_l_CatalogueIndex})
			...
			If (CAT_al_CatalogueSelected{$_l_CatalogueIndex}=1)
				...
				Else 
					...
					If ($_t_NameWithoutExtension#$_t_ExpectedImageName) & ($_t_CurrentImageName#$_t_ExpectedImageName)
						...
						INSERT IN ARRAY:C227($_at_ExpectedNames; Size of array:C274($_at_ExpectedNames)+1; 1)
						INSERT IN ARRAY:C227($_at_SaveName; Size of array:C274($_at_SaveName)+1; 1)
						$_l_ImageNameRow:=Find in array:C230($_at_ExpectedNames; $_t_ExpectedImageName)
						$_at_ExpectedNames{Size of array:C274($_at_ExpectedNames)}:=$_t_ExpectedImageName
						If ($_l_ImageNameRow<0)
							$_t_CatalogueNames:=""
							$_l_CatalogueCount:=0
							For ($_l_CatImageIndex; 1; Size of array:C274(PRD_al_CatItemNamePREF))
								If (PRD_al_CatItemNamePREF{$_l_CatImageIndex}=PRD_al_CatItemNamePREF{$_l_CatalogueRow})
									//same as this one
									$_l_CatalogueRow:=Find in array:C230(PRD_al_PictureDesCatalogueIDs; PRD_al_CatalogueID{$_l_CatImageIndex})
									If ($_l_CatalogueRow>0)
										If (CAT_al_CatalogueSelected{$_l_CatalogueRow}=1)
											$_l_CatalogueCount:=$_l_CatalogueCount+1
											//and we are saving that path
											If ($_t_CatalogueNames#"")
												$_t_CatalogueNames:=$_t_CatalogueNames+", "+PRD_at_CatalogueNames{$_l_CatImageIndex}
											Else 
												$_t_CatalogueNames:=PRD_at_CatalogueNames{$_l_CatImageIndex}
											End if 
										End if 
							...
							If ($_l_CatalogueCount>1)
								Gen_Confirm(" For the catalogues+"+$_t_CatalogueNames+"change image name from"+$_t_NameWithoutExtension+"to"+$_t_ExpectedImageName+"?"; "Yes"; "No")
							Else 
								Gen_Confirm(" For the catalogue+"+$_t_CatalogueNames+"change image name from "+$_t_NameWithoutExtension+"to"+$_t_ExpectedImageName+"?"; "Yes"; "No")
							End if 
...
//End if
//End if
ERR_MethodTrackerReturn("OBJ [PRODUCTS].Products_SimplifiedEntry.Variable5"; $_t_oldMethodName)