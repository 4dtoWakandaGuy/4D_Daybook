If (False:C215)
	//object Name: [PRODUCTS]Products_In.Variable5
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CatalogueRow; $_l_Clash; $_l_ConventionRow; $_l_DestinationRow; $_l_ImageIndex; $_l_ImageNameRow; $_l_NamingRow; $_l_SourceIndex; CAT_l_PageNumber; CAT_l_PictureSource; WIN_l_CurrentWinRef)
	C_LONGINT:C283(WIN_l_CurrentWinRefOLD; $_l_CatalogueCount; $_l_CatalogueRow; $_l_Clash; $_l_ConventionRow; $_l_DestinationRow; $_l_ImageIndex; $_l_ImageNameRow; $_l_NamingRow; $_l_SourceIndex; CAT_l_PageNumber)
	C_LONGINT:C283(CAT_l_PictureSource; CAT_l_SaveSettings; WIN_l_CurrentWinRef; WIN_l_CurrentWinRefOLD)
	C_PICTURE:C286(PRD_Pic_Graphic)
	C_TEXT:C284($_t_CurrentImageName; $_t_ExpectedImageName; $_t_NameWithoutExtension; $_t_oldMethodName; $_t_PictureCode; $_t_SaveImageName; CAT_t_PictureNaming; CAT_T_PicturePath; DB_t_CurrentFormUsage; PROD_t_GraphicPath; $_t_CatalogueName)
	C_TEXT:C284($_t_CurrentImageName; $_t_ExpectedImageName; $_t_NameWithoutExtension; $_t_oldMethodName; $_t_PictureCode; $_t_SaveImageName; CAT_t_PictureNaming; CAT_T_PicturePath; DB_t_CurrentFormUsage; PROD_t_GraphicPath)
End if 
//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("OBJ [PRODUCTS]Products_In.Variable5"; Form event code:C388)
...
If (Picture size:C356(PRD_Pic_Graphic)>0) & (OK=1)
	...
	Else 
		...
		ARRAY TEXT:C222($_at_ExpectedName; 0)
		ARRAY TEXT:C222($_at_SaveName; 0)
		For ($_l_SourceIndex; 1; Size of array:C274(CAT_al_CatalogueSelected))
			If (CAT_al_CatalogueSelected{$_l_SourceIndex}=1)
				...
				Else 
					...
					If ($_t_NameWithoutExtension#$_t_ExpectedImageName) & ($_t_CurrentImageName#$_t_ExpectedImageName)
						...
						If ($_l_ImageNameRow<0)
							$_t_CatalogueName:=""
							$_l_CatalogueCount:=0
							For ($_l_ImageIndex; 1; Size of array:C274(PRD_al_CatItemNamePREF))
								If (PRD_al_CatItemNamePREF{$_l_ImageIndex}=PRD_al_CatItemNamePREF{$_l_CatalogueRow})
									//same as this one
									$_l_DestinationRow:=Find in array:C230(PRD_al_PictureDesCatalogueIDs; PRD_al_CatalogueID{$_l_ImageIndex})
									If ($_l_DestinationRow>0)
										If (CAT_al_CatalogueSelected{$_l_DestinationRow}=1)
											$_l_CatalogueCount:=$_l_CatalogueCount+1
											//and we are saving that path
											If ($_t_CatalogueName#"")
												$_t_CatalogueName:=$_t_CatalogueName+", "+PRD_at_CatalogueNames{$_l_ImageIndex}
											Else 
												$_t_CatalogueName:=PRD_at_CatalogueNames{$_l_ImageIndex}
											End if 
										End if 
					...
					DS_SendPicturetoDrive(Document; PRD_al_PictureDesCatalogueIDs{$_l_SourceIndex}; $_t_SaveImageName)
				End if 
...
//End if
//End if
ERR_MethodTrackerReturn("OBJ:Products_In,BaddGraphic"; $_t_oldMethodName)
