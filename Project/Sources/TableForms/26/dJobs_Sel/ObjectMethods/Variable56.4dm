If (False:C215)
	//object Name: [JOBS]dJobs_Sel.Variable56
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>PER_l_CurrentUserID; $_l_event; $_l_JobsTable; cNAA; cNAL; cSIS; SM_l_QueryMode; SM_l_ReturnQueryID; VClassID)
	C_REAL:C285(VamountF; VamountT; VinvF; vInvT)
	C_TEXT:C284($_t_ContextName; $_t_oldMethodName; $_t_SearchName; ACC_t_CurrencyCode; Vanalysis; vClass; VCompCode; VcompName; VContCOde; VcontrCode; VForename)
	C_TEXT:C284(VJob; VJobCode; Vlayer; VorderNo; VprodCode; Vsales; Vstage; Vsurname; VType)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [JOBS].dJobs_Sel.Variable56"; Form event code:C388)
$_l_event:=Form event code:C388
$_l_JobsTable:=Table:C252(->[JOBS:26])
$_t_ContextName:="Jobs"
...
Case of 
	: ($_l_event=On Clicked:K2:4)
			...
			Case of 
				: (SM_al_SearchIDs{SM_at_SearchNames}>0)
					...
					If (SM_l_QueryMode#0)
						...
						End if 
						Gen_Set_enterable($_bo_Enterable; ->VJobCode; ->VJob; ->VCompCode; ->VcompName; ->VContCOde; ->VForename; ->Vsurname; ->VcontrCode; ->Vsales; ->VType; ->VorderNo; ->Vstage; ->VsDateF; ->VsDateT)
						Gen_Set_enterable($_bo_Enterable; ->SRCH_d_EntryDateFrom; ->SRCH_d_EntryDateTo; ->VRDateF; ->VRDateT; ->VamountF; ->VamountT; ->VinvF; ->vInvT; ->VprodCode; ->Vanalysis; ->ACC_t_CurrencyCode; ->Vlayer; ->SYS_at_RecordStateNames; ->MOD_al_ModuleAccessStatus; ->cSIS; ->cNAA; ->cNAL)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}=-99998)  //Set user default
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							Sm_SetUserSearchPrefs(True:C214; $_l_JobsTable; <>PER_l_CurrentUserID; Abs:C99(SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}))
					...
					Else 
						...
						If (ok=0)
							Sm_SetUserSearchPrefs(True:C214; $_l_JobsTable; <>PER_l_CurrentUserID; 0)
					...
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							...
							Else 
								...
								SM_LoadSearchParameterNames($_l_JobsTable)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}<0)  //Resave
					...
					SM_SaveSearchParameters(Abs:C99(SM_al_SearchIDs{SM_at_SearchNames}); ""; $_l_JobsTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
				...
				Else 
					...
					If ($_t_SearchName#"")
						...
						SM_SaveSearchParameters(0; $_t_SearchName; $_l_JobsTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
						SM_LoadSearchParameterNames($_l_JobsTable)
		...
		End if 
End case 
ERR_MethodTrackerReturn("OBJ [JOBS].dJobs_Sel.Variable56"; $_t_oldMethodName)
