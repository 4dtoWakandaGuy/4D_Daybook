...
If (True:C214)
	...
	C_OBJECT:C1216($_obj_FormData; $_obj_FormEvent)
	C_TEXT:C284($_t_oldMethodName; $_t_VariableName)
End if 
//Code Starts Here
$_obj_FormEvent:=FORM Event:C1606

$_t_oldMethodName:=ERR_MethodTracker("OBJ [WORKFLOW_CONTROL].Workflow_Control_In.oWorkflowTables"; $_obj_FormEvent.code)
$_l_event:=$_obj_FormEvent.code
		...
		Case of 
			...
			: (SD2_l_WFAddMode=1)  //we are editing a row-its always row one
				...
				If ($_l_Row>1)
					...
					LISTBOX SELECT ROW:C912(*; "oWorkflowTables"; 1)
	...
	: ($_l_event=On Data Change:K2:15)
		...
		Case of 
			...
			: ($_l_ClickedColumn=2)  // `Macro names
				...
				If (WFM_al_WorkflowTables{$_l_Row}>0)
					...
					If ($_l_ScriptRow>0)
						Case of 
							...
							: (SD2_at_MacroCodes{$_l_ScriptRow}="-4")  //write a macro"
								...
								WFM_at_ScriptCodes{$_l_Row}:=""
								WFM_at_ScriptNames{$_l_Row}:=""
								Gen_Alert("When creating this Script you should not move the current record pointer or change the selection of records in the primary table. The Script should be used to work out if the sequence applies rather than used to create a record in the time manager. Th"+"e script must return to true or false")
								
								MAC_l_EnterMacroClass:=WF Macros
								$_obj_FormData:=New object:C1471("MacroClass"; WF Macros; "TableNumber"; WFM_al_WorkflowTables{$_l_Row})
								
								$_boj_InputData:=New object:C1471("TableNumber"; Table:C252(->[SCRIPTS:80]); "FormName"; "Scripts"; "RecordTitle"; "Script"; "FormData"; $_obj_FormData)
								
								Minor_In($_boj_InputData)  //->[SCRIPTS]; "Macros"; "Macros")
								
							: (SD2_at_MacroCodes{$_l_ScriptRow}="-2")  //write a macro"
					...
					End if 
				Else 
					Gen_Alert("You need to select the table before you set the option")
				End if 
