If (False:C215)
	//object Name: [CURRENCIES]Currency_In.Variable4
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 23/10/2012 06:23
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [CURRENCIES].Currency_In.Variable4"; Form event code:C388)
If (CUR_at_CurrencyPeriods>0)
	...
	If (CUR_ad_CurrencyPeriodFrom{CUR_at_CurrencyPeriods}<CUR_D_CurrencyMaxDate)
		...
		For ($_l_Index; 1; Size of array:C274(CUR_ad_CurrencyDate))
			...
			If ($_l_CurrencyRow<0)
				$_l_CurrencyRow:=Find in array:C230(WS_at_CurrencyCodes; "")
			End if 
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [CURRENCIES].Currency_In.Variable4"; $_t_oldMethodName)
