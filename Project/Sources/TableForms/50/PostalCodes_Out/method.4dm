If (False:C215)
	//Table Form Method Name: [POSTAL_CODES]PostalCodes_Out
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("FM [POSTAL_CODES].PostalCodes_Out"; Form event code:C388)
...
Case of 
	: ($_l_event=On Header:K2:17)
		...
		End if 
		INT_Setoutput(Table:C252(->[POSTAL_CODES:50]); WIN_t_CurrentOutputform; ->VtitleR)
	...
	: ($_l_event=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		OpenHelp(Table:C252(->[POSTAL_CODES:50]); "PostalCodes_Out")
		Cache_Initialise
		FSetting_SetLabel(WIN_t_CurrentOutputform)
	: ($_l_event=On Activate:K2:9)  //| (vFromIn))
		...
		vButtDisO:="O              DT"
		Out_Buttons
		vTitleR:=Uppercase:C13(Term_PCWT("Postcodes"))
	: ($_l_event=On Outside Call:K2:11)
		Out_ButtCall(->[POSTAL_CODES:50]; ->[POSTAL_CODES:50]Postal_Code_From:1; ->[POSTAL_CODES:50]Area_Code:2; "Postcodes"; "DB_MenuNewRecord"; "Minor_Sel"; "Minor_DelEx"; ""; "Postcodes"; ""; 0; 0)
	...
	: ($_l_event=On Close Detail:K2:24)
		$_t_FormReference:=Table name:C256(->[POSTAL_CODES:50])+"_"+WIN_t_CurrentInputForm  //added NG may 2004
...
End case 
ERR_MethodTrackerReturn("FM:Postcodes Out"; $_t_oldMethodName)
