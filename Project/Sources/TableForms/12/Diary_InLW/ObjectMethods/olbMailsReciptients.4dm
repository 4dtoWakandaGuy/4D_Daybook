If (False:C215)
	//object Name: [DIARY]Diary_inLW.olbMailsReciptients
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_AtPosition; $_l_ColumnNumber; $_l_ContactsIndex; $_l_ContactTitleRow; $_l_EmailPosition; $_l_EndHighlight; $_l_event; $_l_FieldNumber; $_l_ObjectBottom; $_l_ObjectLeft; $_l_ObjectRight)
	C_LONGINT:C283($_l_ObjectTop; $_l_RecordNumber; $_l_Row; $_l_StartHighlight; $_l_TableNumber; $_l_Textposition; $_l_WindowBottom; $_l_WindowLeft; $_l_WindowRight; $_l_WindowTop; EmailChoiceProc)
	C_LONGINT:C283(WS_l_ContactElement)
	C_TEXT:C284($_t_EditedText; $_t_EmailAddress; $_t_EmailAfter; $_t_EmailBefore; $_t_EmailName; $_t_ExtractedContactFirstName; $_t_ExtractedContactLast; $_t_ExtractedContactLastName; $_t_ExtractedContactTitle; $_t_Keystroke; $_t_oldMethodName)
	C_TEXT:C284($_t_VariableName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [DIARY].Diary_inLW.olbMailsReciptients"; Form event code:C388)
...
Case of 
	...
	: ($_l_event=On After Keystroke:K2:26)
		...
		Case of 
			: ($_l_ColumnNumber=2)
				...
				If (Character code:C91($_t_Keystroke)#Backspace:K15:36)
					AA_ParseName($_t_EditedText; 0; ->$_t_ExtractedContactTitle; ->$_t_ExtractedContactFirstName; ->$_t_ExtractedContactLast; ->AA_at_Salutations; ->AA_at_SNprefix; ->AA_at_ProffesionallAdditions)
					If ($_t_ExtractedContactTitle#"")
						//to ensure capitalization is correct
						$_l_ContactTitleRow:=Find in array:C230(AA_at_Salutations; $_t_ExtractedContactTitle)
						If ($_l_ContactTitleRow>0)
							$_t_ExtractedContactTitle:=AA_at_Salutations{$_l_ContactTitleRow}
						End if 
					End if 
					//`````````
						...
						Case of 
							: (Records in selection:C76([CONTACTS:1])=1)
								...
								If ($_t_EmailAddress#"")
									...
									$_l_Textposition:=Position:C15($_t_EditedText; $_t_EmailName)
									//I want only to append the name
									SD2_at_EmailName{$_l_Row}:=Substring:C12($_t_EmailName; $_l_Textposition; Length:C16($_t_EmailName))
									If ([DIARY:12]Contact_Code:2="")
						...
						Case of 
							: (Records in selection:C76([CONTACTS:1])=1)
								...
								If ($_t_EmailAddress#"")
									SD2_at_EmailAddresses{$_l_Row}:=$_t_EmailAddress
									$_t_EmailName:=CON_BuildContactName
									$_l_EmailPosition:=Position:C15(SD2_at_EmailName{$_l_Row}; $_t_EmailName)
									//I want only to append the name
									SD2_at_EmailName{$_l_Row}:=Substring:C12($_t_EmailName; $_l_EmailPosition; Length:C16($_t_EmailName))
									If ([DIARY:12]Contact_Code:2="")
										[DIARY:12]Contact_Code:2:=[CONTACTS:1]Contact_Code:2
										Diary_InD
										MCont_Add("")
		...
		End case 
End case 
ERR_MethodTrackerReturn("OBJ [DIARY].Diary_inLW.olbMailsReciptients"; $_t_oldMethodName)
