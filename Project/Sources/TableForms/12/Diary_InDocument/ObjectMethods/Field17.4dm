If (False:C215)
	//object Name: [DIARY]Diary_InDocument.Field17
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_event; $_l_SelectedEmail; WS_l_SelectedEmailName)
	C_TEXT:C284($_t_oldMethodName; EMAIL_t_EmailType; WS_t_ContactID; WS_t_EmailAddress; WS_t_EmailName)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [DIARY].Diary_InDocument.Field17"; Form event code:C388)
	...
	: ($_l_event=On Getting Focus:K2:7)
		If ((WS_t_EmailName#"") | (WS_t_EmailAddress#"")) | ((WS_t_EmailName="") & (WS_t_EmailAddress=""))
			...
			If (WS_l_SelectedEmailName>0)
				$_l_SelectedEmail:=WS_l_SelectedEmailName
			Else 
				If (WS_t_EmailAddress#"")
					$_l_SelectedEmail:=Find in array:C230(SD2_at_EmailAddresses; WS_t_EmailAddress)
				Else 
					$_l_SelectedEmail:=0
					WS_l_SelectedEmailName:=0
				End if 
			End if 
			If ($_l_SelectedEmail<0)
		...
		SETEMAILFORMOBJECTS
End case 
ERR_MethodTrackerReturn("OBJ [DIARY].Diary_InDocument.Field17"; $_t_oldMethodName)
