If (False:C215)
	//object Name: [DIARY]Diary_PDTEditor.oSD2OrdersOthersVal
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_TEXT:C284($_t_AreaOptions; $_t_CurrentDefinitionsName; $_t_Denominator; $_t_FilterValue; $_t_Nominator; $_t_oldMethodName; $_t_OrderStatus; $_t_Person; $_t_Stage; $_t_ThisValue; $_t_ValueSTR)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [DIARY].Diary_PDTEditor.oSD2OrdersOthersVal"; Form event code:C388)
//THE CURRENT SELECTION
USE SET:C118("BOOKING")
If (SD_at_OtherFIlterValues>0)
	//because we are searching the arrays we need the full set loaded first
	...
	For ($_l_RowsIndex; 1; Size of array:C274($_Ptr_ThisColumnArray->))
		...
		Case of 
			: (Type:C295($_Ptr_ThisColumnArray->)=Text array:K8:16) | (Type:C295($_Ptr_ThisColumnArray->)=String array:K8:15)
				...
				Else 
					If ($_t_FilterValue="BLANKS") & ($_Ptr_ThisColumnArray->{$_l_RowsIndex}="")
						APPEND TO ARRAY:C911($_al_OrderItemNos; $_Ptr_ThisColumnArray2->{$_l_RowsIndex})
					End if 
				End if 
...
End if 
ERR_MethodTrackerReturn("OBJ [DIARY].Diary_PDTEditor.oSD2OrdersOthersVal"; $_t_oldMethodName)
