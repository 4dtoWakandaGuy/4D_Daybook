If (False:C215)
	//object Name: [DIARY]Diary_PDTEditor.oSD2OrdersOthers
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_AddNul; $_bo_NewLevel; $_bo_ValueBOOL)
	C_DATE:C307($_d_ValueDate)
	C_LONGINT:C283(<>l_FS_FieldSortChangedAlert; $_l_CharacterPosition; $_l_Column; $_l_ColumnNumber; $_l_FilterRow; $_l_KeyColumn; $_l_ReuseLevel; $_l_RowsIndex; $_l_SortChangeAlert; $_l_SortDirection; PDT_l_BookingsCount)
	...
	C_TEXT:C284($_t_AreaOptions; $_t_CurrentDefinitionsName; $_t_Denominator; $_t_Nominator; $_t_oldMethodName; $_t_OrderStatus; $_t_Person; $_t_Stage; $_t_ThisValue; $_t_Value; $_t_ValueSTR)
End if 
//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("OBJ [DIARY]Diary_PDTEditor.oSD2OrdersOthers"; Form event code:C388)

//THE CURRENT SELECTION
If (SD_at_OtherFilters>0)
	SD_At_otherFilterValues:=0  // if there is a filter loaded we will clear it so will need to reload
	If (SD_al_OtherFilterColumns{SD_at_OtherFilters}>0)
		...
		For ($_l_RowsIndex; 1; Size of array:C274($_Ptr_ThisColumnArray->))
			...
			Case of 
				: (Type:C295($_Ptr_ThisColumnArray->)=Text array:K8:16) | (Type:C295($_Ptr_ThisColumnArray->)=String array:K8:15)
					$_t_Value:=$_Ptr_ThisColumnArray->{$_l_RowsIndex}
					If ($_t_Value="")
			...
			End case 
			If ($_t_Value#"")
				$_l_FilterRow:=Find in array:C230(SD_At_otherFilterValues; $_t_Value)
				If ($_l_FilterRow<0)
...
End if 
ERR_MethodTrackerReturn("OBJ: [DIARY].Diary_PDTEditor.SD2_at_OrdersPersons"; $_t_oldMethodName)
