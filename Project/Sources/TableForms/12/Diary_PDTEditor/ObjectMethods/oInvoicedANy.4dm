If (False:C215)
	//object Name: [DIARY]Diary_PDTEditor.oInvoicedANy
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>l_FS_FieldSortChangedAlert; $_l_CharacterPosition; $_l_Column; $_l_Denominator; $_l_Nominator; $_l_ReuseLevel; $_l_RowsIndex; $_l_SizeofArray; $_l_SortChangeAlert; $_l_SortDirection; PDT_l_BookingsCount)
	...
	C_TEXT:C284($_t_CurrentDefinitionsName; $_t_Denominator; $_t_Nominator; $_t_oldMethodName; $_t_OrderStatus; $_t_Person; $_t_Reference; $_t_Stage; $_t_ThisRowValue)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [DIARY].Diary_PDTEditor.oInvoicedANy"; Form event code:C388)
...
If ($_l_Column>0)
	...
	For ($_l_RowsIndex; 1; Size of array:C274($_Ptr_ThisColumnArray->))
		$_t_ThisRowValue:=$_Ptr_ThisColumnArray->{$_l_RowsIndex}
		$_l_CharacterPosition:=Position:C15("/"; $_t_ThisRowValue)
		If ($_l_CharacterPosition=0)
			$_l_SizeofArray:=Num:C11($_Ptr_ThisColumnArray->{$_l_RowsIndex})
		Else 
			$_t_Denominator:=Substring:C12($_t_ThisRowValue; 1; $_l_CharacterPosition-1)
			$_t_Nominator:=Substring:C12($_t_ThisRowValue; $_l_CharacterPosition+1)
			$_l_Denominator:=Num:C11($_t_Denominator)
			$_l_Nominator:=Num:C11($_t_Nominator)
			$_l_SizeofArray:=$_l_Denominator/$_l_Nominator
		End if 
		SD2_R_PageTotal:=SD2_R_PageTotal+$_l_SizeofArray
	End for 
End if 
...
CREATE SET:C116([ORDER_ITEMS:25]; "SelectedBookings")
CREATE SET:C116([ORDER_ITEMS:25]; "SelectedBookings")
ERR_MethodTrackerReturn("OBJ [DIARY].Diary_PDTEditor.oInvoicedANy"; $_t_oldMethodName)
