If (False:C215)
	//object Name: [DIARY]Diary_PDTEditor.oSD2OrdersStages
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	ARRAY LONGINT:C221($_al_OrderItemsNos; 0)
	...
	C_LONGINT:C283(<>l_FS_FieldSortChangedAlert; $_l_CharacterPosition; $_l_Column; $_l_ColumnNumber; $_l_KeyColumn; $_l_ReuseLevel; $_l_RowsIndex; $_l_SortChangeAlert; $_l_SortDirection; PDT_l_BookingsCount)
	...
	C_REAL:C285($_r_DenominatorNUM; $_r_NominatorNUM; $_r_Size; $_r_Values; SD2_R_AverageTotal; SD2_R_OrdersCostTotal; SD2_R_OrdersSalesTotal; SD2_R_PageTotal)
	C_TEXT:C284($_t_BookingPerson; $_t_CurrentDefinitionsName; $_t_Denominator; $_t_FIlterValue; $_t_Nominator; $_t_oldMethodName; $_t_OrderStatus; $_t_References; $_t_Stage; $_t_ThisValue; $_t_ValuesSTR)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [DIARY].Diary_PDTEditor.oSD2OrdersStages"; Form event code:C388)
...
If (SD2_at_BookingStageName>0)
	If (SD_At_otherFilterValues>0)
			...
			Case of 
				: (Type:C295($_Ptr_ThisColumnArray->)=Text array:K8:16) | (Type:C295($_Ptr_ThisColumnArray->)=String array:K8:15)
					If ($_Ptr_ThisColumnArray->{$_l_RowsIndex}=$_t_FIlterValue)
						APPEND TO ARRAY:C911($_al_OrderItemsNos; $_Ptr_ThisColumnArray2->{$_l_RowsIndex})
				...
				: (Type:C295($_Ptr_ThisColumnArray->)=Real array:K8:17) | (Type:C295($_Ptr_ThisColumnArray->)=Integer array:K8:18) | (Type:C295($_Ptr_ThisColumnArray->)=LongInt array:K8:19)
					$_r_Values:=$_Ptr_ThisColumnArray->{$_l_RowsIndex}
					$_t_ValuesSTR:=String:C10($_r_Values)
					
					If ($_t_FIlterValue=$_t_ValuesSTR)
						APPEND TO ARRAY:C911($_al_OrderItemsNos; $_Ptr_ThisColumnArray2->{$_l_RowsIndex})
						
					End if 
				: (Type:C295($_Ptr_ThisColumnArray->)=Date array:K8:20)
					$_d_ValueDate:=$_Ptr_ThisColumnArray->{$_l_RowsIndex}
					$_t_ValuesSTR:=String:C10($_d_ValueDate)
					If ($_t_FIlterValue=$_t_ValuesSTR)
						APPEND TO ARRAY:C911($_al_OrderItemsNos; $_Ptr_ThisColumnArray2->{$_l_RowsIndex})
					End if 
				: (Type:C295($_Ptr_ThisColumnArray->)=Boolean array:K8:21)
					...
					Else 
						$_t_ValuesSTR:="FALSE"
					End if 
					If ($_t_ValuesSTR=$_t_FIlterValue)
						APPEND TO ARRAY:C911($_al_OrderItemsNos; $_Ptr_ThisColumnArray2->{$_l_RowsIndex})
					End if 
			End case 
		...
		QUERY WITH ARRAY:C644([ORDER_ITEMS:25]Item_Number:27; $_al_OrderItemsNos)
	...
	If (SD2_at_OrdersPersons>0)
		$_t_BookingPerson:=SD2_at_BookingPerson{SD2_at_OrdersPersons}
		If ("~ALL"#$_t_BookingPerson)
			QUERY SELECTION:C341([ORDER_ITEMS:25]; [ORDER_ITEMS:25]Person:14=$_t_BookingPerson)
		End if 
	End if 
	...
	If (Size of array:C274(SD2_aptr_LBOrderItemsSetup)=0)
		LBi_ArrDefFill(->SD2_aptr_LBOrderItemsSetup; ->SD2_lb_OrderItems; ->[ORDER_ITEMS:25]Item_Number:27; ->[ORDERS:24]Order_Code:3; $_t_References; "Items_in"; 22; "Order Items"; "Order Item"; ""; $_bo_NewLevel; $_l_ReuseLevel)
...
End if 
ERR_MethodTrackerReturn("OBJ [DIARY].Diary_PDTEditor.oSD2OrdersStages"; $_t_oldMethodName)
