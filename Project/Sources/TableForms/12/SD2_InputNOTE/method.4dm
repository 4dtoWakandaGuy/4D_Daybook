If (False:C215)
	//Table Form Method Name: [DIARY]SD2_InputNOTE
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305(<>StatsSwitch; $_bo_Found; $_bo_Hightlight; DB_bo_NoLoad; SD_bo_NewRecord; SD_bo_SaturdayWork; SD_bo_SundayWork)
	C_DATE:C307($_d_StartDate; SD2_D_DisplayDateFrom; SD2_D_DisplayDateTo)
	C_LONGINT:C283(<>newSetsProcess; <>WT_SortFieldName_2; $_l_ButtonPressed; $_l_event; $_l_EventCall; $_l_Index; $_l_KeyPosition; $_l_MonitorCall; $_l_nul; $_l_OutCallBack; $_l_ProcessState)
	C_LONGINT:C283($_l_ProcessTime; $_l_Retries; $_l_RowIndex; $_l_SelectionRow; $_l_TableNumber; DB_l_ButtonClickedFunction; DB_l_OutCallBack; DiaryDetails; oOK; SD_l_CountTimes; SD_l_CurrentTab)
	C_LONGINT:C283(SD2_l_Dataform; SD2_l_EventCall; SM_l_MonitorCall; SM_l_TableNumber; SM_l_UpdateOnCallBackID; vNo; vNo2)
	C_POINTER:C301($_ptr_ArrayFieldPointers; $_ptr_ArrayofArrayPointers; $_Ptr_KeyField; $_Ptr_ThisColumnArray; SD2_ptr_LastObject; SD2_ptr_SearchField)
	C_REAL:C285(SD2_r_Timer; SRCH_r_TimerOLD)
	C_TEXT:C284(<>ButtDis; $_t_oldMethodName; $_t_ProcessName; $_t_SearchName; DB_t_CallOnCloseorSave; DiaryDetailsTXT; FS_t_SortFieldTXT; FS_t_SortVariableName; SD2_t_CreateActionCode; SD2_t_SELActionName; SD2_t_SRCH_Text)
	C_TEXT:C284(SD2_t_SRCH_TextOLD; SM_t_UseSetName; WIN_t_CurrentOutputform; WT_t_SortFieldVARNAME)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("FM [DIARY].SD2_InputNOTE"; Form event code:C388)
...
Case of 
	: ($_l_event=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		If (False:C215)
			...
			//Below to go back in with 4D live window
							...
							: ($_l_MonitorCall=6)
								...
								If (OK=1)  //Why are we doing this bit?
									...
									If (Size of array:C274($_al_CurrentSelection)>0)
										...
										For ($_l_RowIndex; 1; Size of array:C274($_Ptr_ThisColumnArray->))
											$_l_SelectionRow:=Find in array:C230($_al_CurrentSelection; $_l_RowIndex)
											If ($_l_SelectionRow>0)
												If ($_l_KeyPosition>0)
													APPEND TO ARRAY:C911($_at_DiaryRecordCodes; $_Ptr_ThisColumnArray->{$_l_RowIndex})
												End if 
											End if 
										End for 
...
End case 
ERR_MethodTrackerReturn("FM [DIARY].SD2_InputNOTE"; $_t_oldMethodName)
