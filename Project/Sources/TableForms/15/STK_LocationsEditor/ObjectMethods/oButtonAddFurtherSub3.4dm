If (False:C215)
	//object Method Name: Object Name:      [USER].STK_LocationsEditor.oButtonAddFurtherSub3
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 24/11/2009 20:51
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CurrentLevel; $_l_ElementsPerParent; $_l_HeaderLevel; $_l_Index; $_l_Index2; $_l_IndexD; $_l_LocationElements; $_l_ParentIndex; $_l_SelectedRow; $_l_SubLocationPosition; STK_l_Subid)
	...
	C_POINTER:C301($_ptr_SubLevelEdit; $_Ptr_SubLevelLBArea; $_ptr_SubLevelSelect; $_Ptr_SublocationCode)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub3"; Form event code:C388)
...
If ($_ptr_SubLevelSelect->>0)
			...
			If ($_t_SublevelLabel#"") & (OK=1)
				...
				If ($_t_SubLocationCode#"") & (OK=1)
					...
					If (Num:C11($_t_CountSubLevelsSTR)>0)
						...
						For ($_l_ParentIndex; 1; Size of array:C274($_al_SubLocationsWithnoSubLevel))
							QUERY:C277([LOCATIONS:61]; [LOCATIONS:61]LocationID:4=$_al_SubLocationsWithnoSubLevel{$_l_ParentIndex})
							...
							For ($_l_Index; 1; $_l_ElementsPerParent)
								...
								Else 
									...
									End if 
								End if 
								$_ptr_Array6->{$_l_Index}:=$_al_SubLocationsWithnoSubLevel{$_l_ParentIndex}
								...
								[LOCATIONS:61]ParentLocationID:3:=$_al_SubLocationsWithnoSubLevel{$_l_ParentIndex}
	...
	Else 
		...
		Repeat 
			...
			For ($_l_Index; 1; Size of array:C274($_ptr_Array2->))
				...
				$_l_IndexD:=$_ptr_Array2->{$_l_Index}
				QUERY:C277([LOCATIONS:61]; [LOCATIONS:61]ParentLocationID:3=$_l_IndexD)
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub3"; $_t_oldMethodName)
