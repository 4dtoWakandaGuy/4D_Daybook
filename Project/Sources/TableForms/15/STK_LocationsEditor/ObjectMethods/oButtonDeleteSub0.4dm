If (False:C215)
	//object Name: [USER]STK_LocationsEditor.oButtonDeleteSub0
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_HeaderLevel; $_l_LocationPosition; $_l_SelectedRow)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oButtonDeleteSub0"; Form event code:C388)

$_l_HeaderLevel:=FORM Get current page:C276
$_l_HeaderLevel:=$_l_HeaderLevel-1
...
$_ptr_SubLevelSelect:=Get pointer:C304("STK_l_SubLevelSelected"+String:C10($_l_HeaderLevel))
$_ptr_ProdSelected:=Get pointer:C304("STK_l_ProdatlocSelected"+String:C10($_l_HeaderLevel))
...
If ($_ptr_SubLevelSelect->>0)
	...
	If (OK=0)
		...
		Else 
			...
			LISTBOX DELETE ROWS:C914($_Ptr_SubLevelLBArea->; $_l_SelectedRow)
			...
			If (Size of array:C274($_Ptr_LocationNamesArray->)=0)
				...
				$_Ptr_SubLocationNamesArray->{$_l_LocationPosition}:=""
...
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oButtonDeleteSub0"; $_t_oldMethodName)
