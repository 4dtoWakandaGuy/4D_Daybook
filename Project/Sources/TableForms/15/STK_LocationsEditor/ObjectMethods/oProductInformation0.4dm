If (False:C215)
	//object Name: [USER]STK_LocationsEditor.oProductInformation0
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_ColumnNumber; $_l_CurrentLevel; $_l_event; $_l_FieldNumber; $_l_HeaderLevel; $_l_LocationIDRow; $_l_OldProductID; $_l_SelectedRow2; $_l_TableNumber)
	...
	C_TEXT:C284($_t_ColumnName; $_t_oldMethodName; $_t_ProdCodeLabelName; $_t_ProdCountLabelName; $_t_ProdNameLabelName; $_t_SubLevelLabelName; $ProductCodeEntered)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oProductInformation0"; Form event code:C388)
...
Case of 
	: ($_l_event=On Clicked:K2:4)
		...
		$_l_LocationIDRow:=Self:C308->
		//Column FIVE contains the ID
		$_ptr_ProdSelected->:=$_ptr_ProductCodeArray->{$_l_LocationIDRow}
	...
	: ($_l_event=On Data Change:K2:15)
		...
		Case of 
			: ($_l_ColumnNumber=1)  //The product code(no other column should be editable
				...
				$ProductCodeEntered:=$_ptr_ProductCodes->{$_l_SelectedRow2}
				STK_bo_ProductFound:=False:C215
				Check_Product(->$ProductCodeEntered; "Product"; ->STK_bo_ProductFound)
				If (STK_bo_ProductFound)
					...
					If ($_bo_Continue)
						...
						$_ptr_ProductCodes->{$_l_SelectedRow2}:=[PRODUCTS:9]Product_Code:1
						$_ptr_ProductNames->{$_l_SelectedRow2}:=[PRODUCTS:9]Product_Name:2
						$_ptr_ProductIDS->{$_l_SelectedRow2}:=[PRODUCTS:9]X_ID:43
						$_ptr_ProductCounts->{$_l_SelectedRow2}:=0
						...
						$_l_LocationIDRow:=Find in array:C230($_Ptr_LocationIDSArray->; $_ptr_CurrentLocationID->)
						If (Size of array:C274($_ptr_ProductCodes->)=1)
							$_Ptr_ProdCodeArray->{$_l_LocationIDRow}:=[PRODUCTS:9]Product_Code:1
							$_Ptr_ProdNameArray->{$_l_LocationIDRow}:=[PRODUCTS:9]Product_Name:2
							$_Ptr_LocationHoldsArray->{$_l_LocationIDRow}:=[PRODUCTS:9]Product_Code:1+", "+[PRODUCTS:9]Product_Name:2
						Else 
							$_Ptr_ProdCodeArray->{$_l_LocationIDRow}:="Multiple Products"
							$_Ptr_ProdNameArray->{$_l_LocationIDRow}:="Select Location to see product range"
							$_Ptr_LocationHoldsArray->{$_l_LocationIDRow}:=$_Ptr_ProdCodeArray->{$_l_LocationIDRow}+", "+$_Ptr_ProdNameArray->{$_l_LocationIDRow}
				...
				Else 
					...
					Else 
						$_ptr_ProductCodes->{$_l_SelectedRow2}:=""
						$_ptr_ProductNames->{$_l_SelectedRow2}:=""
						$_ptr_ProductIDS->{$_l_SelectedRow2}:=0
						$_ptr_ProductCounts->{$_l_SelectedRow2}:=0
						...
						If (Size of array:C274($_ptr_ProductCodes->)<2)  //NOTE THIS
							$_Ptr_ProdCodeArray->{$_l_LocationIDRow}:=$_ptr_ProductCodes->{1}
							$_Ptr_ProdNameArray->{$_l_LocationIDRow}:=$_ptr_ProductNames->{1}
							If ($_ptr_ProductCodes->{1}#"")
								$_Ptr_LocationHoldsArray->{$_l_LocationIDRow}:=$_ptr_ProductCodes->{1}+", "+$_ptr_ProductNames->{1}
							Else 
								$_Ptr_LocationHoldsArray->{$_l_LocationIDRow}:=""
							End if 
...
End case 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oProductInformation0"; $_t_oldMethodName)
