If (False:C215)
	//object Method Name: Object Name:      [USER].STK_LocationsEditor.oButtonAddFurtherSub0
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	ARRAY TEXT:C222($_at_NextLevelNameTEMP; 0)
	C_BOOLEAN:C305($_bo_AddSubLevels; $_bo_SubLevelsCreated)
	C_LONGINT:C283($_l_CurrentElement; $_l_CurrentLevel; $_l_Elements; $_l_ElementsPerParent; $_l_HeaderLevel; $_l_Index; $_l_Index2; $_l_LocationID; $_l_ParentsIndex; $_l_SelectedRow; $_l_SubLocationPosition)
	C_LONGINT:C283($_l_SubLocationRow; STK_l_Subid)
	...
	C_POINTER:C301($_ptr_NextLevel4; $_ptr_NextLevel5; $_ptr_NextLevel6; $_ptr_NextLevel7; $_ptr_NextLevel8; $_ptr_NextLevel9; $_ptr_NextP1; $_ptr_NextP10; $_ptr_NextP2; $_ptr_NextP3; $_ptr_NextP4)
	C_POINTER:C301($_ptr_NextP5; $_ptr_NextP6; $_ptr_NextP7; $_ptr_NextP8; $_ptr_NextP9; $_ptr_ProdSelected; $_ptr_ProductAdd; $_ptr_ProductCode; $_ptr_ProductCodes; $_ptr_ProductCount; $_ptr_ProductCounts)
	...
	C_POINTER:C301($_Ptr_SubLevelLBArea; $_ptr_SubLevelSelect; $_Ptr_SublocationCode; $_Ptr_SubLocationNamesArray; SublocationsnamesPtr)
	C_TEXT:C284($_t_CountSubLevelsSTR; $_t_oldMethodName; $_t_ParentLocationCode; $_t_ProdCodeLabelName; $_t_ProdCountLabelName; $_t_ProdNameLabelName; $_t_Singular; $_t_SublevelLabel; $_t_SubLevelLabelName; $_t_SubLocationCode; $_t_SubLocationDefault)
	C_TEXT:C284($_t_SubLocationName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub0"; Form event code:C388)
...
If ($_ptr_SubLevelSelect->>0)
	...
	$_ptr_Array2:=$_aptr_ColumnVariables{7}  // Location ID
	$_ptr_Array4:=$_aptr_ColumnVariables{6}  //Sub locatons named
	...
	$_t_SubLocationName:=$_ptr_CurrentLocationSubLevName->
	ARRAY LONGINT:C221($_al_SubLocationsWithnoSubLevel; 0)
	ARRAY TEXT:C222($_at_NamedSubLocations; 0)
	For ($_l_Index; 1; Size of array:C274($_ptr_Array9->))
		...
		Else 
			If ($_ptr_Array4->{$_l_Index}#"No Sublocations")
				$_l_SubLocationRow:=Find in array:C230($_at_NamedSubLocations; $_ptr_Array4->{$_l_Index})
				If ($_l_SubLocationRow<0)
	...
	If (Size of array:C274($_al_SubLocationsWithnoSubLevel)>0)
		...
		If (Size of array:C274($_al_SubLocationsWithnoSubLevel)<Size of array:C274($_ptr_Array2->))
			...
			Gen_Confirm("Some "+$_t_SubLocationName+" Already have sublevels, do you wish to add sublevels only to those with no suble"+"vel"; "No"; "Yes")
			If (OK=0)
		...
		End if 
		If ($_bo_AddSubLevels)
			$_t_SublevelLabel:=Gen_Request("How are the "+$_t_SubLocationName+" divided?"; "")
			//```
			If ($_t_SublevelLabel#"") & (OK=1)
				...
				If ($_t_SubLocationCode#"") & (OK=1)
					...
					If (Num:C11($_t_CountSubLevelsSTR)>0)
						$_l_Elements:=Size of array:C274($_al_SubLocationsWithnoSubLevel)*Num:C11($_t_CountSubLevelsSTR)
						...
						ARRAY TEXT:C222($_Ptr_LocationNamesArray->; $_l_Elements)
						ARRAY LONGINT:C221($_ptr_Array2->; $_l_Elements)
						ARRAY TEXT:C222($_ptr_Array3->; $_l_Elements)
						ARRAY TEXT:C222($_ptr_Array4->; $_l_Elements)
						ARRAY LONGINT:C221($_ptr_Array5->; $_l_Elements)
						ARRAY LONGINT:C221($_ptr_Array6->; $_l_Elements)
						ARRAY BOOLEAN:C223($_ptr_Array7->; $_l_Elements)
						ARRAY TEXT:C222($_ptr_Array8->; $_l_Elements)
						ARRAY TEXT:C222($_ptr_Array9->; $_l_Elements)
						ARRAY TEXT:C222($_ptr_Array10->; $_l_Elements)
						$_l_CurrentElement:=1
						For ($_l_ParentsIndex; 1; Size of array:C274($_al_SubLocationsWithnoSubLevel))
							...
							$_t_ParentLocationCode:=[LOCATIONS:61]Location_Code:1
							For ($_l_Index; 1; $_l_ElementsPerParent)
								If (Size of array:C274($_ptr_Array3->)=0)
								Else 
									If ($_ptr_Array3->{$_l_Index}="")  //Location not coded yet
										$_ptr_Array3->{$_l_Index}:=$_t_ParentLocationCode+$_t_SubLocationCode+String:C10($_l_Index)
									End if 
									If ($_Ptr_LocationNamesArray->{$_l_Index}="")
										$_Ptr_LocationNamesArray->{$_l_Index}:=$_t_ParentLocationCode+", "+$_t_singular+" "+String:C10($_l_Index)
									End if 
								...
								[LOCATIONS:61]LocationCodePrefix:5:=$_t_ParentLocationCode
								...
								If ($_l_SubLocationPosition>0)
									$_Ptr_SubLocationNamesArray->{$_l_SubLocationPosition}:=[LOCATIONS:61]Sub_location_Labelled:6
	...
	Else 
		...
		Repeat 
			...
			For ($_l_Index; 1; Size of array:C274($_ptr_Array2->))
				$_t_SubLocationName:=$_ptr_Array4->{$_l_Index}
				$_l_LocationID:=$_ptr_Array2->{$_l_Index}
				QUERY:C277([LOCATIONS:61]; [LOCATIONS:61]ParentLocationID:3=$_l_LocationID)
				SELECTION TO ARRAY:C260([LOCATIONS:61]LocationID:4; $_al_NextLevelIDsTemp; [LOCATIONS:61]Sub_location_Labelled:6; $_at_NextLevelLabelTEMP; [LOCATIONS:61]Location_Name:2; $_at_NextLevelNameTEMP; [LOCATIONS:61]Location_Code:1; $_at_NextLevelCodesTEMP; [LOCATIONS:61]ParentLocationID:3; $_al_NextLevelParentIDTEMP)
				For ($_l_Index2; 1; Size of array:C274($_al_NextLevelIDsTemp))
					APPEND TO ARRAY:C911($_ptr_NextLevel1->; $_at_NextLevelNameTEMP{$_l_Index2})
			...
			For ($_l_Index; 1; $_ptr_NextLevel2->)
				...
				Else 
					If ($_ptr_NextLevel4->{$_l_Index}#"No Sublocations")
						$_l_SubLocationRow:=Find in array:C230($_at_NamedSubLocations; $_ptr_NextLevel4->{$_l_Index})
						If ($_l_SubLocationRow<0)
			...
			If (Size of array:C274($_al_SubLocationsWithnoSubLevel)>0)
				...
				End if 
				If ($_bo_AddSubLevels)
					$_t_SublevelLabel:=Gen_Request("How is the "+$_t_SubLocationName+" divided?"; "Bins")
					//```
					If ($_t_SublevelLabel#"") & (OK=1)
						$_t_Singular:=GenWordSingle($_t_SublevelLabel)
						Case of   // These default names are just so we can populate something into the request box that looks sensible
						...
						If ($_t_SubLocationCode#"") & (OK=1)
							$_t_CountSubLevelsSTR:=Gen_Request("How many "+$_t_SublevelLabel; "1")
							If (Num:C11($_t_CountSubLevelsSTR)>0)
								$_l_Elements:=$_al_SubLocationsWithnoSubLevel*Num:C11($_t_CountSubLevelsSTR)
								$_l_ElementsPerParent:=Num:C11($_t_CountSubLevelsSTR)
								
								$_ptr_NextP1:=Get pointer:C304("STK_at_SubLocationNames"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP2:=Get pointer:C304("STK_al_SubLocationsIDs"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP3:=Get pointer:C304("STK_at_SubLocationCodes"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP4:=Get pointer:C304("STK_at_SubLevelLabel"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP5:=Get pointer:C304("STK_al_SubCodingPref"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP6:=Get pointer:C304("STK_al_SubParentLocation"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP7:=Get pointer:C304("STK_abo_SUBRECORDMODIFIED"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP8:=Get pointer:C304("STK_at_SubLocProdCode"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP9:=Get pointer:C304("STK_at_SubLocProdName"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextP10:=Get pointer:C304("STK_at_SubLocHolds"+String:C10($_l_CurrentLevel+1))
								ARRAY TEXT:C222($_ptr_NextP1->; $_l_Elements)
								ARRAY LONGINT:C221($_ptr_NextP2->; $_l_Elements)
								ARRAY TEXT:C222($_ptr_NextP3->; $_l_Elements)
								ARRAY TEXT:C222($_ptr_NextP4->; $_l_Elements)
								ARRAY LONGINT:C221($_ptr_NextP5->; $_l_Elements)
								ARRAY LONGINT:C221($_ptr_NextP6->; $_l_Elements)
								ARRAY BOOLEAN:C223($_ptr_NextP7->; $_l_Elements)
								ARRAY TEXT:C222($_ptr_NextP8->; $_l_Elements)
								ARRAY TEXT:C222($_ptr_NextP9->; $_l_Elements)
								ARRAY TEXT:C222($_ptr_NextP10->; $_l_Elements)
								$_l_CurrentElement:=1
								For ($_l_ParentsIndex; 1; Size of array:C274($_al_SubLocationsWithnoSubLevel))
									QUERY:C277([LOCATIONS:61]; [LOCATIONS:61]LocationID:4=$_al_SubLocationsWithnoSubLevel{$_l_ParentsIndex})
									$_t_ParentLocationCode:=[LOCATIONS:61]Location_Code:1
									For ($_l_Index; 1; $_l_ElementsPerParent)
										...
										$_l_SubLocationPosition:=Find in array:C230($_Ptr_SublocationCode->; [LOCATIONS:61]Location_Code:1)
										If ($_l_SubLocationPosition>0)
											SublocationsnamesPtr->{$_l_SubLocationPosition}:=[LOCATIONS:61]Sub_location_Labelled:6
											$_Ptr_LocationHoldsArray->{$_l_SubLocationPosition}:=[LOCATIONS:61]Sub_location_Labelled:6
										End if 
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub0"; $_t_oldMethodName)
