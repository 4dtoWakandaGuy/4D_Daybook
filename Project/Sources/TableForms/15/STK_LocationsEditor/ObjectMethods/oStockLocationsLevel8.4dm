If (False:C215)
	//object Name: [USER]STK_LocationsEditor.oStockLocationsLevel8
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 25/11/2009 08:29
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CurrentLevel; $_l_event; $_l_FieldNumber; $_l_FillCounts; $_l_HeaderLevel; $_l_Index; $_l_Row; $_l_SizeofArray; $_l_TableNumber; PLOC0_l_BUT1; PLOC0_l_BUT10)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oStockLocationsLevel8"; Form event code:C388)
...
Case of 
	: ($_l_event=On Clicked:K2:4)
		...
		//`
		Case of 
			: ($_Ptr_SubLocationNamesArray->{$_l_Row}#"")  //This location has a sublevel named
				//``
				...
				QUERY:C277([LOCATIONS:61]; [LOCATIONS:61]ParentLocationID:3=$_Ptr_LocationIDSArray->{$_l_Row})
				...
				$_l_SizeofArray:=Size of array:C274($_ptr_Array3->)
				ARRAY TEXT:C222($_ptr_Array8->; $_l_SizeofArray)
				ARRAY TEXT:C222($_ptr_Array9->; $_l_SizeofArray)
				ARRAY BOOLEAN:C223($_ptr_Array7->; $_l_SizeofArray)
				//ARRAY REAL($P11->;$_l_SizeofArray)
				ARRAY TEXT:C222($_ptr_Array10->; $_l_SizeofArray)
			...
			: ($_Ptr_ProdCodeArray->{$_l_Row}#"")
				...
				If ($_Ptr_ProdCodeArray->{$_l_Row}="Multiple Products")
					ARRAY LONGINT:C221($_ptr_ProductIDS->; 0)
					ARRAY TEXT:C222($_ptr_ProductCodes->; 0)
					ARRAY TEXT:C222($_ptr_ProductNames->; 0)
					ARRAY REAL:C219($_ptr_ProductCounts->; 0)
					...
					$_t_LocationCode:=$_Ptr_LocationCode->{$_l_Row}
		...
		$_ptr_ProdSelected->:=0
		$_ptr_CurrentLocation->:=$_ptr_LocationNameArray->{$_l_Row}
		$_ptr_CurrentLocationCode->:=$_Ptr_LocationCode->{$_l_Row}
		$_ptr_CurrentLocationID->:=$_Ptr_LocationIDSArray->{$_l_Row}
		$_ptr_CurrentLocationSubLevName->:=$_Ptr_SubLocationNamesArray->{$_l_Row}
		OBJECT SET ENTERABLE:C238(STK_t_CurrentLocation0; True:C214)
		
		//``
		
End case 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oStockLocationsLevel8"; $_t_oldMethodName)
