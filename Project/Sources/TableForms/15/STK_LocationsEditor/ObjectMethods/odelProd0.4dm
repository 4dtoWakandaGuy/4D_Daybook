If (False:C215)
	//object Name: [USER]STK_LocationsEditor.odelProd0
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CurrentLevel; $_l_HeaderLevel; $_l_LocationPosition; $_l_Row)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.odelProd0"; Form event code:C388)
...
If (Size of array:C274($_ptr_ProductCodes->)>0)
	If ($_ptr_ProdSelected->>0)
		$_l_Row:=Find in array:C230($_ptr_ProductIDS->; $_ptr_ProdSelected->)
		If ($_l_Row>0)
			...
			If (Records in selection:C76([x_ProductLocations:185])>0)
				If ($_ptr_ProductCounts->{$_l_Row}>0)
			...
			End if 
			LISTBOX DELETE ROWS:C914($_ptr_ProductList->; $_l_Row)
			$_ptr_ProdSelected->:=0
			...
			If (Size of array:C274($_ptr_ProductIDS->)=0)  //Removed the last product from the location
				...
				$_l_Row:=Find in array:C230($_Ptr_LocationIDSArray->; $_ptr_CurrentLocationID->)
				$_Ptr_ProdCodeArray->{$_l_Row}:=""
				$_Ptr_ProdNameArray->{$_l_Row}:=""
				$_Ptr_LocationHoldsArray->{$_l_Row}:="Unknown"
			...
			Else 
				If (Size of array:C274($_ptr_ProductIDS->)=1)  //only one product
					...
					$_Ptr_LocationIDSArray:=$_aptr_ColumnVariables{7}  //Pointer to array containing the ids
					$_l_Row:=Find in array:C230($_Ptr_LocationIDSArray->; $_ptr_CurrentLocationID->)
					$_Ptr_ProdCodeArray->{$_l_Row}:=$_ptr_ProductCodes->{1}
					$_Ptr_ProdNameArray->{$_l_Row}:=$_ptr_ProductNames->{1}
					$_Ptr_LocationHoldsArray->{$_l_Row}:=$_ptr_ProductCodes->{1}+", "+$_ptr_ProductNames->{1}
...
Else 
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.odelProd0"; $_t_oldMethodName)
