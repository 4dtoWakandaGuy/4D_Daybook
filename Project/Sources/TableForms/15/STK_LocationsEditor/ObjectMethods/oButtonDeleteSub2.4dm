If (False:C215)
	//object Name: [USER]STK_LocationsEditor.oButtonDeleteSub2
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CurrentLevel; $_l_HeaderLevel; $_l_SelectedRow)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oButtonDeleteSub2"; Form event code:C388)
	...
	If (OK=0)
		...
		Else 
			...
			If (Size of array:C274($_Ptr_LocationNamesArray->)=0)
				...
				$_l_SelectedRow:=Find in array:C230($_Ptr_LocationIDSArray->; $_ptr_CurrentLocationID->)
				$_Ptr_ProdCodeArray->{$_l_SelectedRow}:=""
				$_Ptr_ProdNameArray->{$_l_SelectedRow}:=""
				$_Ptr_LocationHoldsArray->{$_l_SelectedRow}:="Unknown"
				$_Ptr_SubLocationNamesArray->{$_l_SelectedRow}:=""
...
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oButtonDeleteSub2"; $_t_oldMethodName)
