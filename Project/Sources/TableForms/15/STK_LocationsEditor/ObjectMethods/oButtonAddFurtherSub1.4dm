If (False:C215)
	//object Method Name: Object Name:      [USER].STK_LocationsEditor.oButtonAddFurtherSub1
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 24/11/2009 20:26
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CurrentLevel; $_l_ElementsPerParent; $_l_HeaderLevel; $_l_Index; $_l_Index2; $_l_LocationElements; $_l_LocationID; $_l_LocationRow; $_l_ParentsIndex; $_l_SelectedRow; $_l_SubLocationPosition)
	...
	C_POINTER:C301($_ptr_NextLevel4; $_ptr_NextLevel5; $_ptr_NextLevel6; $_ptr_NextLevel7; $_ptr_NextLevel8; $_ptr_NextLevel9; $_ptr_ProdSelected; $_ptr_ProductAdd; $_ptr_ProductCode; $_ptr_ProductCodes; $_ptr_ProductCount)
	...
	C_POINTER:C301($_ptr_SubLevelEdit; $_Ptr_SubLevelLBArea; $_ptr_SubLevelSelect; $_Ptr_SublocationCode; $_Ptr_SubLocationNamesArray)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub1"; Form event code:C388)
...
If ($_ptr_SubLevelSelect->>0)
	...
	For ($_l_Index; 1; Size of array:C274($_ptr_Array9->))
		...
		Else 
			If ($_ptr_Array4->{$_l_Index}#"No Sublocations")
				$_l_LocationRow:=Find in array:C230($_at_NamedSubLocations; $_ptr_Array4->{$_l_Index})
				If ($_l_LocationRow<0)
			...
			If ($_t_SublevelLabel#"") & (OK=1)
				...
				If ($_t_SubLocationCode#"") & (OK=1)
					...
					If (Num:C11($_t_CountSubLevelsSTR)>0)
						$_l_LocationElements:=Size of array:C274($_al_SubLocationsWithnoSubLevel)*Num:C11($_t_CountSubLevelsSTR)
						...
						ARRAY TEXT:C222($_ptr_Array3->; $_l_LocationElements)
	...
	Else 
		...
		Repeat 
			...
			For ($_l_Index; 1; Size of array:C274($_ptr_Array2->))
				$_t_LocationName:=$_ptr_Array4->{$_l_Index2}
				$_l_LocationID:=$_ptr_Array2->{$_l_Index2}
			...
			For ($_l_Index; 1; $_ptr_NextLevel2->)
				...
				Else 
					If ($_ptr_NextLevel4->{$_l_Index}#"No Sublocations")
						$_l_LocationRow:=Find in array:C230($_at_NamedSubLocations; $_ptr_NextLevel4->{$_l_Index})
						If ($_l_LocationRow<0)
					...
					If ($_t_SublevelLabel#"") & (OK=1)
						...
						If ($_t_SubLocationCode#"") & (OK=1)
							...
							If (Num:C11($_t_CountSubLevelsSTR)>0)
								...
								$_ptr_NextLevel9:=Get pointer:C304("STK_at_SubLocProdName"+String:C10($_l_CurrentLevel+1))
								$_ptr_NextLevel10:=Get pointer:C304("STK_at_SubLocHolds"+String:C10($_l_CurrentLevel+1))
								ARRAY TEXT:C222($_ptr_NextLevel1->; $_l_LocationElements)
								ARRAY LONGINT:C221($_ptr_NextLevel2->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevel3->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevel4->; $_l_LocationElements)
								ARRAY LONGINT:C221($_ptr_NextLevel5->; $_l_LocationElements)
								ARRAY LONGINT:C221($_ptr_NextLevel6->; $_l_LocationElements)
								ARRAY BOOLEAN:C223($_ptr_NextLevel7->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevel8->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevel9->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevel10->; $_l_LocationElements)
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub1"; $_t_oldMethodName)
