If (False:C215)
	//object Method Name: Object Name:      [USER].STK_LocationsEditor.oButtonEditSub2
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 24/11/2009 20:41
...
If (True:C214)
	...
	C_LONGINT:C283($_l_ProductPosition; $_l_ProductsCount; $_l_Row; $_l_SizeofArray; $_l_ThisLocationID; $_l_WindowBottom; $_l_WindowLeft; $_l_WindowRight; $_l_windowTop; STK_l_Subid; xNext)
	...
	C_TEXT:C284($_t_NextSubLevelNamed; $_t_NextSubLocationCode; $_t_oldMethodName; $_t_ProdCodeLabelName; $_t_ProdCountLabelName; $_t_ProdNameLabelName; $_t_ProductCode; $_t_SelectedOption; $_t_Singular; $_t_SubLevelLabelName; $_t_SubLocationDefault)
	C_TEXT:C284(STK_t_ProductCode)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oButtonEditSub2"; Form event code:C388)
...
If ($_ptr_SubLevelSelect->>0)
	...
	$_l_Row:=Find in array:C230($_ptr_Array2->; $_ptr_SubLevelSelect->)
	
	$_t_LocationName:=$_ptr_Array3->{$_l_Row}
	
	$_t_NextSubLevelNamed:=$_ptr_Array4->{$_l_Row}
	$_t_ProductCode:=$_ptr_Array8->{$_l_Row}
	Case of 
		: ($_t_NextSubLevelNamed#"")  //The location has a sublevel
			...
			$_l_ThisLocationID:=$_ptr_Array2->{$_l_Row}
			...
			$_ptr_ThisLocationName->:=$_Ptr_LocationNamesArray->{$_l_Row}
			//````
				...
				Case of 
					...
					: ($_t_SelectedOption="Add Additional Product to Location")
						...
						If ($_t_ProductCode#"") & (OK=1)
							...
							If (STK_bo_ProductFound)
								If ($_ptr_Array8->{$_l_Row}#"Multiple Products")
									CREATE RECORD:C68([x_ProductLocations:185])
									[x_ProductLocations:185]x_LocationCode:2:=$_ptr_Array3->{$_l_Row}
									[x_ProductLocations:185]x_ProductID:1:=[PRODUCTS:9]X_ID:43
									DB_SaveRecord(->[x_ProductLocations:185])
									
									$_ptr_Array8->{$_l_Row}:="Multiple Products"
									$_ptr_Array9->{$_l_Row}:="Select Location to see product range"
									$_ptr_Array7->{$_l_Row}:=True:C214
									$_ptr_Array10->{$_l_Row}:=$_ptr_Array8->{$_l_Row}+", "+$_ptr_Array8->{$_l_Row}
								Else 
									CREATE RECORD:C68([x_ProductLocations:185])
									[x_ProductLocations:185]x_LocationCode:2:=$_ptr_Array3->{$_l_Row}
									[x_ProductLocations:185]x_ProductID:1:=[PRODUCTS:9]X_ID:43
					...
					: ($_t_SelectedOption="Remove Product from Location")
						...
						If ($_ptr_Array8->{$_l_Row}#"Multiple Products")
							...
							QUERY:C277([x_ProductLocations:185]; [x_ProductLocations:185]x_ProductID:1=$_ptr_Array8->{$_l_Row}; *)
							QUERY:C277([x_ProductLocations:185];  & ; [x_ProductLocations:185]x_LocationCode:2=$_ptr_Array3->{$_l_Row})
							If (Records in selection:C76([x_ProductLocations:185])>0)
								STK_GetActiveThreads($_ptr_Array8->{$_l_Row}; ""; 0; "ANY"; "ANY"; "ANY"; 0; 0; ""; 0; 0; ""; ""; $_ptr_Array3->{$_l_Row})
						...
						Else 
							...
							If ($_t_SelectedOption#"") & (OK=1)
								...
								If (Records in selection:C76([x_ProductLocations:185])=1)
									QUERY:C277([PRODUCTS:9]; [PRODUCTS:9]X_ID:43=[x_ProductLocations:185]x_ProductID:1)
									$_ptr_Array8->{$_l_Row}:=[PRODUCTS:9]Product_Code:1
									$_ptr_Array9->{$_l_Row}:=[PRODUCTS:9]Product_Name:2
									$_ptr_Array7->{$_l_Row}:=True:C214
									$_ptr_Array10->{$_l_Row}:=[PRODUCTS:9]Product_Code:1+", "+[PRODUCTS:9]Product_Name:2
								Else 
									
									$_ptr_Array8->{$_l_Row}:="Multiple Products"
									$_ptr_Array9->{$_l_Row}:="Select Location to see product range"
									$_ptr_Array7->{$_l_Row}:=True:C214
									$_ptr_Array10->{$_l_Row}:=$_ptr_Array8->{$_l_Row}+", "+$_ptr_Array9->{$_l_Row}
								End if 
							End if 
		...
		Else 
			...
			Case of 
				...
				: (OK=0)  //Sublocation
					...
					//STK_T_CurrentLocation1:=$_Ptr_LocationNamesArray->{$_l_Row}
					//``
					ARRAY TEXT:C222($_at_ColumnNames; 0)
					ARRAY TEXT:C222($_at_HeaderNames; 0)
					ARRAY POINTER:C280($_aptr_ColumnVariables; 0)
					...
					If ($_t_NextSublevelLabel#"")
						...
						If ($_t_NextSubLocationCode#"") & (OK=1)
							...
							If (Num:C11($_t_CountNextSubLevelsSTR)>0)
								$_ptr_Array4->{$_l_Row}:=$_t_NextSublevelLabel
								$_ptr_Array10->{$_l_Row}:=$_t_NextSublevelLabel
								...
								For ($_l_FillArray; 1; Size of array:C274($_ptr_SubLocationHolds->))
									$_Ptr_SubLocationCodes->{$_l_FillArray}:=$_ptr_Array3->{$_l_Row}+$_t_NextSubLocationCode+String:C10($_l_FillArray)
									$_ptr_SubLocationNames->{$_l_FillArray}:=$_Ptr_LocationNamesArray->{$_l_Row}+","+$_t_singular+" "+String:C10($_l_FillArray)
									STK_l_Subid:=0
									...
									[LOCATIONS:61]LocationCodePrefix:5:=$_ptr_Array3->{$_l_Row}  //This is correct
...
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oButtonEditSub2"; $_t_oldMethodName)
