If (False:C215)
	//object Name: [USER]STK_LocationsEditor.oButtonAddFurtherSub9
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 25/11/2009 09:13
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CurrentLevel; $_l_ElementsPerParent; $_l_HeaderLevel; $_l_ID; $_l_index; $_l_index2; $_l_LocationElements; $_l_ParentsIndex; $_l_SelectedRow; $_l_SubLocationPosition; STK_l_Subid)
	...
	C_TEXT:C284($_t_CountSubLevelsSTR; $_t_LocationsName; $_t_oldMethodName; $_t_ParentLocationCode; $_t_ProdCodeLabelName; $_t_ProdCountLabelName; $_t_ProdNameLabelName; $_t_Singular; $_t_SublevelLabel; $_t_SubLevelLabelName; $_t_SubLocationCode)
	C_TEXT:C284($_t_SubLocationDefault)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub9"; Form event code:C388)
...
If ($_ptr_SubLevelSelect->>0)
	...
	$_t_LocationsName:=$_ptr_CurrentLocationSubLevName->
	ARRAY LONGINT:C221($_al_SubLocationsWithnoSubLevel; 0)
	ARRAY TEXT:C222($_at_NamedSubLocations; 0)
	...
	If (Size of array:C274($_al_SubLocationsWithnoSubLevel)>0)
		...
		If (Size of array:C274($_al_SubLocationsWithnoSubLevel)<Size of array:C274($_ptr_Array2->))
			...
			Gen_Confirm("Some "+$_t_LocationsName+" Already have sublevels, do you wish to add sublevels only to those with no suble"+"vel"; "No"; "Yes")
			If (OK=0)
		...
		End if 
		If ($_bo_AddSubLevels)
			$_t_SublevelLabel:=Gen_Request("How are the "+$_t_LocationsName+" divided?"; "")
			//```
			If ($_t_SublevelLabel#"") & (OK=1)
				...
				If ($_t_SubLocationCode#"") & (OK=1)
					...
					If (Num:C11($_t_CountSubLevelsSTR)>0)
						$_l_LocationElements:=$_al_SubLocationsWithnoSubLevel*Num:C11($_t_CountSubLevelsSTR)
						...
						ARRAY TEXT:C222($_Ptr_LocationNamesArray->; $_l_LocationElements)
						ARRAY LONGINT:C221($_ptr_Array2->; $_l_LocationElements)
						ARRAY TEXT:C222($_Ptr_LocationCodesArray->; $_l_LocationElements)
						ARRAY TEXT:C222($_ptr_Array4->; $_l_LocationElements)
						ARRAY LONGINT:C221($_ptr_Array5->; $_l_LocationElements)
						ARRAY LONGINT:C221($_ptr_Array6->; $_l_LocationElements)
						ARRAY BOOLEAN:C223($_ptr_Array7->; $_l_LocationElements)
						ARRAY TEXT:C222($_ptr_Array8->; $_l_LocationElements)
						ARRAY TEXT:C222($_ptr_Array9->; $_l_LocationElements)
						ARRAY TEXT:C222($_ptr_Array10->; $_l_LocationElements)
						...
						For ($_l_ParentsIndex; 1; Size of array:C274($_al_SubLocationsWithnoSubLevel))
							...
							For ($_l_index; 1; $_l_ElementsPerParent)
								If ($_Ptr_LocationCodesArray->{$_l_index}="")  //Location not coded yet
									$_Ptr_LocationCodesArray->{$_l_index}:=$_t_ParentLocationCode+$_t_SubLocationCode+String:C10($_l_index)
								End if 
	...
	Else 
		...
		Repeat 
			...
			For ($_l_index; 1; Size of array:C274($_ptr_Array2->))
				$_t_LocationsName:=$_ptr_Array4->{$_l_index}
			...
			If (Size of array:C274($_al_SubLocationsWithnoSubLevel)>0)
				...
				End if 
				If ($_bo_AddSubLevels)
					$_t_SublevelLabel:=Gen_Request("How is the "+$_t_LocationsName+" divided?"; "Bins")
					//```
					If ($_t_SublevelLabel#"") & (OK=1)
						$_t_Singular:=GenWordSingle($_t_SublevelLabel)
						Case of   // These default names are just so we can populate something into the request box that looks sensible
						...
						If ($_t_SubLocationCode#"") & (OK=1)
							...
							If (Num:C11($_t_CountSubLevelsSTR)>0)
								...
								ARRAY TEXT:C222($_ptr_NextLevelp1->; $_l_LocationElements)
								ARRAY LONGINT:C221($_ptr_NextLevelp2->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevelp3->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevelp4->; $_l_LocationElements)
								ARRAY LONGINT:C221($_ptr_NextLevelp5->; $_l_LocationElements)
								ARRAY LONGINT:C221($_ptr_NextLevelp6->; $_l_LocationElements)
								ARRAY BOOLEAN:C223($_ptr_NextLevelp7->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevelp8->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevelp9->; $_l_LocationElements)
								ARRAY TEXT:C222($_ptr_NextLevelp10->; $_l_LocationElements)
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oButtonAddFurtherSub9"; $_t_oldMethodName)
