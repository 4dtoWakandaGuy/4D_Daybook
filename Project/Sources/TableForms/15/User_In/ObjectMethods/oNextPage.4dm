If (False:C215)
	//object Method Name: Object Name:      [USER].User_In.Variable1
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].User_In.Variable1"; Form event code:C388)
If (False:C215)  //OLD SCRIPT
	If (DB_at_USERTAB<Size of array:C274(DB_at_USERTAB))
...
If ($_l_SelectedListItem#0)  // Just in case, check the position
	If ($_l_SelectedListItem<Count list items:C380(USR_hl_DataManList))
...
End if 
ERR_MethodTrackerReturn("OBJ [USER].User_In.Variable1"; $_t_oldMethodName)
