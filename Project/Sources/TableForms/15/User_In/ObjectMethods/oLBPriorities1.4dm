If (False:C215)
	//object Name: [USER]User_In.oLBPriorities1
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CatalogueDefault; $_l_CurrentSortColumn; $_l_event; $_l_FieldNumber; $_l_Index; $_l_SelectedRow; $_l_TableNumber; b_ModPriority)
	C_TEXT:C284($_t_oldMethodName; $_t_VariableName; SD_t_PriorityName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].User_In.oLBPriorities1"; Form event code:C388)
...
Case of 
	: ($_l_event=On Clicked:K2:4)
		...
		$_l_CurrentSortColumn:=LB_GetColumn(->SD_lb_Priorities; $_t_VariableName)
		Case of 
			: ($_l_CurrentSortColumn=1)
				If ($_l_SelectedRow>0)
					$_l_CatalogueDefault:=SD_aI_PriorityDF{$_l_SelectedRow}
...
End case 
ERR_MethodTrackerReturn("OBJ [USER].User_In.oLBPriorities1"; $_t_oldMethodName)
