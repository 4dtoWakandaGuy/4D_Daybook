If (False:C215)
	//object Name: [USER]User_In.Variable10
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 28/11/2010 12:47
...
If (True:C214)
	...
	C_LONGINT:C283(<>DB_at_TableNamesMacro; $_l_CurrentWinRefOLD; $_l_Index; $_l_OwnerClass; $_l_SystemClass; APR_SPOrderAmount0; APR_SPOrderAmount1; APR_SPOrderAmount2; APR_SPOrderAmount3; SCRIPT_l_SetScriptClass; WIN_l_CurrentWinRef)
	C_LONGINT:C283(WIN_l_CurrentWinRefOLD)
	C_TEXT:C284($_t_CloseSave; $_t_oldMethodName; DB_t_CallOnCloseorSave; DB_t_WindowTitle; SP_t_OrderCostScript)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].User_In.Variable10"; Form event code:C388)
...
If (Gen_bo_Create)
	<>DB_at_TableNamesMacro:=Table:C252(->[ORDERS:24])
	...
	Minor_In($_boj_InputData)  //->[SCRIPTS]; "Macros_In"; "Macros")
	DB_t_CallOnCloseorSave:=$_t_CloseSave
	SP_t_OrderCostScript:=[SCRIPTS:80]Script_Code:1
...
Else 
	If (GEN_at_DropDownMenu>0)
		SP_t_OrderCostScript:=SP_at_ScriptCode{GEN_at_DropDownMenu}
		APR_SPOrderAmount3:=1
		APR_SPOrderAmount2:=0
		APR_SPOrderAmount1:=0
		APR_SPOrderAmount0:=0
