If (False:C215)
	//object Method Name: Object Name:      [USER].User_In.oResultselect
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 25/10/2012 10:27
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].User_In.oResultselect"; Form event code:C388)
...
Case of 
	: ($_l_event=On Clicked:K2:4) | ($_l_event=On Double Clicked:K2:5)
		...
		$_l_Row:=Self:C308->
		If ($_l_Row>0)
			$_l_Column:=LB_GetColumn(->SD2_abo_LBres; $_t_ColumnName)
			Case of 
				: ($_l_Column=1)
		...
		End if 
End case 
ERR_MethodTrackerReturn("OBJ [USER].User_In.oResultselect"; $_t_oldMethodName)
