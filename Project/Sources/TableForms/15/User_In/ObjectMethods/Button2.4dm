If (False:C215)
	//object Name: [USER]User_In.Button2
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 28/11/2010 13:00
...
If (True:C214)
	...
	C_LONGINT:C283(<>DB_at_TableNamesMacro; $_l_CurrentWinRefOLD; $_l_Index; $_l_OwnerClass; $_l_SystemClass; CPR_SPORDERAMOUNT0; CPR_SPORDERAMOUNT1; CPR_SPORDERAMOUNT2; SCRIPT_l_SetScriptClass; WIN_l_CurrentWinRef; WIN_l_CurrentWinRefOLD)
	C_TEXT:C284($_t_CloseSave; $_t_oldMethodName; DB_t_CallOnCloseorSave; DB_t_WindowTitle; SP_t_jobCostMacro)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].User_In.Button2"; Form event code:C388)
...
If (Gen_bo_Create)
	<>DB_at_TableNamesMacro:=Table:C252(->[JOBS:26])
	...
	SP_t_jobCostMacro:=[SCRIPTS:80]Script_Code:1
	
	CPR_SPOrderAmount2:=1
	CPR_SPOrderAmount1:=0
	CPR_SPOrderAmount0:=0
	UNLOAD RECORD:C212([SCRIPTS:80])
Else 
	If (GEN_at_DropDownMenu>0)
		SP_t_jobCostMacro:=SP_at_ScriptCode{GEN_at_DropDownMenu}
	...
	Else 
		...
		CPR_SPOrderAmount0:=1
		CPR_SPOrderAmount2:=0
		CPR_SPOrderAmount1:=0
	End if 
End if 
