If (False:C215)
	//object Method Name: Object Name:      [USER].SD2_ScheduleWindow.oActions
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 26/01/2013 17:11
...
If (True:C214)
	...
	C_LONGINT:C283($_l_event; $_l_FieldNumber; $_l_Index; $_l_OwnerRow; $_l_Retries; $_l_SelectedRow; $_l_SelectionCount; $_l_TableNumber; LB_l_ColumnNumber; SD2_l_NewWindowsForDiaries)
	C_TEXT:C284(<>PER_t_CurrentUserInitials; $_t_oldMethodName; $_t_SDProcName; $_t_VariableName; SD_t_CurrentDiaryInitials)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].SD2_ScheduleWindow.oActions"; Form event code:C388)
			...
			: (LB_l_ColumnNumber=2)  //Double clicked on column 2
				If ($_l_SelectedRow>0)
					If (SD_at_PersonInitials{$_l_SelectedRow}#"")  //Double clicked on a name
						...
						$_t_SDProcName:="Schedule"+"-"+SD_at_DiaryOwners{$_l_SelectedRow}
						ZProc("Diary_Mod_SD"; DB_ProcessMemoryinit(6); $_t_SDProcName; SD_at_PersonInitials{$_l_SelectedRow})
					End if 
				End if 
		End case 
			...
			: (LB_l_ColumnNumber=1)
				If ($_l_SelectedRow>0)
					If (SD_at_PersonInitials{$_l_SelectedRow}#"")
						$_l_OwnerRow:=Find in array:C230(SD_at_DiaryOwners; "Multiple Diaries")
						If (SD2_abo_CheckBox{$_l_OwnerRow})  //Combine mulitple diaries
							$_l_Retries:=0
...
End case 
ERR_MethodTrackerReturn("OBJ [USER].SD2_ScheduleWindow.oActions"; $_t_oldMethodName)
