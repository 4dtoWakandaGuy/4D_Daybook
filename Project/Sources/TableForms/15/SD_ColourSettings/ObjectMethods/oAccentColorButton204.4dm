If (False:C215)
	//object Name: [USER]SD_ColourSettings.Variable572
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 18/08/2009 13:28
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].SD_ColourSettings.Variable572"; Form event code:C388)
			...
			If ($_l_Number>0)
				...
				If ($_l_Red=0) & ($_l_Green=0) & ($_l_Blue=0)
					...
					Else 
						$_l_Number:=$_l_Number-8
					End if 
					// 4DCOLOR TO RGB (($_l_Number-1);$_l_Red;$_l_Green;$_l_Blue)
...
End case 
ERR_MethodTrackerReturn("OBJ [USER].SD_ColourSettings.Variable572"; $_t_oldMethodName)
