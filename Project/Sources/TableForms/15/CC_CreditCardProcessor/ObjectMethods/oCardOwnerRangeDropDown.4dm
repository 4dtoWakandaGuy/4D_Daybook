If (False:C215)
	//object Name: [USER]CC_CreditCardProcessor.Variable2
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].CC_CreditCardProcessor.Variable2"; Form event code:C388)
If (CC_at_CardOwnerRange>0)
	If (CC_at_CardOwnerRange=1)
		...
		CC_AS79_CarOwnerRange:=0
		CCM_at_CardNumbers:=0
		CCM_at_CardTypes:=0
		CCM_at_CardIssuers:=0
		CCM_at_CardExpiries:=0
	Else 
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [USER].CC_CreditCardProcessor.Variable2"; $_t_oldMethodName)
