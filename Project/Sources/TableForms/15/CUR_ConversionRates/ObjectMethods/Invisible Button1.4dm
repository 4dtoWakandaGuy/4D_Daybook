If (False:C215)
	//object Name: [USER]CUR_ConversionRates.Invisible Button1
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 31/10/2012 07:54
...
If (True:C214)
	...
	C_LONGINT:C283($_l_CurrencyCodeRow; $_l_CurrentWinRefOLD; CUR_l_CurrencyPeriod; MOD_l_CurrentModuleAccess; vProcaMod; WIN_l_CurrentWinRef; WIN_l_CurrentWinRefOLD)
	C_REAL:C285($_r_NewRate)
	C_TEXT:C284($_t_oldMethodName; DB_t_WindowTitle)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].CUR_ConversionRates.Invisible Button1"; Form event code:C388)
If ([CURRENCIES:71]X_ID:3#0)
	...
	If (MOD_l_CurrentModuleAccess#3)
		...
		COPY ARRAY:C226(CUR_at_CurrencyNames; GEN_at_DropDownMenu)
		COPY ARRAY:C226(CUR_al_CurrencyIDs; GEN_al_DropDownMenuID)
		...
		If (GEN_at_DropDownMenu>0)
			...
			$_l_CurrencyCodeRow:=Find in array:C230(CUR_at_CurrencyToCodes; CUR_at_CurrencyCodes{GEN_at_DropDownMenu})
			If ($_l_CurrencyCodeRow>0)
				//change this to allow/dissalow multiple rates!??:)
				$_bo_MultiRate:=True:C214
				If ($_bo_MultiRate)
					Gen_Confirm("Are sure you wish to enter another rate for "+CUR_at_CurrencyNames{GEN_at_DropDownMenu}+" for this period."; "No"; "Yes")
					If (OK=0)
						// a second rate is wanted
						$_r_NewRate:=Num:C11(Request:C163("Enter New Rate"; String:C10(CUR_ar_CurrencyRates{$_l_CurrencyCodeRow})))
						If ($_r_NewRate>0) & (OK=1)
							APPEND TO ARRAY:C911(CUR_at_CurrencyToCodes; CUR_at_CurrencyCodes{GEN_at_DropDownMenu})
							APPEND TO ARRAY:C911(CUR_at_CurrencyToNames; CUR_at_CurrencyNames{GEN_at_DropDownMenu})
							...
							APPEND TO ARRAY:C911(CUR_ar_CurrencyRates; $_r_NewRate)
			...
			Else 
				...
				$_r_NewRate:=Num:C11(Request:C163("Enter New Rate"; "0.00"))
				If ($_r_NewRate>0) & (OK=1)
					APPEND TO ARRAY:C911(CUR_at_CurrencyToCodes; CUR_at_CurrencyCodes{GEN_at_DropDownMenu})
					APPEND TO ARRAY:C911(CUR_at_CurrencyToNames; CUR_at_CurrencyNames{GEN_at_DropDownMenu})
					APPEND TO ARRAY:C911(CUR_abo_CurrencyModified; True:C214)
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ:CUR_ConversionRates,bPlus"; $_t_oldMethodName)
