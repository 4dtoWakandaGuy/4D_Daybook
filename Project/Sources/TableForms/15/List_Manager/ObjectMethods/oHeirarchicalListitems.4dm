If (False:C215)
	//object Method Name: Object Name:      [USER].List_Manager.oHeirarchicalListitems
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_AccessNumber; $_l_AddtoItemRef; $_l_BeforeItemRef; $_l_CountListItems; $_l_DropPosition; $_l_event; $_l_FieldNumber; $_l_ItemReference; $_l_ItemReferenceOLD; $_l_ItemRow; $_l_ListItemRow)
	...
	C_TEXT:C284($_t_ItemText; $_t_itemTextNEW; $_t_itemTextOLD; $_t_oldMethodName; $_t_Text; $_t_VariableName; $beforeitemText; AA_t_CurrentListItem)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].List_Manager.oHeirarchicalListitems"; Form event code:C388)
...
Case of 
			...
			If (AA_bo_Drop)
				...
				If ($_l_DropPosition>0) & (AA_l_SourceElement>0)
					...
					If ($_l_SelectedItemParentOLD#0)  //make sure we are not adding a parent item
						GET LIST ITEM:C378(AA_HL_ListItems; $_l_DropPosition; $_l_ItemReferenceOLD; $_t_itemTextNEW; $_l_SubListID)
						$_l_SelectedItemParentNEW:=List item parent:C633(AA_HL_ListItems; $_l_ItemReferenceOLD)
						If ($_l_SelectedItemParentOLD#$_l_SelectedItemParentNEW)
							If ($_l_SelectedItemParentNEW=0)
								...
								If ($_l_SubListID=0)
									...
									//$listAccess:="Access:"+String($_l_AccessNumber)+Char(13)
									AA_SetListProperty("ACCESS"; $_l_AccessNumber)
									//TEXT TO BLOB($listAccess;[x_Lists]x_ListAttributes;3)
									DB_SaveRecord(->[X_LISTS:111])
									AA_CheckFileUnlocked(->[X_LISTS:111]x_ID:1)
									//AA_l_CurrentSelectedItemID:=[x_Lists]x_ID
									$_l_ItemRow:=Find in array:C230(AA_al_ListItemIDs; $_l_AddtoItemRef)
									If ($_l_ItemRow>0)
										AA_al_SublistCRID{$_l_ItemRow}:=[X_LISTS:111]x_ID:1
										AA_al_SubListCRHLID{$_l_ItemRow}:=$_l_SubListID
									Else 
										//                 ALERT("ERROR")
										Gen_Alert("ERROR")
							...
							Else 
								GET LIST ITEM:C378(AA_HL_ListItems; $_l_DropPosition+1; $_l_BeforeItemRef; $beforeitemText)
	...
	: ($_l_event=On Data Change:K2:15)
		...
		AA_l_CurrentSelectedItemID:=$_l_ItemReference
End case 
ERR_MethodTrackerReturn("OBJ [USER].List_Manager.oHeirarchicalListitems"; $_t_oldMethodName)
