If (False:C215)
	//object Name: [USER]QV_TYPETEXT.Button3
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_Index; $_l_ListID; $_l_Restriction; $_l_RestrictionID; $_l_RestrictionListID; $_l_RestrictionRow; $_l_RestrictionTableRow; $_l_SublistID; $_l_SublistIndex; $_l_SubListNUM; $_l_SublistRow)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].QV_TYPETEXT.Button3"; Form event code:C388)
If (QV_bo_AddAllowed)
			...
			: (QV_l_CurrentMode=2)
				...
				If (QV_T_MultiADD#"")
					...
					If (QV_l_DisplayedOwnerTableID>0)  //note :difference in this line compared to query window
						If (Is table number valid:C999(QV_l_DisplayedOwnerTableID))
							...
							If ($_l_RestrictionTableRow<0)
								...
								AA_LoadListbyID($_l_RestrictionListID; ->QV_at_TableRestrictionNMs; ->QV_al_TableRestrictionIDs)
							Else 
								$_l_RestrictionID:=QV_al_TableRestrictionIDs{$_l_RestrictionTableRow}
							End if 
						End if 
					End if 
					...
					For ($_l_Index; 1; Size of array:C274(QV_at_QualityGroupsNM))
						...
						$_l_RestrictionRow:=Find in array:C230(QV_at_QualityProperties{$_l_Index}; "Table Restriction")
						$_l_Restriction:=0
						If ($_l_RestrictionRow>0)
							$_l_Restriction:=QV_al_QualityPropertiesNUM{$_l_Index}{$_l_RestrictionRow}
						Else 
...
End if 
CANCEL:C270
ERR_MethodTrackerReturn("OBJ [USER].QV_TYPETEXT.Button3"; $_t_oldMethodName)
