If (False:C215)
	//object Name: [USER]SD2_diaryViewer.oSD2Workflow
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_Column; $_l_CurrentSortColumn; $_l_CurrentWinRefOLD; $_l_event; $_l_FieldNumber; $_l_KeyPosition; $_l_Row; $_l_SelectedColumn; $_l_TableNumber; LB_SetMode; SD2_l_InfoColumn)
	...
	C_TEXT:C284($_t_oldMethodName; $_t_VariableName; SD2_t_MiniDiarycode)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].SD2_diaryViewer.oSD2Workflow"; Form event code:C388)
...
Case of 
	: ($_l_event=On Header Click:K2:40)
		...
		LBi_AreaScript(->SD2_aptr_ListControl; 1; [DIARY:12]Diary_Code:3; ->SD2_at_ConfigureSD2; $_l_Row; $_t_VariableName; "SD2_LBI_Items"; $_l_CurrentSortColumn)
		//Actually we have to re-sort the data
	...
	Else 
		...
		//$_l_SelectedColumn:=LB_GetColumn (->SD_lb_Workflow;$_t_VariableName)
		$_l_Row:=Self:C308->
		...
		End if 
		Case of 
			: ($_l_SelectedColumn=SD2_l_InfoColumn)
				$_l_Row:=Self:C308->
				If ($_l_Row>0)
			...
			Else 
				LBi_AreaScript(->SD2_aptr_ListControl; 1; [DIARY:12]Diary_Code:3; ->SD2_at_ConfigureSD2; $_l_Row; $_t_VariableName; "SD2_LBI_Items")
		End case 
End case 
ERR_MethodTrackerReturn("OBJ:SD2_diaryViewer,SD_lb_Workflow"; $_t_oldMethodName)
