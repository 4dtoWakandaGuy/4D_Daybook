If (False:C215)
	//object Name: [USER]ORDERITEMLABELADMIN.oLBLabelAdmin
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 31/10/2012 06:18
...
If (True:C214)
	...
	C_TEXT:C284($_t_AmountSTR; $_t_oldMethodName; $_t_VariableName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [USER].ORDERITEMLABELADMIN.oLBLabelAdmin"; Form event code:C388)
...
Case of 
	...
	: ($_l_event=On Clicked:K2:4)
		...
		Case of 
			...
			: ($_l_ColumnNumber=3)
				If (LAB_ar_QuantityToInclude{$_l_RowNumber}>0)
					$_t_AmountSTR:=Request:C163("Qty to Print"; String:C10(LAB_ar_QuantityToInclude{$_l_RowNumber}))
					$_l_OK:=OK
				Else 
					$_t_AmountSTR:=Request:C163("Qty to Print"; String:C10(LAB_ar_QuantityRemaining{$_l_RowNumber}))
					$_l_OK:=OK
				End if 
				If ($_l_OK=1)
					If ((Num:C11($_t_AmountSTR))>LAB_ar_QuantityRemaining{$_l_RowNumber})
						LAB_ar_QuantityToInclude{$_l_RowNumber}:=LAB_ar_QuantityRemaining{$_l_RowNumber}
					Else 
						LAB_ar_QuantityToInclude{$_l_RowNumber}:=Num:C11($_t_AmountSTR)
					End if 
...
End case 
ERR_MethodTrackerReturn("OBJ [USER].ORDERITEMLABELADMIN.oLBLabelAdmin"; $_t_oldMethodName)
