If (False:C215)
	//Table Form Method Name: [TAX_CODES]Vat_out
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("FM [TAX_CODES].Vat_out"; Form event code:C388)
...
Case of 
	: ($_l_event=On Header:K2:17)
		If (DEF_l_SettingsCalled=0)
			...
			End if 
			INT_Setoutput(Table:C252(->[TAX_CODES:35]); WIN_t_CurrentOutputform)
	...
	: ($_l_event=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		OpenHelp(Table:C252(->[TAX_CODES:35]); "Vat_out")
		Cache_Initialise
		FSetting_SetLabel(WIN_t_CurrentOutputform)
	: ($_l_event=On Activate:K2:9)  // | (vFromIn))
		...
		vButtDisO:="O              DT"
		Out_Buttons
		vTitleR:=Uppercase:C13(Term_VATWT("VAT Codes"))
	: ($_l_event=On Outside Call:K2:11)
		Out_ButtCall(->[TAX_CODES:35]; ->[TAX_CODES:35]Tax_Code:1; ->[TAX_CODES:35]Tax_Name:2; "VAT"; "DB_MenuNewRecord"; "Minor_Sel"; "Minor_Del"; ""; "VAT Codes"; ""; 0; 0)
	...
	: ($_l_event=On Close Detail:K2:24)
		$_t_FormReference:=Table name:C256(->[TAX_CODES:35])+"_"+WIN_t_CurrentInputForm  //added NG may 2004
		...
		vTitleR:=Uppercase:C13(Term_VATWT("VAT Codes"))
End case 
ERR_MethodTrackerReturn("FM:VAT Out"; $_t_oldMethodName)
