If (False:C215)
	//object Name: [PURCHASE_INVOICES]dPurch Sel.oDropDownContextualPeriods
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [PURCHASE_INVOICES].dPurch Sel.oDropDownContextualPeriods"; Form event code:C388)
...
Case of 
	: ($_l_event=On Clicked:K2:4)
		...
		If (ACC_at_ContextualPeriodF>0)
			...
			If (Records in selection:C76([PERIODS:33])>0)
				...
				If (($_l_offset+1)<Size of array:C274($_ad_FromDates))
					ACC_t_PeriodFrom:=$_at_PeriodCodes{$_l_offset}
				Else 
		...
		End if 
End case 
ERR_MethodTrackerReturn("OBJ [PURCHASE_INVOICES].dPurch Sel.oDropDownContextualPeriods"; $_t_oldMethodName)
