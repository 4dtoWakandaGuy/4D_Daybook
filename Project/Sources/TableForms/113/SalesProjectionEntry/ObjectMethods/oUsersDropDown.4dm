If (False:C215)
	//object Name: [SALES_PROJECTION]SalesProjectionEntry.Variable3
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_ItemReference; $_l_SelectedItemParent; $_l_SelectedListItem; $_l_SelectedListItem2; $_l_SublistID; SP_l_UsersnGroups)
	C_TEXT:C284($_t_ItemText; $_t_oldMethodName; SP_t_SalesProjectionPerson)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [SALES_PROJECTION].SalesProjectionEntry.Variable3"; Form event code:C388)
...
If ($_l_SelectedListItem#0)  // Just in case, check the position
	...
	End if 
End if 
ERR_MethodTrackerReturn("OBJ [SALES_PROJECTION].SalesProjectionEntry.Variable3"; $_t_oldMethodName)
