If (False:C215)
	//object Name: [ORDER_ITEMS]BUILD_PRODUCT.oQuantitytoCreate1
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [ORDER_ITEMS].BUILD_PRODUCT.oQuantitytoCreate1"; Form event code:C388)
	...
	: ($_l_event=On Data Change:K2:15)
		...
		Case of 
			: ((PRD_R_QUantity-PRD_l_ExtraDates)=1)  //has to be consecutive
				...
				End if 
				$_l_CountInserts:=0
				For ($_l_CurrentRow; $_l_Start; ($_l_Start+(PRD_R_Quantity-PRD_l_ExtraDates)); 1)
					If (Size of array:C274(PRD_abo_IssueInsert)>=$_l_CurrentRow)
						...
						If ($_l_CurrentRow=$_l_Start)
							PRD_d_FIrstDate:=PRD_ad_IssueDate{$_l_CurrentRow}
							If ($_l_Start=($_l_Start+((PRD_R_Quantity-PRD_l_ExtraDates)-1)))
		...
		End case 
End case 
ERR_MethodTrackerReturn("OBJ [ORDER_ITEMS].BUILD_PRODUCT.oQuantitytoCreate1"; $_t_oldMethodName)
