If (False:C215)
	//object Name: [ORDER_ITEMS]Items_in.Variable1
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 22/06/2009 10:46
	//Created BY: Nigel Greenlee
	//Date Modified: 
	//Modified By DefaultUser
	//Modification notes 
	
End if 
//Variable Declarations
If (True:C214)
	//ARRAY TEXT(OI_at_OrderItemTabs;0)
	C_TEXT:C284($_t_oldMethodName)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [ORDER_ITEMS].Items_in.Variable1"; Form event code:C388)
If (OI_at_OrderItemTabs<Size of array:C274(OI_at_OrderItemTabs))
	OI_at_OrderItemTabs:=OI_at_OrderItemTabs+1
End if 

If (OI_at_OrderItemTabs#FORM Get current page:C276)
	Case of 
		: (OI_at_OrderItemTabs=2)
			
			FORM GOTO PAGE:C247(2)
			GOTO OBJECT:C206([ORDER_ITEMS:25]Required_Date:10)
		: (OI_at_OrderItemTabs=1)
			FORM GOTO PAGE:C247(1)
			GOTO OBJECT:C206([ORDER_ITEMS:25]Product_Name:13)
			HIGHLIGHT TEXT:C210([ORDER_ITEMS:25]Product_Name:13; 32000; 32000)
			
	End case 
	
End if 
ERR_MethodTrackerReturn("OBJ [ORDER_ITEMS].Items_in.Variable1"; $_t_oldMethodName)
