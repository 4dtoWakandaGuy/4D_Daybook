If (False:C215)
	//object Name: [CONTRACTS]dContr_Sel.Variable56
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_DATE:C307(SRCH_d_EntryDateTo; vDateF5; vDateT5; VsDateF; VsDateT)
	C_LONGINT:C283(<>PER_l_CurrentUserID; $_l_ContractsTable; $_l_event; cNAA; cNAL; cSIS; SM_l_QueryMode; SM_l_ReturnQueryID; vAct5; vNo5)
	C_TEXT:C284($_t_ContextName; $_t_oldMethodName; $_t_SearchName; vCompCode; vContCode; vContrCode; vDesc5; vJob; VorderCode; VprodCode; VType)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [CONTRACTS].dContr_Sel.Variable56"; Form event code:C388)
$_l_event:=Form event code:C388
$_l_ContractsTable:=Table:C252(->[CONTRACTS:17])
$_t_ContextName:="Contracts"
Case of 
	: ($_l_event=On Clicked:K2:4)
			...
			Case of 
				: (SM_al_SearchIDs{SM_at_SearchNames}>0)
					...
					If (SM_l_QueryMode#0)
						...
						Gen_Set_enterable($_bo_Enterable; ->cSIS; ->cNAA; ->cNAL; ->vCompCode; ->vContCode; ->vContrCode; ->VType; ->VsDateF; ->VsDateT; ->SRCH_d_EntryDateTo; ->vJob; ->VorderCode; ->VprodCode)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}=-99998)  //Set user default
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							Sm_SetUserSearchPrefs(True:C214; $_l_ContractsTable; <>PER_l_CurrentUserID; Abs:C99(SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}))
					...
					Else 
						...
						If (ok=0)
							Sm_SetUserSearchPrefs(True:C214; $_l_ContractsTable; <>PER_l_CurrentUserID; 0)
					...
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							...
							Else 
								...
								SM_LoadSearchParameterNames($_l_ContractsTable)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}<0)  //Resave
					...
					SM_SaveSearchParameters(Abs:C99(SM_al_SearchIDs{SM_at_SearchNames}); ""; $_l_ContractsTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
				...
				Else 
					...
					If ($_t_SearchName#"")
						...
						SM_SaveSearchParameters(0; $_t_SearchName; $_l_ContractsTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
						SM_LoadSearchParameterNames($_l_ContractsTable)
		...
		End if 
End case 
ERR_MethodTrackerReturn("OBJ [CONTRACTS].dContr_Sel.Variable56"; $_t_oldMethodName)
