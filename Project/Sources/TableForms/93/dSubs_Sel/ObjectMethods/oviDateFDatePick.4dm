If (False:C215)
	//object Name: [SUBSCRIPTIONS]dSubs Sel.oviDateFDatePick
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	ARRAY DATE:C224($_ad_DatesOff; 0)
	C_BOOLEAN:C305($_bo_Customise; DB_bo_NoLoad; SRCH_bo_VeDateToSet)
	C_DATE:C307(SRCH_d_EntryDateFrom; SRCH_d_EntryDateTo; vDate)
	C_TEXT:C284($_t_oldMethodName; $_t_FormName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [SUBSCRIPTIONS].dSubs Sel.oviDateFDatePick"; Form event code:C388)
...
Case of 
	...
	: (Form event code:C388=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		If ($_bo_Customise)
			ARRAY DATE:C224($_ad_DatesOff; 0)
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 1; 1))  //january 1rst
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 5; 1))
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 5; 8))
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 7; 14))
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 8; 15))
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 11; 1))
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 11; 11))
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2000; 12; 25))  // december 25th
			
			DatePicker SET DAYS OFF($_t_FormName; 1; ->$_ad_DatesOff)  // 1 stands for every year
		End if 
		...
		If ($_bo_Customise)
			...
			ARRAY DATE:C224($_ad_DatesOff; 0)
			APPEND TO ARRAY:C911($_ad_DatesOff; Add to date:C393(!00-00-00!; 2009; 6; 27))
			
			DatePicker SET DAYS OFF($_t_FormName; 2; ->$_ad_DatesOff)  // 2 stands for "only in 2009"
		End if 
...
End case 
ERR_MethodTrackerReturn("OBJ [SUBSCRIPTIONS].dSubs Sel.oviDateFDatePick"; $_t_oldMethodName)
