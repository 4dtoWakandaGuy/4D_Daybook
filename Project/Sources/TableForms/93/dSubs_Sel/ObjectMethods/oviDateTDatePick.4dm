If (False:C215)
	//object Name: [SUBSCRIPTIONS]dSubs Sel.oviDateTDatePick
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 17/08/2012 13:28
...
If (True:C214)
	ARRAY BOOLEAN:C223($_aBO_WeekDaysOffset; 0)
	ARRAY DATE:C224($_ad_DatesOffset; 0)
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [SUBSCRIPTIONS].dSubs Sel.oviDateTDatePick"; Form event code:C388)
...
Case of 
	...
	: (Form event code:C388=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		If ($_bo_Customise)
			ARRAY BOOLEAN:C223($_aBO_WeekDaysOffset; 7)  //lets say wednesday and sunday are days off
			$_aBO_WeekDaysOffset{Wednesday:K10:15}:=True:C214
			$_aBO_WeekDaysOffset{Sunday:K10:19}:=True:C214
			DatePicker SET DAYS OFF($_t_FormName; 0; ->$_aBO_WeekDaysOffset)  //0 means weekly days off
		End if 
		...
		If ($_bo_Customise)
			ARRAY DATE:C224($_ad_DatesOffset; 0)
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 1; 1))  //january 1rst
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 5; 1))
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 5; 8))
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 7; 14))
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 8; 15))
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 11; 1))
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 11; 11))
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2000; 12; 25))  // december 25th
			
			DatePicker SET DAYS OFF($_t_FormName; 1; ->$_ad_DatesOffset)  // 1 stands for every year
		End if 
		...
		If ($_bo_Customise)
			...
			ARRAY DATE:C224($_ad_DatesOffset; 0)
			APPEND TO ARRAY:C911($_ad_DatesOffset; Add to date:C393(!00-00-00!; 2009; 6; 27))
			
			DatePicker SET DAYS OFF($_t_FormName; 2; ->$_ad_DatesOffset)  // 2 stands for "only in 2009"
		End if 
...
End case 
ERR_MethodTrackerReturn("OBJ [SUBSCRIPTIONS].dSubs Sel.oviDateTDatePick"; $_t_oldMethodName)
