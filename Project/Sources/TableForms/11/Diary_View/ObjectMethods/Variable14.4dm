If (False:C215)
	//object Name: [PERSONNEL]Diary_View.Variable14
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_TEXT:C284($_t_oldMethodName; MSG_t_Message; MSG_t_MessageFrom; Phone; SD_at_TimeFrame; SD_t_TimeSetting)
	C_TIME:C306(SD_ti_SettingsTimeEnd; SD_ti_SettingsTimeStart)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [PERSONNEL].Diary_View.Variable14"; Form event code:C388)
Case of 
	: (SD_at_AddAction=1)
		//Add event
		
		SD_AutoAddRec("EVENT"; SD_D_CurrentviewDate; ?00:00:00?)
	...
	: (SD_at_AddAction=3)
		//task
		SD_AutoAddRec("TASK"; SD_D_CurrentviewDate; ?00:00:00?)
	...
	: (SD_at_AddAction=7)
		...
		Else 
			...
			Phone:=Request:C163("Mobile number to send to:")
			If (phone#"") & (OK=1)
				MSG_t_Message:=""
				MSG_bo_NewMessage:=True:C214  // 19/08/03 pb
				SMS_SendOneMessage(->phone)
			End if 
		End if 
		
End case 
SD_at_AddAction:=0
ERR_MethodTrackerReturn("OBJ:Diary_View,SD_at_AddAction"; $_t_oldMethodName)
