If (False:C215)
	//object Method Name: Object Name:      [PERSONNEL].Diary_View.Check Box3
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_event; $_l_Index; $_l_QueryRow; $_l_RecordRow; SD_l_IncQueryCo)
	C_TEXT:C284($_t_CompanyName; $_t_oldMethodName; $_t_PersonText)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [PERSONNEL].Diary_View.Check Box3"; Form event code:C388)
...
Case of 
	: ($_l_event=On Clicked:K2:4)
		...
		If (SD_at_Query>0)  //only if something is loaded
			...
			End if 
		End if 
		For ($_l_Index; 1; Size of array:C274(SD_at_Query))
			$_l_RecordRow:=Find in array:C230(SD_al_RecordNumbers; SD_al_AppointmentRecordIDs{$_l_Index})
			...
			If ($_l_RecordRow>0)
				...
				If ((SD_abo_DoneStatus{$_l_RecordRow}) & (SD_at_ActionDetails2{$_l_RecordRow}#"")) | (SD_at_ActionDetails{$_l_RecordRow}="")
					SD_at_Query{$_l_Index}:=String:C10(SD_ad_SortDate{$_l_RecordRow}; 4)+" :"+$_t_CompanyName+" "+$_t_PersonText+" :"+SD_at_ItemActionCode{$_l_RecordRow}+" :"+SD_at_ActionDetails2{$_l_RecordRow}  //create: diary item time and description
				Else 
					SD_at_Query{$_l_Index}:=String:C10(SD_ad_SortDate{$_l_RecordRow}; 4)+" :"+$_t_CompanyName+" "+$_t_PersonText+" :"+SD_at_ItemActionCode{$_l_RecordRow}+" :"+SD_at_ActionDetails{$_l_RecordRow}  //create: diary item time and description
				End if 
		...
		End for 
End case 
ERR_MethodTrackerReturn("OBJ [PERSONNEL].Diary_View.Check Box3"; $_t_oldMethodName)
