If (False:C215)
	//object Name: [PERSONNEL]dAsk_Function.Variable27
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [PERSONNEL].dAsk_Function.Variable27"; Form event code:C388)
...
If (User in group:C338(Current user:C182; "ADMINISTRATOR"))
	...
	If (Ok=1)
		...
		While (Semaphore:C143("LoadingPrefsSkins"))
			...
			DelayTicks($_l_Retries*2)
		End while 
		$_l_ItemID:=PREF_GetPreferenceID("LoadingPrefsSkins")
		If ($_l_ItemID>0)
...
End if 
ERR_MethodTrackerReturn("OBJ:dAsk_Function,Variable8"; $_t_oldMethodName)
