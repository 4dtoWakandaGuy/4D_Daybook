If (False:C215)
	//object Method Name: Object Name:      [PERSONNEL].Personnel_In.oVolumeAccess
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	//ARRAY TEXT(MOD_at_ModuleName;0)
	C_BOOLEAN:C305($_bo_Disabled; $_bo_HasIcon; DB_bo_RecordModified; vMod3)
	C_LONGINT:C283($_l_ControlLevel; $_l_event; $_l_FieldNumber; $_l_ModuleRow; $_l_Row; $_l_TableNumber; PER_l_InitIM)
	C_TEXT:C284($_t_oldMethodName; $_t_VariableName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [PERSONNEL].Personnel_In.oVolumeAccess"; Form event code:C388)
	...
	: ($_l_event=On Clicked:K2:4) | ($_l_event=On Data Change:K2:15)
		...
		Case of 
			: ($_t_VariableName="MOD_abo_AccessBoolean")
				$_bo_Disabled:=False:C215
				If (Not:C34(MOD_abo_AccessBoolean{$_l_Row}))
					//Check if this is enabled at  group level first ..if so then you cant turn it off here
					$_l_ModuleRow:=Find in array:C230(GRP_at_AModName; MOD_at_ModuleName{$_l_Row})
					If ($_l_ModuleRow>0)
						If (MOD_al_GroupModuleAccessStatus{$_l_ModuleRow}=1)
							Gen_Confirm("This module is enabled at group level, you may not turn it off here, do you wish "+"to disable access for this user?"; "Yes"; "No")
							If (OK=1)
								MOD_abo_DisableVolume{$_l_Row}:=True:C214
								$_bo_Disabled:=True:C214
							Else 
				...
				If (MOD_abo_AccessBoolean{$_l_Row}) & (Not:C34($_bo_Disabled))
					MOD_abo_DisableVolume{$_l_Row}:=False:C215
				End if 
				...
				If (MOD_at_ModuleName{$_l_Row}="Internet Manager")
					If (MOD_al_ModuleAccessStatus{$_l_Row}=1)  // it is enabled
						//_O_ENABLE BUTTON(PER_l_InitIM)
						OBJECT SET ENABLED:C1123(PER_l_InitIM; True:C214)
					Else 
						//_O_DISABLE BUTTON(PER_l_InitIM)
						OBJECT SET ENABLED:C1123(PER_l_InitIM; False:C215)
						PER_l_InitIM:=0
						//MOD_al_InitStartup{$Ref}:=0
						//MOD_abo_InitStartup{$Ref}:=False
						MOD_al_InitStartup{$_l_Row}:=0  //kmw 07/10/08 v631b120, fixed to use $_l_Row index instead of $Ref index
						MOD_abo_InitStartup{$_l_Row}:=False:C215  //kmw 07/10/08 v631b120, fixed to use $_l_Row index instead of $Ref index
					End if 
			...
			: ($_t_VariableName="MOD_abo_DisableVolume")
				If (MOD_abo_DisableVolume{$_l_Row})
					If (MOD_al_ModuleAccessStatus{$_l_Row}=1)
						//This module is enabled..is it enabled at group level
						$_l_ModuleRow:=Find in array:C230(GRP_at_AModName; MOD_at_ModuleName{$_l_Row})
						If ($_l_ModuleRow>0)
							If (Not:C34(MOD_al_GroupModuleAccessStatus{$_l_ModuleRow}=1))  //it is NOT enabled at group level
								Gen_Alert("This module is not enabled for the group it can be disabled by unchecking access"; "OK")
								MOD_abo_DisableVolume{$_l_Row}:=False:C215
								MOD_abo_AccessBoolean{$_l_Row}:=False:C215
							End if 
						End if 
					Else 
						Gen_Alert("This module is not enabled anyway so no need to disable it"; "OK")
						MOD_abo_DisableVolume{$_l_Row}:=False:C215
					End if 
		...
		End case 
End case 
ERR_MethodTrackerReturn("OBJ [PERSONNEL].Personnel_In.oVolumeAccess"; $_t_oldMethodName)
