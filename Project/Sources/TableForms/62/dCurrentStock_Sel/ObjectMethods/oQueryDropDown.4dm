If (False:C215)
	//object Name: [CURRENT_STOCK]dCurrentStock_Sel.oQueryDropDown
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>PER_l_CurrentUserID; $_l_CurrentStockTable; $_l_event; cNAA; cNAL; cSIS; SM_l_QueryMode; SM_l_ReturnQueryID; VItemNum)
	C_REAL:C285(VamountF; VamountT; vmarginF; vmarginPF; vmarginPT; vmarginT)
	C_TEXT:C284($_t_ContextName; $_t_oldMethodName; $_t_SearchName; ACC_t_CurrencyCode; Vanalysis; Vlayer; Vlocation; VorderCode; VprodCode; VSerialNo; Vtype)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("OBJ [CURRENT_STOCK].dCurrentStock_Sel.oQueryDropDown"; Form event code:C388)
$_l_event:=Form event code:C388
$_l_CurrentStockTable:=Table:C252(->[CURRENT_STOCK:62])
$_t_ContextName:="Current stock"
...
Case of 
	: ($_l_event=On Clicked:K2:4)
			...
			Case of 
				: (SM_al_SearchIDs{SM_at_SearchNames}>0)
					...
					If (SM_l_QueryMode#0)
						...
						End if 
						Gen_Set_enterable($_bo_Enterable; ->VprodCode; ->VSerialNo; ->Vtype; ->Vlocation; ->Vanalysis; ->ACC_t_CurrencyCode; ->Vlayer; ->VorderCode; ->VItemNum; ->VamountF; ->VamountT; ->vmarginF; ->vmarginT; ->vmarginPF; ->vmarginPT; ->cSIS; ->cNAA; ->cNAL)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}=-99998)  //Set user default
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							Sm_SetUserSearchPrefs(True:C214; $_l_CurrentStockTable; <>PER_l_CurrentUserID; Abs:C99(SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}))
					...
					Else 
						...
						If (ok=0)
							Sm_SetUserSearchPrefs(True:C214; $_l_CurrentStockTable; <>PER_l_CurrentUserID; 0)
					...
					If (SM_al_SearchIDs{Size of array:C274(SM_al_SearchIDs)-1}<0)
						...
						If (ok=1)
							...
							Else 
								...
								SM_LoadSearchParameterNames($_l_CurrentStockTable)
				...
				: (SM_al_SearchIDs{SM_at_SearchNames}<0)  //Resave
					...
					SM_SaveSearchParameters(Abs:C99(SM_al_SearchIDs{SM_at_SearchNames}); ""; $_l_CurrentStockTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
				...
				Else 
					...
					If ($_t_SearchName#"")
						...
						SM_SaveSearchParameters(0; $_t_SearchName; $_l_CurrentStockTable; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
						SM_LoadSearchParameterNames($_l_CurrentStockTable)
		...
		End if 
End case 
ERR_MethodTrackerReturn("OBJ [CURRENT_STOCK].dCurrentStock_Sel.oQueryDropDown"; $_t_oldMethodName)
