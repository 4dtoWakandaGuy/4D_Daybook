If (False:C215)
	//object Method Name: Object Name:      [COMPANIES].dAsk_StartImp.oAddUser4
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_ColumnIndex; $_l_ColumnIndex2; $_l_ColumnNumber; $_l_ColumnsIndex; $_l_FieldColumn; $_l_FieldsIndex; $_l_FontSize; $_l_LastColumn; $_l_LastNumber; $_l_Number; $_l_offset)
	C_LONGINT:C283($_l_RecordNumber; $_l_RecordRow; $_l_RowsIndex; $_l_TableRow; $_l_TablesIndex; LAY_l_ConfigCancel; LAY_l_ConfigSave; LAY_l_CurrentDefsTable)
...
End if 
//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("OBJ [COMPANIES]dAsk_StartImp.oAddUser4"; Form event code:C388)
...
If (LAY_bo_RecordEdited=True:C214)
	...
	$_l_RecordRow:=Find in array:C230(LAY_al_RecordNumbers; $_l_RecordNumber)
	If ($_l_RecordRow<0)
		$_l_RecordRow:=Find in array:C230(LAY_al_RecordNumbers; 0)
	End if 
	LAY_al_ColumnNumber{$_l_RecordRow}:=$_l_ColumnNumber
	LAY_at_ColumnTitle{$_l_RecordRow}:=$_t_ColumnTitle
	LAY_al_RecordNumbers{$_l_RecordRow}:=$_l_RecordNumber
	SORT ARRAY:C229(LAY_al_ColumnNumber; LAY_at_ColumnTitle; LAY_al_RecordNumbers)
	$_l_LastNumber:=0
	For ($_l_RowsIndex; 1; Size of array:C274(LAY_al_ColumnNumber))
		If (LAY_al_RecordNumbers{$_l_RowsIndex}=0)
			LAY_al_RecordNumbers{$_l_RowsIndex}:=1
		End if 
		If (LAY_al_ColumnNumber{$_l_RowsIndex}=$_l_LastNumber)
			//its a clash
			If ($_l_RecordNumber#LAY_al_RecordNumbers{$_l_RowsIndex})
				LAY_al_ColumnNumber{$_l_RowsIndex}:=LAY_al_ColumnNumber{$_l_RowsIndex}+1
				GOTO RECORD:C242([LIST_LAYOUTS:96]; LAY_al_RecordNumbers{$_l_RowsIndex})
				[LIST_LAYOUTS:96]Order:7:=LAY_al_RecordNumbers{$_l_RowsIndex}
				DB_SaveRecord(->[LIST_LAYOUTS:96])
			Else 
				$_l_offset:=0
				For ($_l_ColumnIndex2; $_l_RowsIndex; 1; -1)
					If (LAY_al_ColumnNumber{$_l_ColumnIndex2}=LAY_al_ColumnNumber{$_l_RowsIndex})
						LAY_al_ColumnNumber{$_l_ColumnIndex2}:=LAY_al_ColumnNumber{$_l_RowsIndex}+$_l_offset
						GOTO RECORD:C242([LIST_LAYOUTS:96]; LAY_al_RecordNumbers{$_l_RowsIndex})
						[LIST_LAYOUTS:96]Order:7:=LAY_al_RecordNumbers{$_l_RowsIndex}
						DB_SaveRecord(->[LIST_LAYOUTS:96])
					End if 
			...
			End if 
		End if 
		$_l_LastNumber:=LAY_al_ColumnNumber{$_l_RowsIndex}
	End for 
	SORT ARRAY:C229(LAY_al_ColumnNumber; LAY_at_ColumnTitle; LAY_al_RecordNumbers)
	$_l_ColumnNumber:=0
...
If ([LIST_LAYOUTS:96]Table_Number:2#0)
	...
	LL_t_TableName2:=""
	LL_t_FieldName2:=""
	For ($_l_TablesIndex; 1; Size of array:C274(LL_al_TableIDs))
		If (User in group:C338(Current user:C182; "Designer"))
			APPEND TO ARRAY:C911(LL_at_TableNames2; LL_at_TableNames{$_l_TablesIndex})
			
		Else 
			If (LL_al_TableAccess{$_l_TablesIndex}>0)
				APPEND TO ARRAY:C911(LL_at_TableNames2; LL_at_TableNames{$_l_TablesIndex})
...
If ($_l_TableRow>0)
	...
	For ($_l_FieldsIndex; 1; Size of array:C274(LL_al_FieldNos{$_l_TableRow}))
		...
		Else 
			If (LL_al_FieldAccess{$_l_TableRow}{$_l_FieldsIndex}>0)
				APPEND TO ARRAY:C911(LL_at_FieldName; LL_at_FieldNames{$_l_TableRow}{$_l_FieldsIndex})
				APPEND TO ARRAY:C911(LL_al_FieldNums; LL_al_FieldNos{$_l_TableRow}{$_l_FieldsIndex})
				APPEND TO ARRAY:C911(LL_at_FieldName2; LL_at_FieldNames{$_l_TableRow}{$_l_FieldsIndex})
				APPEND TO ARRAY:C911(LL_al_FieldNums2; LL_al_FieldNos{$_l_TableRow}{$_l_FieldsIndex})
...
End if 
ERR_MethodTrackerReturn("OBJ [COMPANIES].dAsk_StartImp.oAddUser4"; $_t_oldMethodName)
