If (False:C215)
	//object Method Name: Object Name:      [COMPANIES].dAsk_StartImp.oAddUser5
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 09/08/2012 10:59
...
If (True:C214)
	...
	C_BOOLEAN:C305(DB_bo_RecordModified; LAY_bo_AddRecord; LAY_bo_RecordEdited; LAY_bo_ShowLastColumn)
	C_LONGINT:C283($_l_ColumnNumber; $_l_ColumnsIndex; $_l_ColumnsIndex2; $_l_FieldNumberRow; $_l_FontSize; $_l_LastColumn; $_l_Number; $_l_offset; $_l_PreviousRecordNumber; $_l_RecordNumber; $_l_RecordNumberRow)
	C_LONGINT:C283(LAY_l_ConfigCancel; LAY_l_ConfigSave; LAY_l_CurrentDefsTable)
...
End if 
//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("OBJ [COMPANIES]dAsk_StartImp.oAddUser5"; Form event code:C388)
...
If (LAY_bo_RecordEdited=True:C214)
	...
	$_l_PreviousRecordNumber:=0
	For ($_l_ColumnsIndex; 1; Size of array:C274(LAY_al_ColumnNumber))
		If (LAY_al_RecordNumbers{$_l_ColumnsIndex}=0)
			LAY_al_RecordNumbers{$_l_ColumnsIndex}:=1
		End if 
		If (LAY_al_ColumnNumber{$_l_ColumnsIndex}=$_l_PreviousRecordNumber)
			//its a clash
			If ($_l_RecordNumber#LAY_al_RecordNumbers{$_l_ColumnsIndex})
				LAY_al_ColumnNumber{$_l_ColumnsIndex}:=LAY_al_ColumnNumber{$_l_ColumnsIndex}+1
				GOTO RECORD:C242([LIST_LAYOUTS:96]; LAY_al_RecordNumbers{$_l_ColumnsIndex})
				[LIST_LAYOUTS:96]Order:7:=LAY_al_RecordNumbers{$_l_ColumnsIndex}
				DB_SaveRecord(->[LIST_LAYOUTS:96])
			Else 
				$_l_offset:=0
				For ($_l_ColumnsIndex2; $_l_ColumnsIndex; 1; -1)
					If (LAY_al_ColumnNumber{$_l_ColumnsIndex2}=LAY_al_ColumnNumber{$_l_ColumnsIndex})
						LAY_al_ColumnNumber{$_l_ColumnsIndex2}:=LAY_al_ColumnNumber{$_l_ColumnsIndex}+$_l_offset
						GOTO RECORD:C242([LIST_LAYOUTS:96]; LAY_al_RecordNumbers{$_l_ColumnsIndex})
						[LIST_LAYOUTS:96]Order:7:=LAY_al_RecordNumbers{$_l_ColumnsIndex}
						DB_SaveRecord(->[LIST_LAYOUTS:96])
					End if 
			...
			End if 
		End if 
		$_l_PreviousRecordNumber:=LAY_al_ColumnNumber{$_l_ColumnsIndex}
	End for 
	SORT ARRAY:C229(LAY_al_ColumnNumber; LAY_at_ColumnTitle; LAY_al_RecordNumbers)
	$_l_ColumnNumber:=0
	For ($_l_ColumnsIndex; 1; Size of array:C274(LAY_al_ColumnNumber))
		$_l_ColumnNumber:=$_l_ColumnsIndex
		If (LAY_al_ColumnNumber{$_l_ColumnsIndex}#$_l_ColumnNumber)
			GOTO RECORD:C242([LIST_LAYOUTS:96]; LAY_al_RecordNumbers{$_l_ColumnsIndex})
			LAY_al_ColumnNumber{$_l_ColumnsIndex}:=$_l_ColumnNumber
...
If ([LIST_LAYOUTS:96]Table_Number:2#0)
	...
	$_l_RecordNumberRow:=Find in array:C230(LL_al_TableIDs; [LIST_LAYOUTS:96]Table_Number:2)
	ARRAY TEXT:C222(LL_at_TableNames2; 0)
	LL_t_TableName2:=""
	LL_t_FieldName2:=""
	For ($_l_ColumnsIndex; 1; Size of array:C274(LL_al_TableIDs))
		If (User in group:C338(Current user:C182; "Designer"))
			APPEND TO ARRAY:C911(LL_at_TableNames2; LL_at_TableNames{$_l_ColumnsIndex})
			
		Else 
			If (LL_al_TableAccess{$_l_ColumnsIndex}>0)
				APPEND TO ARRAY:C911(LL_at_TableNames2; LL_at_TableNames{$_l_ColumnsIndex})
				
			End if 
		End if 
	End for 
	LL_t_TableName:=LL_at_TableNames{$_l_RecordNumberRow}
Else 
	$_l_RecordNumberRow:=0
	LL_t_TableName:=""
End if 
If ($_l_RecordNumberRow>0)
	...
	For ($_l_ColumnsIndex; 1; Size of array:C274(LL_al_FieldNos{$_l_RecordNumberRow}))
		If (User in group:C338(Current user:C182; "Designer"))
			APPEND TO ARRAY:C911(LL_at_FieldName; LL_at_FieldNames{$_l_RecordNumberRow}{$_l_ColumnsIndex})
			APPEND TO ARRAY:C911(LL_al_FieldNums; LL_al_FieldNos{$_l_RecordNumberRow}{$_l_ColumnsIndex})
			APPEND TO ARRAY:C911(LL_at_FieldName2; LL_at_FieldNames{$_l_RecordNumberRow}{$_l_ColumnsIndex})
			APPEND TO ARRAY:C911(LL_al_FieldNums2; LL_al_FieldNos{$_l_RecordNumberRow}{$_l_ColumnsIndex})
			
		Else 
			If (LL_al_FieldAccess{$_l_RecordNumberRow}{$_l_ColumnsIndex}>0)
				APPEND TO ARRAY:C911(LL_at_FieldName; LL_at_FieldNames{$_l_RecordNumberRow}{$_l_ColumnsIndex})
				APPEND TO ARRAY:C911(LL_al_FieldNums; LL_al_FieldNos{$_l_RecordNumberRow}{$_l_ColumnsIndex})
				APPEND TO ARRAY:C911(LL_at_FieldName2; LL_at_FieldNames{$_l_RecordNumberRow}{$_l_ColumnsIndex})
				APPEND TO ARRAY:C911(LL_al_FieldNums2; LL_al_FieldNos{$_l_RecordNumberRow}{$_l_ColumnsIndex})
	...
	End for 
	$_l_FieldNumberRow:=Find in array:C230(LL_al_FieldNos{$_l_RecordNumberRow}; [LIST_LAYOUTS:96]Field_Number:3)
	If ($_l_FieldNumberRow>0)
		LL_t_FieldName:=LL_at_FieldNames{$_l_RecordNumberRow}{$_l_FieldNumberRow}
	Else 
		LL_t_FieldName:="???"
	End if 
End if 
...
If (Size of array:C274(DB_aptr_Setup)>=20)
	...
	Case of 
		...
		Else 
			...
			//unknown type!
	End case 
	For ($_l_ColumnsIndex; Size of array:C274($_ptr_ArrayPointerArrayNames->); 1; -1)  //this gets the highest used already array number
		$_t_NumberDesignation:=$_ptr_ArrayPointerArrayNames->{$_l_ColumnsIndex}
...
End if 
ERR_MethodTrackerReturn("OBJ [COMPANIES].dAsk_StartImp.oAddUser5"; $_t_oldMethodName)
