If (False:C215)
	//object Name: [COMPANIES]Company_In.Variable41
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_TEXT:C284($_t_oldMethodName; COM_t_Country; COM_t_PostalCode)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [COMPANIES].Company_In.Variable41"; Form event code:C388)
				...
				If ($_l_AddressTypesRow>0)
					...
					Case of 
						...
						: (CO_at_AddressTypes{$_l_AddressTypesRow}="Invoice Address")
							...
							Companies_InLPD
							COM_t_PostalCode:=[COMPANIES:2]Invoice_Postcode:22
						Else 
			...
			: (CUR_l_SelectedAddressTypeRow<0)
				...
				Else 
					...
					If ($_l_AddressRow<0)
						...
						CO_at_PostalCode{Size of array:C274(CO_at_PostalCode)}:=COM_t_PostalCode
...
End case 
ERR_MethodTrackerReturn("OBJ:Company_In,COM_t_PostalCode"; $_t_oldMethodName)
