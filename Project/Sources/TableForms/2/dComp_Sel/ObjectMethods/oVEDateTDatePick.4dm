If (False:C215)
	//object Name: [COMPANIES]dComp_Sel.oVEDateTDatePick
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 17/08/2012 11:44
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_Customize; DB_bo_NoLoad; SRCH_bo_VeDateToSet)
	C_DATE:C307(ACC_d_EntryDateFrom; SRCH_d_EntryDateTo; vDate)
	C_TEXT:C284($_t_ObjectName; $_t_oldMethodName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ [COMPANIES].dComp_Sel.oVEDateTDatePick"; Form event code:C388)
...
Case of 
	...
	: (Form event code:C388=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		If ($_bo_Customize)
			...
			DatePicker SET DAYS OFF($_t_ObjectName; 0; ->$_abo_WeekDaysOff)  //0 means weekly days off
		End if 
		...
		//DatePicker SET MIN DATE ($_t_ObjectName;!01/01/2000!)
		//DatePicker SET MAX DATE ($_t_ObjectName;Current date)
		If ($_bo_Customize)
		...
		If ($_bo_Customize)
			...
			DatePicker SET DAYS OFF($_t_ObjectName; 1; ->$_ad_Daysoff)  // 1 stands for every year
		End if 
		...
		If ($_bo_Customize)
			...
			DatePicker SET DAYS OFF($_t_ObjectName; 2; ->$_ad_Daysoff)  // 2 stands for "only in 2009"
		End if 
		
		
		// The exemple below show how to define the first day of the week (default is monday)
		If ($_bo_Customize)
			DatePicker SET WEEK FIRST DAY($_t_ObjectName; Wednesday:K10:15)  // lets start the week on wednesday (why not)
		End if 
		
End case 
ERR_MethodTrackerReturn("OBJ [COMPANIES].dComp_Sel.oVEDateTDatePick"; $_t_oldMethodName)
