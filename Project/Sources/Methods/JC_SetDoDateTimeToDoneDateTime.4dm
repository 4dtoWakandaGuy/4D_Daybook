//%attributes = {}
If (False:C215)
	//Project Method Name:      JC_SetDoDateTimeToDoneDateTime
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 22/06/2009 10:45
	//Created BY: Nigel Greenlee
	//Date Modified: 
	//Modified By DefaultUser
	//Modification notes 
	
End if 
//Variable Declarations
If (True:C214)
	C_TEXT:C284($_t_oldMethodName)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("JC_SetDoDateTimeToDoneDateTime")

[DIARY:12]Date_Do_From:4:=[DIARY:12]Date_Done_From:5
[DIARY:12]Date_Do_To:33:=[DIARY:12]Date_Done_To:34
[DIARY:12]Time_Do_From:6:=[DIARY:12]Time_Done_From:7
[DIARY:12]Time_Do_To:35:=[DIARY:12]Time_Done_To:36
ERR_MethodTrackerReturn("JC_SetDoDateTimeToDoneDateTime"; $_t_oldMethodName)