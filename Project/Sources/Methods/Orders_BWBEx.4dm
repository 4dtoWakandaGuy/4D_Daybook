//%attributes = {}
If (False:C215)
	//Project Method Name:      Orders_BWBEx
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>SYS_l_LoggedInaccount; $_l_CharPosition; $_l_DN; $_l_ItemIndex; $_l_OrderBatchNumber; MOD_l_CurrentModuleAccess)
	C_TEXT:C284($_t_BatchNumberS; $_t_DC; $_t_DNS; $_t_OCI; $_t_oldMethodName; vContrCode; vTitle5)
End if 
//Code Starts Here


$_t_oldMethodName:=ERR_MethodTracker("Orders_BWBEx")
//Orders_BWBEx
COPY NAMED SELECTION:C331([ORDERS:24]; "$Selection")
Userset
FIRST RECORD:C50([ORDERS:24])
$_l_OrderBatchNumber:=[ORDERS:24]Export_Batch_Number:45
If ($_l_OrderBatchNumber#0)
	Gen_Confirm("BWB Export Function"+Char:C90(13)+Char:C90(13)+"Do you want to find Orders not exported, or those with Batch No "+String:C10($_l_OrderBatchNumber)+"?"; "Not exported"; "Batch No")
	If (OK=1)
		$_l_OrderBatchNumber:=0
	End if 
End if 
QUERY SELECTION:C341([ORDERS:24]; [ORDERS:24]Export_Batch_Number:45=$_l_OrderBatchNumber)
If (Records in selection:C76([ORDERS:24])>0)
	...
	If (OK=1)
		...
		If ($_l_OrderBatchNumber=0)
			$_l_OrderBatchNumber:=Gen_Code(26; ->[ORDERS:24]Export_Batch_Number:45)
		End if 
		If ($_l_OrderBatchNumber>0)
			If ($_l_OrderBatchNumber>999)
				$_l_OrderBatchNumber:=$_l_OrderBatchNumber-999
				QUERY:C277([IDENTIFIERS:16]; [IDENTIFIERS:16]Table_Number:1=26)
				[IDENTIFIERS:16]Number:2:=$_l_OrderBatchNumber
				DB_SaveRecord(->[IDENTIFIERS:16])
				UNLOAD RECORD:C212([IDENTIFIERS:16])
			End if 
			$_t_BatchNumberS:=STR_LeadZeros(String:C10($_l_OrderBatchNumber); 3)
			...
			If (OK=1)
				...
				While (Not:C34(End selection:C36([ORDERS:24])))
					...
					If ([ORDERS:24]Export_Batch_Number:45=0)
						[ORDERS:24]Export_Batch_Number:45:=$_l_OrderBatchNumber
						$_bo_Save:=True:C214
					End if 
					If ($_bo_Save)
					...
					If (($_l_DN>[USER:15]BWB Doc No T:167) | ($_l_DN<[USER:15]BWB Doc No F:166))
						...
						UNLOAD RECORD:C212([IDENTIFIERS:16])
					End if 
					$_t_DNS:=STR_LeadZeros(String:C10($_l_DN); 8)
					...
					SEND PACKET:C103(STR_LeadZeros(Substring:C12(String:C10(Records in selection:C76([ORDER_ITEMS:25])+1); 1; 2); 2))  //no of A02 +1 for the A01
					SEND PACKET:C103(STR_LeadZeros(Substring:C12(String:C10(Abs:C99(Sum:C1([ORDER_ITEMS:25]Quantity:3))); 1; 5); 5))  //=#Qty field! =total of the qties of A02, 45;5
					...
					While (Not:C34(End selection:C36([ORDER_ITEMS:25])))
						...
						SEND PACKET:C103("A02"+$_t_BatchNumberS+$_t_OCI+$_t_DNS+STR_LeadZeros(String:C10($_l_ItemIndex); 2))
						...
						Else 
							...
							SEND PACKET:C103(STR_LeadZeros(String:C10(Abs:C99(Int:C8([ORDER_ITEMS:25]Quantity:3))); 5))
	...
	If ($_l_OrderBatchNumber>0)
		...
		If (OK=1)
			...
			If ($_bo_Unload)
				UNLOAD RECORD:C212([USER:15])
			End if 
			vContrCode:=STR_LeadZeros(String:C10($_l_OrderBatchNumber); 3)
			//FORM SET OUTPUT([ORDERS]; "Orders_BWB")
			PRINT SELECTION:C60([ORDERS:24])
			
			FORM SET OUTPUT:C54([ORDERS:24]; "Orders_Out"+("V"*(Num:C11(MOD_l_CurrentModuleAccess=13))))
		End if 
	End if 
...
ERR_MethodTrackerReturn("Orders_BWBEx"; $_t_oldMethodName)
