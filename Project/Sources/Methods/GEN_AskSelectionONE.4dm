//%attributes = {}
If (False:C215)
	//Project Method Name:      GEN_AskSelectionONE
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created:  11/11/2010 11:22
...
If (True:C214)
	...
	C_BOOLEAN:C305(DB_bo_SelectOne)
	C_LONGINT:C283(<>SYS_l_WindowTop; $_l_ArraySize; $_l_CurrentWinRefOLD; $_l_ParameterCount; $_l_ParameterIndex; $1; $2; WIN_l_CurrentWinRef; WIN_l_CurrentWinRefOLD)
	C_POINTER:C301($_ptr_ThisArray; $5)
...
$_t_oldMethodName:=ERR_MethodTracker("GEN_AskSelectionONE")
//Wrapper method to call Gen_AskSelection where we only select one record
DB_bo_SelectOne:=True:C214
...
If (Count parameters:C259>=8)
	...
	If (Count parameters:C259>=5)
		...
		End for 
	End if 
	$_ptr_ThisArray:=Get pointer:C304($6)
	...
	$_l_ParameterCount:=((Count parameters:C259-5)/4)+1
	For ($_l_ParameterIndex; 6; Count parameters:C259; 4)
		$_ptr_ThisArray:=Get pointer:C304(${$_l_ParameterIndex})
		APPEND TO ARRAY:C911(GEN_aptr_ColumnArrays; $_ptr_ThisArray)
...
End if 
WS_KeepFocus
ERR_MethodTrackerReturn("GEN_AskSelectionONE"; $_t_oldMethodName)