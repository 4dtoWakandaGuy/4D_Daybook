...
If (False:C215)
	//Project Method Name:      VER_GetLicenceVersion
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 07/04/2010 16:42
...
If (True:C214)
	C_BLOB:C604($_blb_myPrivateKey; $_blb_VersionBlob; $_blb_VersionPublicKey; $VER_Blob; $Versionblob)
	C_LONGINT:C283($_l_offset)
	C_TEXT:C284(<>VER_T_LicenceVersion; $_t_DataFIlePath; $_t_oldMethodName; $_t_StructurePath; $_t_VersionControlPath; $_t_VersLicenceString; $0; VER_T_PrivateKEYTEXT; VER_T_PublicKEYTEXT)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("VER_GetLicenceVersion")
//BSW 14/08/02

//NG August 2008 this method returns the LICENCE version to use for this structure
$0:=""
...
Case of 
	: (Application type:C494=4D Remote mode:K5:5)  //Get Version from server structure res file
		$_t_VersLicenceString:="0000"
		GET PROCESS VARIABLE:C371(-1; <>VER_T_LicenceVersion; $_t_VersLicenceString)
		$0:=$_t_VersLicenceString
		
	Else 
		If (<>VER_T_LicenceVersion="")
			...
			DECRYPT BLOB:C690($VER_Blob; $_blb_VersionPublicKey)
			$_l_offset:=0
			$_t_VersLicenceString:=BLOB to text:C555($VER_Blob; Mac Pascal string:K22:8; $_l_offset)
		Else 
			$_t_VersLicenceString:=<>VER_T_LicenceVersion
		End if 
		
		$0:=$_t_VersLicenceString
		
		<>VER_T_LicenceVersion:=$_t_VersLicenceString
...
If ($0="")
	$0:=VER_DefaultLicenceVersion
End if 
CLEAR SEMAPHORE:C144("LOCKVERSION")
ERR_MethodTrackerReturn("VER_Get Licence Version"; $_t_oldMethodName)
