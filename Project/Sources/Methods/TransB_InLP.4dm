...
If (False:C215)
	//Project Method Name:      TransB_InLP
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(PAL_but_LastRecord; PAL_but_PreviousRecord; PAL_but_NextRecord; PAL_but_FirstRecord)
...
$_t_oldMethodName:=ERR_MethodTracker("TransB_InLP")

//$_t_oldMethodName:=ERR_MethodTracker ("TransB_InLP")
//TransB_InLP - see also TransB_InLPJ
...
Case of 
	: ($_l_Event=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		OpenHelp(Table:C252(->[TRANSACTION_BATCHES:30]); "TransB_in")
		
		WS_AutoscreenSize(1; 560; 2000; Table name:C256(->[TRANSACTION_BATCHES:30])+"_"+"TransB_in")
		
		INT_SetInput(Table:C252(->[TRANSACTION_BATCHES:30]); WIN_t_CurrentInputForm)
		In_ButtChkDis(->vButtDisTRNB)
		
		DB_SetInputFormMenu(Table:C252(->[TRANSACTION_BATCHES:30]); "TransB_in")
	...
	: ($_l_Event=On Deactivate:K2:10)
		...
		// PAUSE PROCESS(vProc)
End case 
//ERR_MethodTrackerReturn ("TransB_InLP";$_t_oldMethodName)
