//%attributes = {}
If (False:C215)
	//Project Method Name:      Subscript_SelP
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_AndLoaded; $_bo_NoSearch; $_bo_OpenWindow; $_bo_SessionWindowOwner; SM_bo_SessionWindow; SUB_bo_FFLoaded; WIN_bo_TrackerInited)
	...
	C_LONGINT:C283($_l_SIzeofArray; $_l_TableNumber; $_l_WhilesCounter; $_l_WindowBottomOLD; $_l_WindowLeftOLD; $_l_WindowReferenceRow; $_l_WindowRightOLD; $_l_WindowTopOLD; $1; ch1; Ch2)
	C_LONGINT:C283(cNAA; cNAL; cSIS; SM_al_NextElement; SM_l_QueryMode; SM_l_QuerySearchID; SM_l_ReturnQueryID; SM_l_SearchNumber; SRCH_l_And; SRCH_l_Not; SRCH_l_or)
	C_LONGINT:C283(vCalls; vItemNum; WIN_l_CurrentWinRef)
	C_POINTER:C301($2; $3)
	C_REAL:C285(VamountF; VAmountT)
	C_TEXT:C284($_t_oldMethodName; $_t_WindowTitleOLD; vInvNo; VProdCode; VProdName; VSELPREV; Vtype)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Subscript_SelP")
//Subscript_SelP
$_bo_OpenWindow:=False:C215

Sel_Beginning(False:C215; ->[SUBSCRIPTIONS:93])
$_l_CurrentRow:=1
$_l_QueryID:=0
$_l_Mode:=0
...
End if 
$_l_WhilesCounter:=0
While ((vCalls=1) & ((SRCH_l_And=1) | (SRCH_l_or=1) | (SRCH_l_Not=1)))
	...
	If ($_l_Mode=0)
		If ($_l_WhilesCounter=1)
			Sel_Beginning2(False:C215; ->[SUBSCRIPTIONS:93])
		End if 
	Else 
		Sel_Beginning2(False:C215; ->[SUBSCRIPTIONS:93])
	End if 
	...
	If ($_l_QueryID=0)
		...
		If (Not:C34(SM_bo_SessionWindow))
			...
			Open_AnyTypeWindow(277; 428; 4; "Find Subscriptions")
		...
		Else 
			...
			If ($_bo_SessionWindowOwner)
				$_bo_OpenWindow:=True:C214
			End if 
			SET WINDOW TITLE:C213("Find Subscriptions")
		End if 
		DIALOG:C40([SUBSCRIPTIONS:93]; "dSubs_Sel")
		...
		If (SM_l_QueryMode=1) & (OK=1)
			...
			If (SM_l_ReturnQueryID=0)  // not a linked query
				SM_SearchVarsToBlob(->SM_blb_SearchBlob; ->VProdCode; ->VProdName; ->Vtype; ->vIDateF; ->vIDateT; ->vDnDateF; ->vDnDateT; ->VamountF; ->VAmountT; ->vItemNum; ->vInvNo; ->Ch2; ->cSIS; ->cNAA; ->cNAL)
	...
	Else 
		...
		If ($_l_QueryID<0)
			...
			$_l_offset:=0
			SM_l_ReturnQueryID:=0
			If (Not:C34(SUB_bo_FFLoaded))
				SM_SetUpFFArray(->[SUBSCRIPTIONS:93])
				SUB_bo_FFLoaded:=True:C214
			End if 
	...
	Else 
		Sel_S(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Product_Code:1; vProdCode)
		Sel_SNum(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Order_Item_Number:4; vItemNum)
		Sel_S(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Type:7; vType)
		Sel_SIDate(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Subscription_Date:3; vIDateF; vIDateT)
		Sel_SIDate(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Date_Done:9; vDnDateF; vDnDateT)
		If (ch1=1)
			QUERY:C277([SUBSCRIPTIONS:93]; [SUBSCRIPTIONS:93]Date_Done:9=!00-00-00!)
			Master2(->[SUBSCRIPTIONS:93])
		Else 
			If ((ch2=1) & (vDnDateF=!00-00-00!) & (vDnDateT=!00-00-00!))
				QUERY:C277([SUBSCRIPTIONS:93]; [SUBSCRIPTIONS:93]Date_Done:9#!00-00-00!)
				Master2(->[SUBSCRIPTIONS:93])
			End if 
		End if 
		Sel_SIS(->[SUBSCRIPTIONS:93])
		Sel_SSStrSin(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Product_Name:2; vProdName)
		Sel_SSStrSin(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Invoice_Number:10; vInvNo)
		Sel_SSNum(->[SUBSCRIPTIONS:93]; ->[SUBSCRIPTIONS:93]Amount_Excluding_Tax:8; vAmountF; vAmountT)
		
		Big_One(->[SUBSCRIPTIONS:93])
	End if 
End while 
Sel_Ending(->[SUBSCRIPTIONS:93])
If ($_bo_OpenWindow)
	CLOSE WINDOW:C154
...
End if 
ERR_MethodTrackerReturn("Subscript_SelP"; $_t_oldMethodName)
