...
If (False:C215)
	//Database Method Name:      CODE_GetFormObjects
	//------------------ Method Notes ------------------
...
If (True:C214)
	//C_UNKNOWN(O_On Mac Toolbar Button)
	ARRAY LONGINT:C221($_al_FormEventiDS; 0)
	ARRAY LONGINT:C221($_aL_FormEvents; 0)
	ARRAY LONGINT:C221($_al_FormObpropIDS; 0)
	ARRAY LONGINT:C221($_al_FormObTyepIDS; 0)
	ARRAY LONGINT:C221($_aL_PagesArray; 0)
	//ARRAY LONGINT(CODE_al_ConstantIDS;0)
	...
	ARRAY TEXT:C222($_at_FormEvents; 0)
	ARRAY TEXT:C222($_at_FormObjectProps; 0)
	ARRAY TEXT:C222($_at_FormObjectTypes; 0)
	ARRAY TEXT:C222($_aT_ObjectsArray; 0)
	//ARRAY TEXT(CODE_at_ConstantGroups;0)
	//ARRAY TEXT(CODE_at_ConstantNames;0)
	...
	C_LONGINT:C283($_L_altBackGroundColour; $_L_BackgroundColour; $_L_BorderLineStyle; $_l_BuildEvents; $_l_EventIDPosition; $_l_FieldNumber; $_L_FixedSubform; $_L_FontSize; $_L_FontStyle; $_L_ForegroundColour; $_L_FormEventsPos)
	C_LONGINT:C283($_l_FormHeight; $_l_FormWIdth; $_L_HorizontalResizing; $_L_index; $_L_index2; $_l_IndexRef; $_L_Indicator; $_L_ListReference; $_l_lOBottom; $_l_lOLeft; $_l_lORight)
	C_LONGINT:C283($_l_lOTop; $_L_lSHortcutModifier; $_L_MaxHeight; $_L_MaxWidth; $_L_minHeight; $_L_MultiLine; $_L_mWidth; $_L_NonInvertObjects; $_L_NumPages; $_L_ObjectAlignment; $_L_ObjectMaximumValue)
	C_LONGINT:C283($_L_ObjectMimimumValue; $_L_ObjectOrientation; $_L_objectpropsPos; $_L_objectType; $_L_objectTypesPos; $_L_Selector; $_l_StylePos; $_L_SubformHeight; $_L_SubformWidth; $_L_TableNum; $_l_TypePostion)
	C_LONGINT:C283($_L_verticalAlignment; $_L_VerticalResizing; $2)
	C_OBJECT:C1216($_Obj_ObjectObject; CORE_ob_FormObject)
	C_POINTER:C301($_ptr_FormObject; $_Ptr_ptrToSourceObject; $_Ptr_SubformTable; $_Ptr_TablePointer; $3)
...
If ($2>0)
	...
	FORM LOAD:C1103($_Ptr_TablePointer->; $_Txt_FormName; *)
	FORM GET PROPERTIES:C674($_Ptr_TablePointer->; $_Txt_FormName; $_l_FormWIdth; $_l_FormHeight; $_L_NumPages; $_Bo_FixedWidth; $_Bo_FixedHeight; $_Txt_FormTitle)
Else 
	FORM LOAD:C1103($_Txt_FormName; *)
	
	FORM GET PROPERTIES:C674($_Txt_FormName; $_l_FormWIdth; $_l_FormHeight; $_L_NumPages; $_Bo_FixedWidth; $_Bo_FixedHeight; $_Txt_FormTitle)
End if 
...
$_L_Selector:=0
If ($2>0)
	_O_FORM GET PARAMETER:C969($_Ptr_TablePointer->; $_Txt_FormName; $_L_Selector; $_L_NonInvertObjects)
Else 
	_O_FORM GET PARAMETER:C969($_Txt_FormName; $_L_Selector; $_L_NonInvertObjects)
End if 
...
If (CORE_bo_GetFormObject)  //to prevent this being run accidently
	EXECUTE METHOD:C1007("DIFFU_GetConstants"; *; ->CODE_at_ConstantNames; ->CODE_al_ConstantIDS; ->CODE_at_ConstantGroups)
	
	$_L_FormEventsPos:=Find in array:C230(CODE_at_ConstantGroups; "Form Events")
	ARRAY TEXT:C222($_at_FormEvents; 0)
	ARRAY LONGINT:C221($_al_FormEventiDS; 0)
	
	If ($_L_FormEventsPos>0)
		For ($_l_BuildEvents; $_L_FormEventsPos; Size of array:C274(CODE_at_ConstantGroups))
			If (CODE_at_ConstantGroups{$_l_BuildEvents}="Form Events")
				APPEND TO ARRAY:C911($_at_FormEvents; CODE_at_ConstantNames{$_l_BuildEvents})
				APPEND TO ARRAY:C911($_al_FormEventiDS; CODE_al_ConstantIDS{$_l_BuildEvents})
			Else 
				$_l_BuildEvents:=Size of array:C274(CODE_at_ConstantGroups)
	...
	For ($_L_index; 1; Size of array:C274($_aL_FormEvents))
		$_l_EventIDPosition:=Find in array:C230($_al_FormEventiDS; $_aL_FormEvents{$_L_index})
		If ($_l_EventIDPosition>0)
			APPEND TO ARRAY:C911($_aT_EnabledFormEvents; $_at_FormEvents{$_l_EventIDPosition})
		Else 
			Case of 
	...
	For ($_L_index; 1; Size of array:C274($_aT_ObjectsArray))
		...
		$_L_objectTypesPos:=Find in array:C230(CODE_at_ConstantGroups; "Form Object Types")
		ARRAY TEXT:C222($_at_FormObjectTypes; 0)
		ARRAY LONGINT:C221($_al_FormObTyepIDS; 0)
		
		If ($_L_objectTypesPos>0)
			For ($_l_BuildEvents; $_L_objectTypesPos; Size of array:C274(CODE_at_ConstantGroups))
				If (CODE_at_ConstantGroups{$_l_BuildEvents}="Form Object Types")
					APPEND TO ARRAY:C911($_at_FormObjectTypes; CODE_at_ConstantNames{$_l_BuildEvents})
					APPEND TO ARRAY:C911($_al_FormObTyepIDS; CODE_al_ConstantIDS{$_l_BuildEvents})
				Else 
					$_l_BuildEvents:=Size of array:C274(CODE_at_ConstantGroups)
			...
			End for 
		End if 
		$_l_TypePostion:=Find in array:C230($_al_FormObTyepIDS; $_L_objectType)
		$_Txt_objectType:=""
		If ($_l_TypePostion>0)
			$_Txt_objectType:=$_at_FormObjectTypes{$_l_TypePostion}
		End if 
		...
		Case of 
			...
			: ($_L_objectType=Object type web area:K79:41)
				...
				$_Bo_HasAlignment:=False:C215
				$_Bo_HasFont:=False:C215
			: ($_L_objectType=Object type write pro area:K79:42)
				$_Txt_objectType:="object type write pro area"
				$_Bo_HasFont:=False:C215
				$_Bo_HasAlignment:=False:C215
				//$_bo_TRACE:=True
				//TRACE
			Else 
				//if we are here this mehtod needs updating
				ALERT:C41("ERROR in Get Form Objects(In DIFFu). Unknown object type-see around line 430")
				
				$_Txt_objectType:="OTHER???"+String:C10($_L_objectType)+$_Txt_objectType
				$_Bo_HasDataSource:=False:C215
				$_Bo_HasFont:=False:C215
				$_Bo_HasAlignment:=False:C215
		End case 
		...
		For ($_L_index2; 1; Size of array:C274($_aL_FormEvents))
			$_l_EventIDPosition:=Find in array:C230($_al_FormEventiDS; $_aL_FormEvents{$_L_index2})
			If ($_l_EventIDPosition>0)
				APPEND TO ARRAY:C911($_aT_EnabledFormEvents; $_at_FormEvents{$_l_EventIDPosition})
			Else 
				...
				Case of 
					...
					: ($_aL_FormEvents{$_L_index2}=On Mouse Up:K2:58)
						APPEND TO ARRAY:C911($_aT_EnabledObjectEvents; "On mouse up")
		...
		////
		$_L_objectpropsPos:=Find in array:C230(CODE_at_ConstantGroups; "Form Objects (Properties)")
		
		ARRAY TEXT:C222($_at_FormObjectProps; 0)
		ARRAY LONGINT:C221($_al_FormObpropIDS; 0)
		
		If ($_L_objectpropsPos>0)
			For ($_l_BuildEvents; $_L_objectpropsPos; Size of array:C274(CODE_at_ConstantGroups))
				If (CODE_at_ConstantGroups{$_l_BuildEvents}="Form Objects (Properties)")
					APPEND TO ARRAY:C911($_at_FormObjectProps; CODE_at_ConstantNames{$_l_BuildEvents})
					APPEND TO ARRAY:C911($_al_FormObpropIDS; CODE_al_ConstantIDS{$_l_BuildEvents})
				Else 
					$_l_BuildEvents:=Size of array:C274(CODE_at_ConstantGroups)
		...
		$_l_StylePos:=Find in array:C230($_al_FormObpropIDS; $_L_BorderLineStyle)
		If ($_l_StylePos>0)
			$_Txt_BorderLinestyle:=$_at_FormObjectProps{$_l_StylePos}
		Else 
			Case of 
		...
		If ($_Bo_HasDataSource)  //object is a variable-this exludes lines static pictures and text
			...
			Else 
				If (Position:C15("."; $_Txt_VariableName)>0)  //with these the variable name can  change everytime you get the form objects.
					$_Txt_VariableName:="Dynamic Variable"
				End if 
			End if 
		...
		$_L_ListReference:=0  //OBJECT Get list reference(*;$_aT_ObjectsArray{$_L_index})//the problem here is these are dynamic so constantly change.
