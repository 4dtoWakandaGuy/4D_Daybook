//%attributes = {}
If (False:C215)
	//Project Method Name:      PurchOrd_StckP2
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305(<>TrackStockposting; $_bo_Edits; $_bo_ReadOnlyState; DB_bo_RecordModified; <>PersAnOver; $_bo_Edits; $_bo_ReadOnlyState; DB_bo_RecordModified)
	C_DATE:C307(<>DB_d_CurrentDate)
	C_LONGINT:C283($_l_RecordsInSelection)
	C_REAL:C285($_r_CostPrice)
	C_TEXT:C284(<>SYS_t_BaseCurrency; $_t_oldMethodName; DB_t_CallOnCloseorSave; WIN_t_CurrentInputForm; <>Per_t_CurrentDefaultAnalCode; <>SYS_t_BaseCurrency; $_t_oldMethodName; DB_t_CallOnCloseorSave; WIN_t_CurrentInputForm)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("PurchOrd_StckP2")
...
Else 
	...
	If ((Records in selection:C76([MOVEMENT_TYPES:60])>0) & ([STOCK_TYPES:59]Type_Code:1#""))
			...
			If ([PRODUCTS:9]Maintain_Stock:14)
				...
				Else 
					...
					If (<>TrackStockposting)
						STK_PostTracker("Purchase order"+[PURCHASE_ORDERS:57]Purchase_Order_Number:2+"Stock movement"+[STOCK_MOVEMENTS:40]Movement_Code:1+"Product "+[PURCHASE_ORDERS_ITEMS:169]Product_Code:1+" has a 0 quantity")
						
					End if 
				End if 
			Else 
				If (<>TrackStockposting)
					STK_PostTracker("Purchase order"+[PURCHASE_ORDERS:57]Purchase_Order_Number:2+"Stock movement"+[STOCK_MOVEMENTS:40]Movement_Code:1+"Product "+[PURCHASE_ORDERS_ITEMS:169]Product_Code:1+" does not maintain stock")
				End if 
		...
		If (OK=1)
			...
			If (OK=1)
				FORM SET INPUT:C55([STOCK_MOVEMENTS:40]; "Movements_in")  //NG April 2004 Removed ◊Screen
...
<>TrackStockposting:=False:C215
ERR_MethodTrackerReturn("PurchOrd_StckP2"; $_t_oldMethodName)
