...
If (True:C214)
	...
	C_BOOLEAN:C305(<>IME_bo_Debug; <>LOG_bo_IOLogging; <>SYS_bo_MACOptimize; IME_bo_FlushAllowed; IME_bo_Flushed; IWS_bo_ResponseHeaderSent)
	C_LONGINT:C283(<>IWS_l_PagesServed; <>LOG_l_IOLoggingNo; $_l_BlobSize; $_l_BytesSend; $_l_Error; $_l_LastCall; $_l_offset; $1; IWS_l_Stream)
	C_REAL:C285(<>IWS_r_BytesServed)
	C_TEXT:C284(<>Web_t_ServerHandler; $_t_oldMethodName; IME_t_ImageLocation; IWS_t_Error; IWS_t_ResponseHeader)
	C_TIME:C306($_ti_DocumentRef)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("IME_Flush")
...
Case of 
	: (<>Web_t_ServerHandler="ITK")
		...
		If ((IME_bo_FlushAllowed) & (IME_bo_Flushed=False:C215))
			...
			//$_l_Error:=ITK_TCPSendBlob (IWS_l_Stream;IME_blb_Result;0;0;0;0) ITK OUT
			If ($_l_Error<0)
				//There was an error which needs reporting
				IWS_t_Error:="Error #"+String:C10($_l_Error)+" sending blob with ITK_TCPSendBlob"
			Else 
				If (<>LOG_bo_IOLogging)
					...
					While ($_l_offset<$_l_BlobSize)  //$_l_offset will be incremented each time
						SEND PACKET:C103($_ti_DocumentRef; BLOB to text:C555(IME_blb_Result; Mac text without length:K22:10; $_l_offset; 32000))
					End while 
				End if 
			End if 
...
//LOG_ResetMethod
ERR_MethodTrackerReturn("IME_Flush"; $_t_oldMethodName)