//%attributes = {}
If (False:C215)
	//Project Method Name:      Gen_PayDate
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 16/12/2009 14:50`Method: Gen_PayDate
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_Selection; $2; ACC_bo_CurrencyWriteOff; ACC_bo_DebtWriteoff; ACC_bo_LockAmounts; CCM_bo_IsLicenced; CCM_bo_OfflineHandling; GEN_Bo_ShowCreate; INV_bo_HidePaymentType; PAY_bo_CreateAuths; WIN_bo_TrackerInited)
...
$_t_oldMethodName:=ERR_MethodTracker("Gen_PayDate")
//Gen_PayDate
...
While (((vDate=!00-00-00!) | (Records in selection:C76([ACCOUNTS:32])#1)) & ($_l_OK=1))
	...
	INV_bo_HidePaymentType:=False:C215
	If ($1="P")
		INV_bo_HidePaymentType:=True:C214
	End if 
...
If ($_bo_Selection)  // in a CC transaction the amount wwill always be in the amount of the invoice
	If ((OK=1) & (DB_GetModuleSettingByNUM(Module_MultiCurrency)>1)) & (ACC_t_CurrencyCode#vCountry)  //4.0.031"
		...
		DIALOG:C40([INVOICES:39]; "dPayment_Curr")
		CLOSE WINDOW:C154
		$_l_OK:=OK
		OK:=1
		If (WIN_bo_TrackerInited)
...
End if 
ERR_MethodTrackerReturn("Gen_PayDate"; $_t_oldMethodName)