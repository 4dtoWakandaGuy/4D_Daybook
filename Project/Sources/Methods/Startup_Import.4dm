//%attributes = {}
If (False:C215)
	//Project Method Name:      Startup_Import
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("Startup_Import")
//Startup_Import
Load_Fields
	...
	If (Size of array:C274(GEN_at_Identity)>0)
		...
		If (OK=1)
			...
			If (Gen_Option)
				...
				DIALOG:C40([COMPANIES:2]; "dStartup_ImpOpt")
				CLOSE WINDOW:C154
				If (WIN_bo_TrackerInited)
					$_l_WindowReferenceRow:=Find in array:C230(WIN_al_ThisWIndowReference; WIN_l_CurrentWinRef)
					If ($_l_WindowReferenceRow>0)
				...
				If (OK=1)
					...
					If (OK=1)
						...
						If (vT="Field:")
							...
							Startup_Import2(1)
						Else 
							Gen_Alert("That is not a valid Startup_Import file"; "Cancel")
						End if 
						CLOSE DOCUMENT:C267(vDoc)
					Else 
...
End if 
ON ERR CALL:C155("")
ERR_MethodTrackerReturn("Startup_Import"; $_t_oldMethodName)