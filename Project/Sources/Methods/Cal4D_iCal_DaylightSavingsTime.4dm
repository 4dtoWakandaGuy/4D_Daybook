...
If (False:C215)
	//Project Method Name:      Cal4D_iCal_DaylightSavingsTime
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 23/12/2009 12:01
...
$_t_oldMethodName:=ERR_MethodTracker("Cal4D_iCal_DaylightSavingsTime")
...
Case of 
	: (($_l_Month>=3) & ($_l_Month<=11))
		Case of 
			: ($_l_Month=3)
				$_bo_DST:=($_l_Day>13)
				If (Not:C34($_bo_DST))
					$_l_1stDayNo:=Day of:C23(Date:C102("03/01/"+String:C10(Year of:C25($_d_Date))))
					Case of 
			...
			: ($_l_Month=11)
				If ($_l_Day>6)
					$_bo_DST:=False:C215
				Else 
					$_bo_DST:=($_l_DayNo#Sunday:K10:19)
				End if 
			Else 
				$_bo_DST:=True:C214
		End case 
...
$0:=$_bo_DST
ERR_MethodTrackerReturn("Cal4D_iCal_DaylightSavingsTime"; $_t_oldMethodName)