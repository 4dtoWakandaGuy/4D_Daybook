//%attributes = {}
If (False:C215)
	//Project Method Name:      StatsView_Print
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("StatsView_Print")
//StatsView_Print
...
If (<>SYS_l_CancelProcess#Current process:C322)
	...
	If (OK=1)
		...
		// PAGE SETUP([COMPANIES];"SV_PrintHdr")
		
		//PRINT SETTINGS
		If ($_l_OK>=0)  // See comming in Print_setSize abouthis
			Print form:C5([COMPANIES:2]; "SV_PrintHdr")
			...
			While (($_l_Index<=$_l_SizeofArray) & (<>SYS_l_CancelProcess#Current process:C322))
				...
				vTot2:=aSV3{$_l_Index}
				vTot3:=aSV4{$_l_Index}
				Print form:C5([COMPANIES:2]; "SV_PrintDet")
				$_l_Index:=$_l_Index+1
			End while 
			Print form:C5([COMPANIES:2]; "SV_PrintFtr")
			PAGE BREAK:C6
			PRT_SetPrinterPaperOptions(Vtitle1)  //NG June 2005
		End if 
	End if 
End if 
ERR_MethodTrackerReturn("StatsView_Print"; $_t_oldMethodName)
