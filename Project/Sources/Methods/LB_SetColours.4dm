//%attributes = {}
If (False:C215)
	//Project Method Name:      LB_SetColours
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 22/06/2009 10:45
	//Created BY: Nigel Greenlee
	//Date Modified: 
	//Modified By DefaultUser
	//Modification notes 
	
End if 
//Variable Declarations
If (True:C214)
	C_LONGINT:C283($_l_AltBackGround; $_l_BackGround; $_l_Colour; $_l_Colour2; $_l_Foreground; $2; $3; $4)
	C_POINTER:C301($1)
	C_TEXT:C284($_t_AltBackGround; $_t_BackGround; $_t_Foreground; $_t_oldMethodName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("LB_SetColours")
//Color:=-(Foreground+(256 * Background))
// $1 = pointer to the object (e.g., listbox column (array)
// $2 = foreground colour
// $3 = background colour
// $4 (optional) - 2nd bg colour for stripey effect

// examples:
//LB_SetColours (->atOne;0;255;243)  ` stripey - white font black/grey stipes
//LB_SetColours (->atOne;0;255)  ` white font/ black bg
//LB_SetColours (->atOne;255;243;0)  ` black font/ grey+white stripey bg
//LB_SetColours (->atOne;White ;Black ;Light Grey )  ` stripey - white font black/grey stipes
//LB_SetColours (->SC_lb_PROD;Black ;Grey ;Light Grey )  ` stripey - black font light grey/grey stipes

If (Count parameters:C259>=3)
	Case of 
		: (Count parameters:C259=3)
			$_l_Colour:=-($2+(256*$3))
			$_t_Foreground:=UTIL_4DColorToRGB($2)
			$_t_BackGround:=UTIL_4DColorToRGB($3)
			OBJECT SET RGB COLORS:C628($1->; $_t_Foreground; $_t_BackGround)
			
			//_O_OBJECT SET COLOR($1->;$_l_Colour)
			
		: (Count parameters:C259=4)
			//$_l_Colour:=-($2+(256*$3))
			//$_l_Colour2:=-($4+(256*$5))
			//$_l_Colour2:=-($2+(256*$4))
			//_O_OBJECT SET COLOR($1->;$_l_Colour;$_l_Colour2)
			$_t_Foreground:=UTIL_4DColorToRGB($2)
			$_t_BackGround:=UTIL_4DColorToRGB($3)
			$_t_AltBackGround:=UTIL_4DColorToRGB($4)
			OBJECT SET RGB COLORS:C628($1->; $_t_Foreground; $_t_BackGround; $_t_AltBackGround)
			
	End case 
	
End if 
ERR_MethodTrackerReturn("LB_SetColours"; $_t_oldMethodName)
