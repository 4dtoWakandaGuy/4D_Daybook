...
If (False:C215)
	//Project Method Name:      LOC_SUbLevelInformation
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_arraySize; $_l_CurrentLevel; $_l_event; $_l_FillArray; $_l_HeaderLevel; $_l_Index; $_l_NextLevel; $_l_NextLevelHeader; $_l_oBottom; $_l_oLeft; $_l_oRight)
	C_LONGINT:C283($_l_oTop; $_l_ProductID; $_l_ProductPosition; $_l_ProductsCount; $_l_SelectedRow; $_l_ThisLocationID; $_l_WindowBottom; $_l_WindowLeft; $_l_WindowRight; $_l_windowTop; $1)
	C_LONGINT:C283($2; STK_l_Subid; xNext)
	...
	C_POINTER:C301($_ptr_CurrentLocation; $_ptr_CurrentLocationCode; $_ptr_CurrentLocationCount; $_ptr_CurrentLocationID; $_ptr_CurrentLocationSubLevName; $_ptr_ListBoxArea; $_Ptr_LocationNamesArray; $_Ptr_NextAddPrdl; $_Ptr_NextAddSubLevel; $_Ptr_NEXTCurrentLocation; $_Ptr_NEXTCurrentLocationCode)
	...
	C_TEXT:C284($_t_ButtonsName; $_t_CountNextSubLevelsSTR; $_t_Label; $_t_LevelName; $_t_LocationSubdivisionName; $_t_NEXTProdCodeLabelName; $_t_NEXTProdCountLabelName; $_t_NEXTProdNameLabelName; $_t_NextSubLevelLabel; $_t_NEXTSubLevelLabelName; $_t_NextSubLocationCode)
	C_TEXT:C284($_t_oldMethodName; $_t_ProdCodeLabelName; $_t_ProdCountLabelName; $_t_ProdNameLabelName; $_t_ProductCode; $_t_SelectedOption; $_t_Singular; $_t_Singular2; $_t_SublevelName; $_t_SubLocationDefault; $_t_NextSubLevelNamed)
	C_TEXT:C284($_t_ProductCode; STK_t_ProductCode)
End if 

//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("LOC_SUbLevelInformation")
$_l_event:=$1
...
Case of 
	: ($_l_event=On Clicked:K2:4)
		//```
		//$_l_CurrentLevel:=FORM Get current page
		...
		$_t_SublevelName:="oSubLocationCountLabel"+String:C10($_l_HeaderLevel)
		...
		$_ptr_SubLevelSelect->:=$_ptr_Array2->{$_l_SelectedRow}
	...
	: ($_l_event=On Double Clicked:K2:5)
		...
		Case of 
			: ($_t_NextSubLevelNamed#"")  //The location has a sublevel
				...
				$_l_NextLevelHeader:=$_l_CurrentLevel
				...
				//````
				//$_Ptr_NEXTCurrentLocationcount->:=Num($_t_CountNextSubLevelsSTR)
				...
				$_Ptr_NEXTCurrentLocationcount->:=Records in selection:C76([LOCATIONS:61])
				...
				$_l_arraySize:=Size of array:C274($_Ptr_SubLocationCodes->)
				ARRAY TEXT:C222($_ptr_SubLocationPrdCd->; $_l_arraySize)
				ARRAY TEXT:C222($_Ptr_SubLocationPrdNm->; $_l_arraySize)
				ARRAY BOOLEAN:C223($_ptr_SubLocationModified->; $_l_arraySize)
				ARRAY REAL:C219($_ptr_SubLocationStkCount->; $_l_arraySize)
				ARRAY TEXT:C222($_ptr_SubLocationHolds->; $_l_arraySize)
				...
				LB_SetColumnHeaders($_ptr_ListBoxArea; $_t_ButtonsName; 1; "Location Point"; "Location Code"; "Location Content"; "Product At location"; "Product Name"; "SubLevel"; "Location ID"; "Modified")
				...
				//````
				FORM GOTO PAGE:C247($_l_CurrentLevel+1)
				//```
					...
					Case of 
							...
							If ($_t_ProductCode#"") & (OK=1)
								...
								If (STK_bo_ProductFound)
									If ($_ptr_Array8->{$_l_SelectedRow}#"Multiple Products")
										...
										[x_ProductLocations:185]x_LocationCode:2:=$_ptr_Array3->{$_l_SelectedRow}
						...
						: ($_t_SelectedOption="Remove Product from Location")
							...
							If ($_ptr_Array8->{$_l_SelectedRow}#"Multiple Products")
								...
								QUERY:C277([x_ProductLocations:185];  & ; [x_ProductLocations:185]x_LocationCode:2=$_ptr_Array3->{$_l_SelectedRow})
								If (Records in selection:C76([x_ProductLocations:185])>0)
									STK_GetActiveThreads($_ptr_Array8->{$_l_SelectedRow}; ""; 0; "ANY"; "ANY"; "ANY"; 0; 0; ""; 0; 0; ""; ""; $_ptr_Array3->{$_l_SelectedRow})
				...
				Case of 
					...
					: (OK=0)  //Sublocation
						...
						If ($_t_NextSubLevelLabel#"")
							$_t_Singular2:=GenWordSingle($_t_NextSubLevelLabel)
							Case of   // These default names are just so we can populate something into the request box that looks sensible
								...
								Else 
									If (Length:C16($_t_NextSubLevelLabel)>1)
										$_t_SubLocationDefault:=Substring:C12($_t_NextSubLevelLabel; 1; 2)
									Else 
										$_t_SubLocationDefault:=$_t_NextSubLevelLabel
									End if 
							End case 
							$_t_NextSubLocationCode:=Gen_Request("Enter "+$_t_Singular2+" Coding prefix"; $_t_SubLocationDefault)
							If ($_t_NextSubLocationCode#"") & (OK=1)
								...
								If (Num:C11($_t_CountNextSubLevelsSTR)>0)
									...
									QUERY:C277([LOCATIONS:61]; [LOCATIONS:61]LocationID:4=$_ptr_Array2->{$_l_SelectedRow})
									...
									For ($_l_FillArray; 1; Size of array:C274($_ptr_SubLocationHolds->))
										$_Ptr_SubLocationCodes->{$_l_FillArray}:=$_ptr_Array3->{$_l_SelectedRow}+$_t_NextSubLocationCode+String:C10($_l_FillArray)
										$_ptr_SubLocationNames->{$_l_FillArray}:=$_Ptr_LocationNamesArray->{$_l_SelectedRow}+","+$_t_Singular2+" "+String:C10($_l_FillArray)
										STK_l_Subid:=0
										...
										[LOCATIONS:61]LocationCodePrefix:5:=$_ptr_Array3->{$_l_SelectedRow}  //This is correct
...
End case 
ERR_MethodTrackerReturn("OBJ [USER].STK_LocationsEditor.oSubLevelInformation0"; $_t_oldMethodName)