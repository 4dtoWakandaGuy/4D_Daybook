//%attributes = {}
If (False:C215)
	//Project Method Name:      Gen_Confirm_WithHideOption
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305(<>SYS_bo_CurrentWindowModal; $_bo_Change; $_bo_OK; GEN_bo_ConfirmOption; GEN_bo_ShowOption)
	C_LONGINT:C283($_l_ArraySize; $_l_BlobOffset; $_l_CurrentWinRefOLD; $_l_Delay; $_l_IdentRow; $_l_ItemID; $_l_offset; $_l_Repeats; $_l_ScreenCentreVertical; B_CheckHideWarning; WIN_l_CurrentWinRef)
	C_LONGINT:C283(WIN_l_CurrentWinRefOLD; xCancel)
	C_REAL:C285($0; $1)
	C_TEXT:C284(<>ButtCanc; <>ButtOK; <>PER_t_CurrentUserInitials; $_t_oldMethodName; $2; $3; $4; vConfirm)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Gen_Confirm_WithHideOption")
//Gen_Confirm
...
//1=Macro Item from the SM_HL_l_Setmanager in WS set manager
//2=Macro Create auto search(from Ws_setManager
//301 Is do you want to cehck PO when pritning a PO from a SO
//301=  do you want to update the product cost price from OrderI_SubCost
//302 =Price update on product from sales order
...
Else 
	...
	APPEND TO ARRAY:C911(Gen_al_ConfirmOption; 0)  //Not relevant on a two button confirm
...
If (GEN_bo_ConfirmOption=False:C215) | (Macintosh option down:C545) | (Windows Alt down:C563)
	...
	If ($_l_IdentRow>0)
		If (Gen_abo_ConfirmPreference{$_l_IdentRow}#(B_CheckHideWarning=1))
