//%attributes = {}
If (False:C215)
	//Project Method Name:      Sequences_InLP
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_ButtonNumber; $_l_Event; $_l_ModuleRow; $1; DB_l_ButtonClickedFunction; DB_l_WindowCurrentModule; iCancel; iOK; ModuleNumber; PAL_but_FirstRecord; PAL_but_LastRecord)
...
$_t_oldMethodName:=ERR_MethodTracker("Sequences_InLP")
//Sequences_InLP
If (Count parameters:C259>=1)
	$_obj_Event:=FORM Event:C1606
	$_l_Event:=$_obj_Event.code
Else 
	$_l_Event:=$1
End if 
...
Case of 
	...
	: ($_l_Event=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		OBJECT SET VISIBLE:C603(*; "oInputButtonNextRecord"; (Records in selection:C76([WORKFLOW_CONTROL:51])>1))
		OBJECT SET VISIBLE:C603(*; "oInputButtonPreviousRecord"; (Records in selection:C76([WORKFLOW_CONTROL:51])>1))
		OBJECT SET VISIBLE:C603(*; "oInputButtonFirstRecord"; (Records in selection:C76([WORKFLOW_CONTROL:51])>1))
		OBJECT SET VISIBLE:C603(*; "oInputButtonLastRecord"; (Records in selection:C76([WORKFLOW_CONTROL:51])>1))
		DB_MenuAction("File"; "Next Record"; 3; ""; Current process:C322; (Records in selection:C76([WORKFLOW_CONTROL:51])>1); DB_t_CUrrentInputMenuRef)
		DB_MenuAction("File"; "Previous Record"; 3; ""; Current process:C322; (Records in selection:C76([WORKFLOW_CONTROL:51])>1); DB_t_CUrrentInputMenuRef)
	...
	: ($_l_Event=On Deactivate:K2:10)
		// PAUSE PROCESS(vProc)
End case 
ERR_MethodTrackerReturn("Sequences_InLP"; $_t_oldMethodName)
