...
If (False:C215)
	//Project Method Name:      DBWS_GetQuerySetup
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_TEXT:C284($_t_oldMethodName; $_t_String; $_t_WebGetTableContext; $_t_WebGetTableCurrentForm; $_t_WebGetTableSetup; $1; DBWS_t_DataGridJSON)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("DBWS_GetQuerySetup")


//This method returns several things
//) Should the search box be displayed for the current form.
//) What are the drop down values for the search director selector
...
If (True:C214)
		...
		If ($_l_FormVarRow>0)
			...
			If (Records in selection:C76([WEB_ACCESS:130])=1)
				...
				If ($_l_UserProcess>0)
					...
					//````````
					DBWS_t_DataGridJSON:=""
					SET PROCESS VARIABLE:C370(<>DB_AL_UserProcessID{$_l_UserProcess}; DBWS_l_CallAction; 3)
	...
	End if 
End if 
ERR_MethodTrackerReturn("DBWS_GetQuerySetup"; $_t_oldMethodName)