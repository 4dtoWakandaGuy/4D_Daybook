//%attributes = {}
If (False:C215)
	//Project Method Name:      ACC_AutoBudgets
	//------------------ Method Notes ------------------
	//------------------ Revision Control ----------------
	//Date Created:    16/11/2010 21:14
...
If (True:C214)
	...
	C_LONGINT:C283($_l_AnalysisCodeCount; $_l_CurrencyCodeCount; $_l_Index; $_l_Index2; $_l_ModuleAccess; $_l_PeriodID; $_l_WindowReferenceRow; ch1; MOD_l_CurrentModuleAccess; vNo; WIN_l_CurrentWinRef)
...
$_t_oldMethodName:=ERR_MethodTracker("ACC_AutoBudgets")
$_l_ModuleAccess:=DB_GetModuleSettingByNUM(Module_NominalLedger)

If (($_l_ModuleAccess=2) | ($_l_ModuleAccess=4))
	If (DB_DataisSetup(Table:C252(->[ACCOUNTS:32])))
			...
			If (OK=1)
				...
				While (Not:C34(End selection:C36([ACCOUNTS:32])))
					...
					If ((Records in selection:C76([ACCOUNT_BALANCES:34])>0) | ($_obj_FormData.FilterinEmptyAccounts))
						...
						ARRAY TEXT:C222($_at_AnalysisCode; 0)
						ARRAY TEXT:C222($_at_CurrencyCode; 0)
						DISTINCT VALUES:C339([ACCOUNT_BALANCES:34]Analysis_Code:1; $_at_analysisCode)
						DISTINCT VALUES:C339([ACCOUNT_BALANCES:34]Currency_Code:6; $_at_CurrencyCode)
						MESSAGES ON:C181
						$_l_AnalysisCodeCount:=Size of array:C274($_at_analysisCode)
						$_l_CurrencyCodeCount:=Size of array:C274($_at_CurrencyCode)
						If (($_l_AnalysisCodeCount>1) | ($_l_CurrencyCodeCount>1))
							COPY NAMED SELECTION:C331([ACCOUNT_BALANCES:34]; "$Copy")
							For ($_l_Index; 1; $_l_AnalysisCodeCount)
								QUERY SELECTION:C341([ACCOUNT_BALANCES:34]; [ACCOUNT_BALANCES:34]Analysis_Code:1=$_at_analysisCode{$_l_Index})
								COPY NAMED SELECTION:C331([ACCOUNT_BALANCES:34]; "$Copy2")
								For ($_l_Index2; 1; $_l_CurrencyCodeCount)
									QUERY SELECTION:C341([ACCOUNT_BALANCES:34]; [ACCOUNT_BALANCES:34]Currency_Code:6=$_at_CurrencyCode{$_l_Index2})
									If ((Records in selection:C76([ACCOUNT_BALANCES:34])>0) | ($_obj_FormData.FilterinEmptyAccounts))
										...
										Acc_BalBudCreate2($_obj_FormData)
	...
	End if 
End if 
ERR_MethodTrackerReturn("ACC_AutoBudgets"; $_t_oldMethodName)
