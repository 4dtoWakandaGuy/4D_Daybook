...
If (True:C214)
	...
	C_BOOLEAN:C305(<>DB_bo_useFileOpenPlugin; <>Err_bo_HandlerInited; <>Err_bo_PrefShowChainInErrorEmai; <>Err_bo_PrefShowChainInErrorMess; <>PW_Bo_WindowsAutoLogin; <>ST_Bo_ShowSplash; <>SYS_bo_inStartup; $_bo_BootModeOK; $_bo_Compiled; $_bo_loggedIn; $_bo_Login)
	C_BOOLEAN:C305($_bo_NewDataFile; $_bo_Startup; CD_bo_ClearDayLogin)
	C_DATE:C307(<>DB_d_CurrentDate)
	C_LONGINT:C283(<>LIC_l_Seed; <>Menuwindow; $_l_Error; $_l_Platform; $_l_Process; $_l_RecordsInTables; $_l_TableIndex; DB_l_currentuserID; DB_l_UserIdentWindow; ST_l_StartupMacros)
	C_POINTER:C301($_ptr_Table)
	C_TEXT:C284($_t_DirectorySymbol; $_t_NewPath; $_t_oldMethodName; $_t_ResourcesPath; $_t_UserConstantsPath; $_t_UserName; $_t_VersionNumber; $_t_VersionNumber2)
...
If (DB_ControlDataFilePath)
	...
	Else 
		...
		If (Not:C34(User in group:C338(Current user:C182; "ThrowOut"))) | (Is compiled mode:C492) | (True:C214)
			...
			//$_l_Process:=New process("MOD_SHowModuleSettings";DB_ProcessMemoryinit(2);"Showing Modules settings")
			//SET PLUGIN ACCESS(808464697;"Designer")
			//Edit_Access
			$_bo_Compiled:=True:C214
			If (Not:C34(Is compiled mode:C492))
				$_bo_Compiled:=False:C215
				$_l_Process:=New process:C317("DesignSaver"; DB_ProcessMemoryinit(4); "$designsaverProcess")
				ARRAY TEXT:C222($_at_ComponentNames; 0)
			...
			If (Application type:C494=4D Volume desktop:K5:2)
				//HIDE WINDOW(<>SYS_l_ApplicationWindowRef)
			End if 
			HIDE TOOL BAR:C434
			...
			If ($_bo_Login)
				...
				If ($_bo_BootModeOK)
					...
					//4DBKC_Init NG Huly 2007 as not using not initing
					//$_l_Process:=New process("START_inProcess";DB_ProcessMemoryinit(8);"StartUpinProcess")
					...
					If (DB_l_currentuserID>0)
						...
						If (User in group:C338(Current user:C182; "Designer"))
							If (Not:C34(Is compiled mode:C492))
								//$_l_Process:=New process("DesignSaver";DB_ProcessMemoryinit(4);"$designsaverProcess";
						...
						If ($_bo_Startup)
							...
							Else 
								...
								If ([USER:15]Last Version:92#<>SYS_t_VersionNumber)  // | ($_t_VersionNumber#$_t_VersionNumber2)
									...
									User_VersionUp($_t_VersionNumber; $_t_VersionNumber2)
								...
								$_l_Process:=New process:C317("ProcessServices"; DB_ProcessMemoryinit(4); "Service Process"; *)  // 16/4/02 pb
								
								$_l_Process:=New process:C317("DM_DataRetreiveManager"; DB_ProcessMemoryinit(16); "Data RetreiveManager"; *)
				...
				If (Application type:C494#4D Remote mode:K5:5) & (Not:C34(Is compiled mode:C492))
					...
					$_l_Process:=New process:C317("DB_MonitorMethodChanges"; DB_ProcessMemoryinit(2); "Export Code"; *)
					
				End if 
				
				If ($_bo_Startup)
					//SPLASH_END_GRACEFULLY
					//SPLASH_TIMER (1)  `update splash timer with average time it took to startup
				Else 
					//SPLASH_END_GRACEFULLY
					//SPLASH_TIMER (1;60)  `update splash timer with as user didn't log in properly
				End if 
			...
			End if 
			CLEAR SEMAPHORE:C144("$DuringLogin")
			//$_l_Process:=New process("PW_CHECKUSERS";DB_ProcessMemoryinit(4);"Saving Passwords")
