//%attributes = {}
If (False:C215)
	//Project Method Name:      InvoiceItems_SelP
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 12/02/2010 12:12
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_AndLoaded; $_bo_NoSearch; $_bo_OpenWindow; $_bo_SessionWindowOwner; INVI_bo_FFLoaded; SM_bo_SessionWindow; WIN_bo_TrackerInited)
	...
	C_LONGINT:C283(cSetAdd; cSetMod; cSIS; r0; r1; r2; r3; r4; r5; r6; r7)
	C_LONGINT:C283(SM_al_NextElement; SM_l_QueryMode; SM_l_QuerySearchID; SM_l_ReturnQueryID; SM_l_SearchNumber; SRCH_l_And; SRCH_l_Not; SRCH_l_or; VItemNum; vOrdI; WIN_l_CurrentWinRef)
	C_POINTER:C301($2; $3)
	C_REAL:C285(VamountF; VamountT; vInvI; VMarginF; VMarginT)
	C_TEXT:C284($_t_oldMethodName; $_t_WindowTitleOLD; ACC_t_CurrencyCode; Varea; vCOntrCode; VInvNoF; VInvNoT; vItemRef; Vlayer; VOrderCode; vOrderNo)
	C_TEXT:C284(VProbCode; VProdCode; vProdName; VSales; VSELPREV; VSerialNo; VSolCode)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("InvoiceItems_SelP")

$_bo_OpenWindow:=False:C215


Sel_Beginning(False:C215; ->[INVOICES_ITEMS:161])
$_l_CurrentRow:=1
$_l_QueryID:=0
$_l_Mode:=0
...
End if 
$_l_WhilesCounter:=0
While ((vInvI=1) & ((SRCH_l_And=1) | (SRCH_l_or=1) | (SRCH_l_Not=1))) | ($_l_CurrentRow<=$_l_ElementCount)
	...
	If ($_l_Mode=0)
		If ($_l_WhilesCounter=1)
			Sel_Beginning2(False:C215; ->[INVOICES_ITEMS:161])
		End if 
	Else 
		Sel_Beginning2(False:C215; ->[INVOICES_ITEMS:161])
	End if 
	...
	If ($_l_QueryID=0)
		...
		If (Not:C34(SM_bo_SessionWindow))
			...
			Open_AnyTypeWindow(322; 528; 4; "Find Invoice Items")
		Else 
			...
			If ($_bo_SessionWindowOwner)
				$_bo_OpenWindow:=True:C214
			End if 
			SET WINDOW TITLE:C213("Find Invoice items")
		End if 
		DIALOG:C40([ORDER_ITEMS:25]; "ditems_sel")
	...
	Else 
		...
		If ($_l_QueryID<0)
			...
			$_l_offset:=0
			SM_l_ReturnQueryID:=0
			If (Not:C34(INVI_bo_FFLoaded))
				SM_SetUpFFArray(->[INVOICES_ITEMS:161])
				INVI_bo_FFLoaded:=True:C214
			End if 
...
End if 
ERR_MethodTrackerReturn("InvoiceItems_SelP"; $_t_oldMethodName)