//%attributes = {}
If (False:C215)
	//Project Method Name:      Projects_SelP
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(cSIS; SM_al_NextElement; SM_l_QueryMode; SM_l_QuerySearchID; SM_l_ReturnQueryID; SM_l_SearchNumber; SRCH_l_And; SRCH_l_Not; SRCH_l_or; vA; WIN_l_CurrentWinRef)
	C_POINTER:C301($2; $3)
	C_TEXT:C284($_t_oldMethodName; $_t_WindowTitleOLD; vCompCode; vCOmpName; vCOntCode; vForename; vJob; vJobcode; vSales; VSELPREV; vSUrname)
	C_TEXT:C284(vType)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Projects_SelP")
//Projects_SelP
$_bo_OpenWindow:=False:C215


Sel_Beginning(False:C215; ->[PROJECTS:89])
$_l_CurrentRow:=1
$_l_QueryID:=0
$_l_Mode:=0
...
End if 
$_l_WhilesCounter:=0
While ((vA=1) & ((SRCH_l_And=1) | (SRCH_l_or=1) | (SRCH_l_Not=1))) | ($_l_CurrentRow<=$_l_ElementCount)
	...
	If ($_l_Mode=0)
		If ($_l_WhilesCounter=1)
			Sel_Beginning2(False:C215; ->[PROJECTS:89])
		End if 
	Else 
		Sel_Beginning2(False:C215; ->[PROJECTS:89])
	End if 
	...
	If ($_l_QueryID=0)
		...
		If (Not:C34(SM_bo_SessionWindow))
			...
			Open_AnyTypeWindow(322; 528; 4; "Find Projects")
		Else 
			...
			If ($_bo_SessionWindowOwner)
				$_bo_OpenWindow:=True:C214
			End if 
			SET WINDOW TITLE:C213("Find Projects")
		End if 
		DIALOG:C40([PROJECTS:89]; "dProjects_Sel")
		...
		If (SM_l_QueryMode=1) & (OK=1)
			...
			If (SM_l_ReturnQueryID=0)  // not a linked query
				SM_SearchVarsToBlob(->SM_blb_SearchBlob; ->vJobcode; ->vJob; ->vCompCode; ->vCOmpName; ->vCOntCode; ->vForename; ->vSUrname; ->vSales; ->vType; ->vsDateF; ->vsDateT; ->SRCH_d_EntryDateFrom; ->SRCH_d_EntryDateTo; ->MOD_al_ModuleAccessStatus; ->SYS_at_RecordStateNames; ->cSIS; ->cNAA; ->cNAL)
	...
	If (OK=0) | ($_l_Mode#0) | ($_bo_NoSearch=True:C214)
		...
		End if 
	Else 
		Sel_S(->[PROJECTS:89]; ->[PROJECTS:89]Company_Code:3; vCompCode)
		Sel_S(->[PROJECTS:89]; ->[PROJECTS:89]Contact_Code:4; vContCode)
		Sel_S(->[PROJECTS:89]; ->[PROJECTS:89]Project_Code:1; vJobCode)
		Sel_S(->[PROJECTS:89]; ->[PROJECTS:89]Project_Name:2; vJob)
		Sel_S(->[PROJECTS:89]; ->[PROJECTS:89]Project_Type:7; vType)
		Sel_S(->[PROJECTS:89]; ->[PROJECTS:89]Person:8; vSales)
		Sel_State(->[PROJECTS:89]; ->[PROJECTS:89]State:9)
		Sel_SIS(->[PROJECTS:89])
		Sel_SSDate(->[PROJECTS:89]; ->[PROJECTS:89]Start_Date:5; vSDateF; vSDateT)
		Sel_SSDate(->[PROJECTS:89]; ->[PROJECTS:89]End_Date:6; SRCH_d_EntryDateFrom; SRCH_d_EntryDateTo)
		
		If (vCompName#"")
			Comp_SelPOther(->[PROJECTS:89]; ->[PROJECTS:89]Company_Code:3; "Master2"; vCompName)
		End if 
		If ((vForename#"") | (vSurname#""))
			Cont_SelPOther(->[PROJECTS:89]; ->[PROJECTS:89]Contact_Code:4; "Master2"; vForename; VSurname)
		End if 
		
		Big_One(->[PROJECTS:89])
	End if 
End while 
Sel_Ending(->[PROJECTS:89])
If ($_bo_OpenWindow)
	CLOSE WINDOW:C154
...
//Sel_EndArrClear
ERR_MethodTrackerReturn("Projects_SelP"; $_t_oldMethodName)
