//%attributes = {}
If (False:C215)
	//Project Method Name:      JobStages_SelP
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(cSIS; SM_al_NextElement; SM_l_QueryMode; SM_l_QuerySearchID; SM_l_ReturnQueryID; SM_l_SearchNumber; SRCH_l_And; SRCH_l_Not; SRCH_l_or; vJ; WIN_l_CurrentWinRef)
	C_POINTER:C301($2; $3)
	C_TEXT:C284($_t_oldMethodName; $_t_WindowTitleOLD; VcompCode; VcontName; VJobCode; VSELPREV; Vstage)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("JobStages_SelP")
//JobStages_SelP
$_bo_OpenWindow:=False:C215

Sel_Beginning(False:C215; ->[JOB_STAGES:47])
$_l_CurrentRow:=3
...
While ((vJ=1) & ((SRCH_l_And=1) | (SRCH_l_or=1) | (SRCH_l_Not=1))) | ($_l_CurrentRow<=$_l_ElementCount)
	...
	If ($_l_Mode=0)
		If ($_l_WhilesCounter=1)
			Sel_Beginning2(False:C215; ->[JOB_STAGES:47])
		End if 
	Else 
		Sel_Beginning2(False:C215; ->[JOB_STAGES:47])
	End if 
	...
	If ($_l_QueryID=0)
		...
		If (Not:C34(SM_bo_SessionWindow))
			...
			Open_AnyTypeWindow(277; 428; 4; "Find Job Stages")
		...
		Else 
			...
			If ($_bo_SessionWindowOwner)
				$_bo_OpenWindow:=True:C214
			End if 
			SET WINDOW TITLE:C213("Find Job Stages")
		End if 
		DIALOG:C40([JOB_STAGES:47]; "dJobStages_Sel")
	...
	If (OK=0) | ($_l_Mode#0) | ($_bo_NoSearch=True:C214)
		...
		End if 
	Else 
		Sel_S(->[JOB_STAGES:47]; ->[JOB_STAGES:47]Invoice_Number:10; vCompCode)
		Sel_S(->[JOB_STAGES:47]; ->[JOB_STAGES:47]Job_Code:1; vJobCode)
		Sel_S(->[JOB_STAGES:47]; ->[JOB_STAGES:47]Stage_Code:2; vStage)
		Sel_SIS(->[JOB_STAGES:47])
		Sel_SSStrSin(->[JOB_STAGES:47]; ->[JOB_STAGES:47]Stage_Name:3; vContName)
		Big_One(->[JOB_STAGES:47])
	End if 
End while 
Sel_Ending(->[JOB_STAGES:47])
If ($_bo_OpenWindow)
	CLOSE WINDOW:C154
...
End if 
ERR_MethodTrackerReturn("JobStages_SelP"; $_t_oldMethodName)