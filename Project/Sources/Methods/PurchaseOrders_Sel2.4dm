//%attributes = {}
If (False:C215)
	//Project Method Name:      PurchaseOrders_Sel2
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created:   15/06/2010 11:05
...
$_t_oldMethodName:=ERR_MethodTracker("PurchaseOrders_Sel2")
$_t_Search:=$1
vSelPrev:=""
...
Else 
	...
	Else 
		...
		Else 
			If ($_t_Search="Order: @") | ($_t_Search="Sales Order No: @")
				...
				Join_SelFast(->[ORDER_ITEMS:25]; ->[ORDER_ITEMS:25]Purchase_Order_Number:18; ->[PURCHASE_ORDERS:57]; ->[PURCHASE_ORDERS:57]Purchase_Order_Number:2)
	...
	End if 
End if 
ERR_MethodTrackerReturn("PurchaseOrders_Sel2"; $_t_oldMethodName)
