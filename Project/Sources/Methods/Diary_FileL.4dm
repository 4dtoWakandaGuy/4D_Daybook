	...
	Case of 
		...
		Else 
			...
			If ($_t_DocumentWriteType="WS")
				$_t_DocumentWriteType:="W"
			End if 
			FORM SET INPUT:C55([DIARY:12]; "Diary_inL"+$_t_DocumentWriteType)  //NG April 2004 removed reference to ◊screen
			WIN_t_CurrentInputForm:="Diary_inL"+$_t_DocumentWriteType  //NG April 2004 removed reference to ◊screen
	End case 
Else 
	$_t_DocumentWriteType:=Documents_4DWriteAccessType(True:C214)
	If ($_t_DocumentWriteType="WS")
...
If ($_t_DocumentWriteType#"")
	FORM SET OUTPUT:C54([DIARY:12]; "Diary_OutL")
	WIN_t_CurrentOutputform:="Diary_OutL"
	vD:=1
