//%attributes = {}
If (False:C215)
	//Project Method Name:      Jobs_SRMargRep2
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("Jobs_SRMargRep2")
...
vNo:=Records in selection:C76([JOB_STAGES:47])+1+Records in selection:C76([ORDER_ITEMS:25])
Form_SRArray(vNo)
...
While (Not:C34(End selection:C36([JOB_STAGES:47])))
	...
	srdesc{vNo1}:=[JOB_STAGES:47]Stage_Name:3+" Total"
	...
	While (Not:C34(End selection:C36([ORDER_ITEMS:25])))
		srprice{vNo1}:=0
		srqtyt{vNo1}:=""
		srrate{vNo1}:=[ORDER_ITEMS:25]Cost_Amount:15
		sramount{vNo1}:=0
		srdesc{vNo1}:="    "+[ORDER_ITEMS:25]Product_Name:13+" ("+String:C10([ORDER_ITEMS:25]Quantity:3)+")"
		vNo1:=vNo1+1
...
srrate{vNo1}:=Sum:C1([ORDER_ITEMS:25]Cost_Amount:15)
sramount{vNo1}:=0
srdesc{vNo1}:="Unallocated Cost Items Total"
...
While (Not:C34(End selection:C36([ORDER_ITEMS:25])))
	...
	NEXT RECORD:C51([ORDER_ITEMS:25])
End while 
ERR_MethodTrackerReturn("Jobs_SRMargRep2"; $_t_oldMethodName)