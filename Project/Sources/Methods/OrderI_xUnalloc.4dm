//%attributes = {}
If (False:C215)
	//Project Method Name:      OrderI_xUnalloc
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OrderI_xUnalloc")
//OrderI_xUnalloc
		...
		If (Records in selection:C76([CURRENT_STOCK:62])>0)
			...
			Else 
				...
				If ((Records in selection:C76([MOVEMENT_TYPES:60])>0) & ([MOVEMENT_TYPES:60]Stock_Plus:3#""))
					[STOCK_MOVEMENTS:40]Type_Number:4:=MovementTypes_No(6; [MOVEMENT_TYPES:60]Type_Code:1; [MOVEMENT_TYPES:60]Type_Code:1)
...
Else 
	USE NAMED SELECTION:C332("$CurentStock")
End if 
ERR_MethodTrackerReturn("OrderI_xUnalloc"; $_t_oldMethodName)
