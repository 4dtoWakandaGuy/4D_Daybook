...
If (False:C215)
	//Database Method Name:      DB_LoadJobBillingPreferences
	//------------------ Method Notes ------------------
	//This method moves the Job Billing Preferences to the preferences records.
...
If (True:C214)
	...
	C_LONGINT:C283(<>PER_l_CurLoggedinDataOwnerID; $_l_LedgersData; $_l_offset; $_l_ProcessID; $3; DB_l_CurrentOwnerRequest; DM_l_NomAgingStage1; DM_l_NomAgingStage2; DM_l_NomAgingStage3)
...
//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("DB_LoadJobBillingPreferences")
...
If (Not:C34($_bo_FromTransaction))
	While (Semaphore:C143("LockJCPreferences"))
		DelayTicks(2)
	End while 
End if 
...
If (Not:C34($_bo_SavePreferences))  //note to save they must be loaded!!
	...
	If (Records in selection:C76([PREFERENCES:116])=0)
		If (In transaction:C397)
			...
			If (Application type:C494=4D Server:K5:6)
				If (DB_l_CurrentOwnerRequest>0)
					$_l_ProcessID:=New process:C317("DB_LoadLedgerPreferences"; DB_ProcessMemoryinit(4); "Load Ledger Preferences"; False:C215; True:C214; DB_l_CurrentOwnerRequest)
				Else 
					$_l_ProcessID:=New process:C317("DB_LoadLedgerPreferences"; DB_ProcessMemoryinit(4); "Load Ledger Preferences"; False:C215; True:C214)
				End if 
			Else 
				$_l_ProcessID:=New process:C317("DB_LoadLedgerPreferences"; DB_ProcessMemoryinit(4); "Load Ledger Preferences"; False:C215; True:C214)
			End if 
			
			While (Process state:C330($_l_ProcessID)>=0)
				DelayTicks(5)
			End while 
		Else 
...
Else 
	If ($_bo_SavePreferences)
		If (In transaction:C397)
			...
			If (Application type:C494=4D Server:K5:6)
				If (DB_l_CurrentOwnerRequest>0)
					$_l_ProcessID:=New process:C317("DB_LoadLedgerPreferences"; DB_ProcessMemoryinit(4); "Save Ledger Preferences"; True:C214; True:C214; DB_l_CurrentOwnerRequest)
				Else 
					$_l_ProcessID:=New process:C317("DB_LoadLedgerPreferences"; DB_ProcessMemoryinit(4); "Save Ledger Preferences"; True:C214; True:C214)
				End if 
			Else 
				$_l_ProcessID:=New process:C317("DB_LoadLedgerPreferences"; DB_ProcessMemoryinit(4); "Save Ledger Preferences"; True:C214; True:C214)
			End if 
...
End if 
If (Not:C34($_bo_FromTransaction))
	CLEAR SEMAPHORE:C144("LockJCPreferences")
End if 
ERR_MethodTrackerReturn("DB_LoadJobBillingPreferences"; $_t_oldMethodName)