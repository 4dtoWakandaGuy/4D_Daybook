//%attributes = {}
If (False:C215)
	//Project Method Name:      CCM_EditProviders
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>Mod_l_MaxModules; $_l_FormDefaultHeight; $_l_FormDefaultWidth; CCM_l_authReadWrite; MOD_l_CurrentModuleAccess)
	C_TEXT:C284($_t_oldMethodName; WIN_t_CurrentInputForm; WIN_t_CurrentOutputform)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("CCM_EditProviders")
...
If (User in group:C338(Current user:C182; "Administrator")) | (User in group:C338(Current user:C182; "Designer"))
	...
	Else 
		CCM_l_authReadWrite:=0
		If (MOD_l_CurrentModuleAccess#3)
			CCM_l_authReadWrite:=1
		End if 
		
		FORM GET PROPERTIES:C674([USER:15]; "CCM_Prefs"; $_l_FormDefaultWidth; $_l_FormDefaultHeight)
		WIN_t_CurrentOutputform:="ACC_PeriodControl"
		WIN_t_CurrentInputForm:="ACC_PeriodControl"
		Open_Pro_Window("Credit Card Auth Settings"; 0; 1; ->[USER:15]; "CCM_Prefs"; "CCM_ClosePrefs")
		
		//StartTransaction
		DIALOG:C40([USER:15]; "CCM_Prefs")
		Close_ProWin
		If ($_bo_StartProcess)
			Process_End
		End if 
...
Else 
	Gen_Alert("You do not have access to these settings!")
	
	
End if 
ERR_MethodTrackerReturn("CCM_EditProviders"; $_t_oldMethodName)