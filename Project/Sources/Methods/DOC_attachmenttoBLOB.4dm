//%attributes = {}
If (False:C215)
	//Project Method Name:      DOC_attachmenttoBLOB
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>DOC_ServerComm; $_l_CurrentRow; $_l_FolderIndex; $_l_Length; $_l_Level; $_l_Tick; $_l_VolumeRow)
	...
	C_TEXT:C284($_t_SubString; $1; $2; Email_Attachment_Creator; Email_Attachment_Extension; Email_Attachment_Type; Email_AttachmentPath)
	C_TIME:C306(Email_ti_DocCreatedtime; EMail_ti_DocModifiedTime)
End if 

//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("DOC_attachmenttoBLOB")
...
While (Semaphore:C143("Lifting_Document_On_Server"))
	//set this on the server...(the client will wait)
	DelayTicks(2)
End while 
<>DOC_ServerComm:=1
If (Application type:C494=4D Server:K5:6)
	...
	If (Count parameters:C259>=1)
		If ($1#"")
			$_t_oldMethodName2:=ERR_MethodTracker("DS_attachmenttoBLOB:3")
			$_t_DocumentPath:=$1
				...
				If (Count parameters:C259>=2)
					...
					Repeat 
						If (Length:C16($_t_Filename)>=$_l_Length)
							$_t_SubString:=Substring:C12($_t_Filename; 1; $_l_Length)
							If ($_t_SubPath[[Length:C16($_t_SubPath)]]=Folder separator:K24:12)
								$_t_SubPath:=$_t_SubPath+$_t_SubString
							Else 
					...
					If ($_bo_Exit) & (DOC_bo_EmailGotAttachment=False:C215)
						...
						End if 
						$_t_oldMethodName2:=ERR_MethodTracker("DS_attachmenttoBLOB:15")
						If (Test path name:C476($_t_PrimaryPath)=Is a folder:K24:2)
							...
							For ($_l_FolderIndex; 1; Size of array:C274($_at_FolderList))
								If ($_t_PrimaryPath[[Length:C16($_t_PrimaryPath)]]=Folder separator:K24:12)
									$_t_SubPath:=$_t_PrimaryPath+$_at_FolderList{$_l_FolderIndex}
								Else 
									$_t_SubPath:=$_t_PrimaryPath+Folder separator:K24:12+$_at_FolderList{$_l_FolderIndex}
								End if 
								$_t_NewPathName:=DS_CheckNameWithExtension($_t_SubPath+Folder separator:K24:12+$_t_Filename; $_t_Extension; ->DS_at_ValidExtensions)
								If (Test path name:C476($_t_NewPathName)=Is a document:K24:1)
									DOCUMENT TO BLOB:C525($_t_NewPathName; vEmail_Attachment_BLOB)
...
ERR_MethodTrackerReturn("DOC_attachmenttoBLOB"; $_t_oldMethodName)