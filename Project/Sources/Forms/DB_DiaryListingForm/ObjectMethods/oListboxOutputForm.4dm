If (False:C215)
	//object Method Name: Object Name:      DB_DiaryListingForm.oListboxOutputForm
	//------------------ Method Notes ------------------
...
If (True:C214)
	ARRAY BOOLEAN:C223($_abo_ColsVisible; 0)
	...
	C_BOOLEAN:C305(<>FS_bo_FSettingsLoaded; $_bo_OptionKey; $_bo_ProcessVisible; $_bo_RelationFound; DB_bo_AutorelationsTemp; DB_bo_DisplaySelections; DB_bo_MouseActive; SEL_SelectionsShare)
	C_LONGINT:C283($_l_ArraySize; $_l_ColumnNum; $_l_ColumnWidth; $_l_CurrentSoftSetting; $_l_CurrentSortColumn; $_l_Event; $_l_FieldNumber; $_l_FormSettingsRow; $_l_FSSortDirection; $_l_IgnoreColumns; $_l_Index)
	C_LONGINT:C283($_l_Index2; $_l_LayoutColumnNumber; $_l_ObjectBottom; $_l_ObjectLeft; $_l_ObjectRight; $_l_objectTop; $_l_OneTable; $_l_ProcessState; $_l_ProcessTime; $_l_RelationsIndex; $_l_Retries)
	C_LONGINT:C283($_l_SelectedColumn; $_l_SelectedRow; $_l_SortDirection; $_l_SortFieldNumber; $_l_SourceProcess; $_l_SourceRow; $_l_TableNumber; $0; $oneField; DB_l_CurrentDisplayedForm; DB_l_LBOutputForm)
	C_LONGINT:C283(FS_l_SortDirection; LB_l_CurrentSortColumn; LB_l_LastRow; SEL_configselection)
	C_PICTURE:C286(DB_pic_SelectedSet)
	C_POINTER:C301($_ptr_ArrayFieldPointers; $_ptr_ArrayReferences; $_Ptr_ColumnHeaderVar; $_ptr_ColumnWIdths; $_ptr_Field; $_ptr_Formulas; $_ptr_Listbox; $_ptr_ListboxArea; $_ptr_ListboxSetup; $_ptr_SourceObject; $_ptr_Table)
	C_REAL:C285(DB_l_newSetClass)
	C_TEXT:C284(<>PER_t_CurrentUserInitials; $_t_CurrentDefinitionsName; $_t_CurrentOutputform; $_t_DocumentPath; $_t_DocumentType; $_t_oldMethodName; $_t_processName; $_t_VariableName; $LAY_t_CurrentUserDefs; DB_t_CurrentContext; SM_t_loadedsetName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ DB_DiaryListingForm.oListboxOutputForm"; Form event code:C388)
...
Case of 
	...
	: ($_l_Event=On Mouse Enter:K2:33)  //Column sort
	...
	: ($_l_Event=On Header Click:K2:40)  //Column sort
		...
		DB_bo_MouseActive:=False:C215
		$_l_FormSettingsRow:=Find in array:C230(DB_al_ContextTableNumber; DB_l_CurrentDisplayedForm)
		If ($_l_FormSettingsRow>0)  //it shoud be!
			$_ptr_ListboxSetup:=DB_aptr_ContextPointers{$_l_FormSettingsRow}
			LB_l_LastRow:=0
			...
			$_l_ColumnNum:=$_l_CurrentSortColumn
			...
			LISTBOX GET ARRAYS:C832($_ptr_Listbox->; $_at_ColumnNames; $_at_HeaderNames; $_aptr_ColumnVariables; $_aptr_HeaderVariables; $_abo_ColsVisible; $_aptr_ColumnStyles)
			...
			End while 
			$_l_FormSettingsRow:=Find in array:C230(<>FS_at_FormReference; $_t_CurrentDefinitionsName)
			$_Ptr_ColumnHeaderVar:=$_aptr_HeaderVariables{$_l_CurrentSortColumn}
			
			$_l_CurrentSoftSetting:=$_Ptr_ColumnHeaderVar->
			$_l_FSSortDirection:=FS_l_SortDirection
			Case of 
					
				: ($_l_CurrentSoftSetting>=2)
			...
			If ($_l_FormSettingsRow>0)
				
				$_l_SortDirection:=$_l_FSSortDirection
				
				If ($_l_SortFieldNumber=<>FS_al_FormSortFieldNum{$_l_FormSettingsRow}) | ((-Abs:C99($_l_ColumnNum))=<>FS_al_FormSortFieldNum{$_l_FormSettingsRow})
			...
			If ($_l_TableNumber#DB_l_CurrentDisplayedForm)
				...
				For ($_l_RelationsIndex; 1; Get last field number:C255(DB_l_CurrentDisplayedForm))
					If (Is field number valid:C1000(DB_l_CurrentDisplayedForm; $_l_RelationsIndex))
						GET RELATION PROPERTIES:C686(DB_l_CurrentDisplayedForm; $_l_RelationsIndex; $_l_OneTable; $oneField)
						If ($_l_OneTable=$_l_TableNumber)
							//TRACE
							$_ptr_Field:=Field:C253(DB_l_CurrentDisplayedForm; $_l_RelationsIndex)
							SET FIELD RELATION:C919($_ptr_Field->; Automatic:K51:4; Manual:K51:3)
							$_bo_RelationFound:=True:C214
							$_l_RelationsIndex:=99999
						End if 
					End if 
				End for 
			...
			If ($_l_SortFieldNumber>0) & ($_l_TableNumber>0)
				...
				Else 
					If (Type:C295(Field:C253($_l_TableNumber; $_l_SortFieldNumber)->)=7)
						FSetting_UpdateArray($_t_CurrentDefinitionsName; $_l_SortDirection; -Abs:C99($_l_ColumnNum); $_l_TableNumber; ""; "")
					End if 
	...
	: ($_l_Event=On Column Resize:K2:31)
		...
		LISTBOX GET ARRAYS:C832(DB_l_LBOutputForm; $_at_ColumnNames; $_at_HeaderNames; $_aptr_ColumnVariables; $_aptr_HeaderVariables; $_abo_ColsVisible; $_aptr_ColumnStyles)
		$_l_FormSettingsRow:=Find in array:C230(DB_al_ContextTableNumber; DB_l_CurrentDisplayedForm)
		If ($_l_FormSettingsRow>0)  //it shoud be!
			...
			$_ptr_Field:=$_ptr_ListboxSetup->{2}
			$_ptr_Formulas:=$_ptr_ListboxSetup->{6}
			$_l_ArraySize:=Size of array:C274($_ptr_ColumnWIdths->)
			...
			For ($_l_Index; 1; Size of array:C274($_ptr_ColumnWIdths->))
				If (Position:C15("LB_GetRelation"; $_ptr_Formulas->{$_l_Index})=0) & ($_ptr_ColumnWIdths->{$_l_Index}#0)
					$_l_IgnoreColumns:=0
					$_l_SelectedColumn:=$_l_Index
					For ($_l_Index2; 1; $_l_SelectedColumn)
						If (Position:C15("LB_GetRelation"; $_ptr_Formulas->{$_l_Index2})#0)
							$_l_IgnoreColumns:=$_l_IgnoreColumns+1
						End if 
						If ($_ptr_ColumnWIdths->{$_l_Index2}=0)  // Zero width columns are invisible so dont get counted
							//$_l_IgnoreColumns:=$_l_IgnoreColumns-1
						End if 
					End for 
					$_l_ColumnWidth:=LISTBOX Get column width:C834(*; $_at_ColumnNames{$_l_SelectedColumn})
					$_l_LayoutColumnNumber:=$_l_SelectedColumn-$_l_IgnoreColumns
					USE NAMED SELECTION:C332("$TempSelection")
					QUERY SELECTION:C341([LIST_LAYOUTS:96]; [LIST_LAYOUTS:96]Order:7=$_l_LayoutColumnNumber)
					If ([LIST_LAYOUTS:96]Width:5#$_ptr_ColumnWIdths->{$_l_SelectedColumn})
						[LIST_LAYOUTS:96]Width:5:=$_ptr_ColumnWIdths->{$_l_SelectedColumn}
							...
							If (OK=1)
								If (DB_bo_DisplaySelections)
									...
									If (OK=0)
										...
										$_l_ColumnWidth:=$_l_ObjectRight-$_l_ObjectLeft
										LB_SetColumnWidths(->DB_lb_Setsandselections; "SMGR_t"; 1; 30; $_l_ColumnWidth-(30+20); 0; 0; 0)
...
End case 
ERR_MethodTrackerReturn("OBJ DB_DiaryListingForm.oListboxOutputForm"; $_t_oldMethodName)
