If (False:C215)
	//object Name: Object Name:      WEB_Vingettes.oButtonSave
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 03/03/2012 14:01
...
If (True:C214)
	...
	C_LONGINT:C283($_l_DocumentRow; DBSWS_l_vinWinBottom; DBSWS_l_vinWinLeft; DBSWS_l_vinWinRight; DBSWS_l_vinWinTop; DBWS_l_DocumentClass)
	C_TEXT:C284($_t_oldMethodName; DBWS_t_CurrentLoadedDocument; DBWS_t_DocumentText; DBWS_t_DocumentTitle)
End if 
//Code Starts Here
$_t_oldMethodName:=ERR_MethodTracker("OBJ [projectForm]/WEB_Vingettes/oButtonSave"; Form event code:C388)
If (DBWS_t_DocumentTitle#"") & (DBWS_t_DocumentText#"")
	...
	If (DBWS_t_CurrentLoadedDocument="")
		...
		If (DBWS_l_DocumentClass=900) | (DBWS_l_DocumentClass=-901)
			APPEND TO ARRAY:C911(DBWS_at_DocumentTypes; "Text with no markup")
		Else 
			APPEND TO ARRAY:C911(DBWS_at_DocumentTypes; "HTML Marked up text")
		End if 
...
Else 
	Gen_Alert("You must provide the text and an asset name")
	
End if 
ERR_MethodTrackerReturn("OBJ WEB_Vingettes.oButtonSave"; $_t_oldMethodName)
