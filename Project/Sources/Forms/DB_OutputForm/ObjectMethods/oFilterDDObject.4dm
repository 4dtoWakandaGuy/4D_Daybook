If (False:C215)
	//object Method Name: Object Name:      DB_OutputForm.oFilterDDObject
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_FiltersActive; DB_bo_DisplayFilters; DB_bo_FiltersActivated; DB_bo_FiltersTouched; DB_bo_ResetFilters; LAY_bo_AnalSetup; LAY_bo_AreasSetup; LAY_bo_BCAnalSetup; LAY_bo_BCLayerSetup; LAY_bo_CONsourcesSetup; LAY_bo_CONStatusSetup)
	C_BOOLEAN:C305(LAY_bo_ContractTypesSetup; LAY_bo_ConTypeSetup; LAY_bo_COsourcesSetup; LAY_bo_COStatusSetup; LAY_bo_CoTypesSetup; LAY_bo_CountiesSetup; LAY_bo_CountriesReady; LAY_bo_CountriesSetup; LAY_bo_CreditStatesSetup; LAY_bo_CSAnalSetup; LAY_bo_CSLayerSetup)
	C_BOOLEAN:C305(LAY_bo_CurrenciesSetup; LAY_bo_FiltersActive; LAY_bo_HeadingsSetup; LAY_bo_INVAnalSetup; LAY_bo_INVLayerSetup; LAY_bo_JBAnalSetup; LAY_bo_LayerSetup; LAY_bo_MoveTypesSetup; LAY_bo_ORDsourcesSetup; LAY_bo_OrdStatesSetup; LAY_bo_OrdTypeSetup)
	C_BOOLEAN:C305(LAY_bo_pBrandsSetup; LAY_bo_pCollectionsSetup; LAY_bo_pGroupsSetup; LAY_bo_PIAnalSetup; LAY_bo_PILayerSetup; LAY_bo_POAnalSetup; LAY_bo_POLayerSetup; LAY_bo_POStatesSetup; LAY_bo_RolesSetup; LAY_bo_SalesPersonSetup; LAY_bo_SLAnalSetup)
	C_BOOLEAN:C305(LAY_bo_SLLayerSetup; LAY_bo_SMAnalSetup; LAY_bo_SMLayerSetup; LAY_bo_StockLocationsSetup; LAY_bo_StockTypesSetup; LAY_bo_TRAnalSetup; LAY_bo_TRLayerSetup; Lay_bo_UpdateFilters; SI_bo_FilterZeros)
	C_LONGINT:C283(<>PER_l_CurrentUserID; $_l_event; $_l_FilterID; $_l_FilterRow; $_l_RemoveFilterRow; $_l_RemoveRow; $_l_ResaveRow; $_l_TableNumber; ACC_l_BalanceSheet; ACC_l_Both; ACC_l_ContextualFilterFrom)
	C_LONGINT:C283(ACC_l_ContextualFilterTo; ACC_l_ProfitSheet; CO_l_Both; CO_l_Company; CO_l_PrivateAddress; DB_l_CurrentDisplayedForm; DB_l_SelectedFilterRef; INV_l_Both; INV_l_Closed; INV_l_Open)
	C_TEXT:C284($_t_FilterName; $_t_oldMethodName; $_t_SearchName; DB_T_CurrentFilterName)
End if 
//Code Starts Here
			...
			Case of 
				: (DB_at_FilterNames{DB_at_FilterNames}=Get localized string:C991("Save_Filter"))
					...
					If ($_t_SearchName#"")
						...
						DB_SaveFilterSettings(0; $_t_SearchName; $_l_TableNumber; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
						DB_LoadFilters($_l_TableNumber)
						$_l_FilterRow:=Find in array:C230(DB_at_FilterNames; $_t_SearchName)
						If ($_l_FilterRow>0)
							$_l_ResaveRow:=Find in array:C230(DB_at_FilterNames; Get localized string:C991("Resave filter"))
							If ($_l_ResaveRow<0)
								$_l_RemoveRow:=Find in array:C230(DB_at_FilterNames; "Remove Filters")
								If ($_l_RemoveRow>0)
									DELETE FROM ARRAY:C228(DB_at_FilterNames; $_l_RemoveRow)
									DELETE FROM ARRAY:C228(DB_al_FilterIDs; $_l_RemoveRow)
								End if 
								...
								//$Ref:=Find in array(DB_at_FilterNames;Get localized string("Resave filter"))
								//DB_al_FilterIDs{$Ref}:=Abs(DB_al_FilterIDs{$Ref3})
				...
				: (DB_al_FilterIDs{DB_at_FilterNames}<0)  //Load a filter
					...
					$_l_ResaveRow:=Find in array:C230(DB_at_FilterNames; Get localized string:C991("Resave filter"))
					If ($_l_ResaveRow<0)
						$_l_RemoveFilterRow:=Find in array:C230(DB_at_FilterNames; "Remove Filters")
						If ($_l_RemoveFilterRow>0)
							DELETE FROM ARRAY:C228(DB_at_FilterNames; $_l_RemoveFilterRow)
							DELETE FROM ARRAY:C228(DB_al_FilterIDs; $_l_RemoveFilterRow)
					...
					End if 
					//DB_al_FilterIDs{$Ref}:=Abs(DB_al_FilterIDs{DB_at_FilterNames})
				...
				: (DB_al_FilterIDs{DB_at_FilterNames}=0)  //Resave
					...
					SET BLOB SIZE:C606(SM_blb_SearchBlob; 0)
					$_l_FilterID:=DB_al_FilterIDs{0}
					
					
					$_l_FilterRow:=Find in array:C230(DB_al_FilterIDs; $_l_FilterID)
					If ($_l_FilterRow>0)
						DB_SaveFilterSettings($_l_FilterID; ""; $_l_TableNumber; <>PER_l_CurrentUserID; ->SM_blb_SearchBlob)
