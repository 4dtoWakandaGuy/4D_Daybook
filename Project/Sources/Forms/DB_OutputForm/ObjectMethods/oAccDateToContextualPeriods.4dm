If (False:C215)
	//object Name: Object Name:      DB_OutputForm.oAccDateToContextualPeriods
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ DB_OutputForm.oAccDateToContextualPeriods"; Form event code:C388)
...
Case of 
	: ($_l_event=On Clicked:K2:4)
		...
		If (ACC_at_ContextualPeriodT>0)
			...
			If (Records in selection:C76([PERIODS:33])>0)
				...
				If (($_l_offset+1)<Size of array:C274($_ad_FromDates))
					...
					If (Find in array:C230(ACC_at_PeriodToCode; $_at_PeriodCodes{$_l_offset+1})>0)
						ACC_at_PeriodToCode:=Find in array:C230(ACC_at_PeriodToCode; $_at_PeriodCodes{$_l_offset+1})
					Else 
				...
				$_l_PeriodRow:=Find in array:C230(ACC_at_PeriodFromCode; ACC_at_PeriodFromCode{0})
				If ($_l_PeriodRow>0)
					If (ACC_ad_PeriodFromFromDate{$_l_PeriodRow}>ACC_ad_PeriodToFromDate{ACC_at_PeriodToCode})
						$_l_PeriodRow:=Find in array:C230(ACC_at_PeriodFromCode; ACC_at_PeriodToCode{ACC_at_PeriodToCode})
						If ($_l_PeriodRow<0)
							...
							$_l_PeriodRow:=1
						End if 
						ACC_at_PeriodFromCode:=$_l_PeriodRow
		...
		End if 
End case 
ERR_MethodTrackerReturn("OBJ DB_OutputForm.oAccDateToContextualPeriods"; $_t_oldMethodName)
