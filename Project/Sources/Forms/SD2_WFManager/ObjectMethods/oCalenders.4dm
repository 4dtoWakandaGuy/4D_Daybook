If (False:C215)
	//object Method Name: Object Name:      SD2_WFManager.oCalenders
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283($_l_event; $_l_FieldNumber; $_l_Index; $_l_MultiDiaryLocation; $_l_Retries; $_l_SelectedRow; $_l_SelectionCount; $_l_TableNumber; LB_l_ColumnNumber; SD2_l_NewWindowsForDiaries)
	C_TEXT:C284(<>PER_t_CurrentUserInitials; $_t_oldMethodName; $_t_ProcessName; $_t_VariableName; SD_t_CurrentDiaryInitials)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ SD2_WFManager.oCalenders"; Form event code:C388)
			...
			: (LB_l_ColumnNumber=2)  //Double clicked on column 2
				If ($_l_SelectedRow>0)
					If (SD_at_PersonInitials{$_l_SelectedRow}#"")  //Double clicked on a name
						...
						$_t_ProcessName:="Schedule"+"-"+SD_at_DiaryOwners{$_l_SelectedRow}
						ZProc("Diary_Mod_SD"; DB_ProcessMemoryinit(6); $_t_ProcessName; SD_at_PersonInitials{$_l_SelectedRow})
					End if 
				End if 
		End case 
			...
			: (LB_l_ColumnNumber=1)
				If ($_l_SelectedRow>0)
					If (SD_at_PersonInitials{$_l_SelectedRow}#"")
						$_l_MultiDiaryLocation:=Find in array:C230(SD_at_DiaryOwners; "Multiple Diaries")
						If (SD2_abo_CheckBox{$_l_MultiDiaryLocation})  //Combine mulitple diaries
							$_l_Retries:=0
						...
						Else   //Select a single diary
							...
							Else 
								...
								Else 
									...
									$_t_ProcessName:="View SuperDiary"+"-"+SD_at_DiaryOwners{$_l_SelectedRow}
									ZProc("Diary_Mod_SD"; DB_ProcessMemoryinit(6); $_t_ProcessName; SD_at_PersonInitials{$_l_SelectedRow})
...
End case 
ERR_MethodTrackerReturn("OBJ SD2_WFManager.oCalenders"; $_t_oldMethodName)
