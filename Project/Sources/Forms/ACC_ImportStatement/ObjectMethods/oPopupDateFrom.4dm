If (False:C215)
	//object Name: Object Name:      ACC_ImportStatement.oPopupDate
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 14/07/2012 11:27
...
If (True:C214)
	...
	C_DATE:C307(ACC_D_TransactsFrom; ACC_D_TransactsFromPick; ACC_D_TransactsToPick; Vdate)
	C_TEXT:C284($_t_FormName; $_t_ObjectName; $_t_oldMethodName)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("OBJ ACC_ImportStatement.oPopupDate"; Form event code:C388)
//PopupDate sample code

Case of 
	: (Form event code:C388=On Clicked:K2:4)
		
		Vdate:=ACC_D_TransactsFrom
	: (Form event code:C388=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		If ($_bo_Customise) & (False:C215)
			ARRAY BOOLEAN:C223($_abo_WeekDaysOff; 7)  //lets say wednesday and sunday are days off
			
			//$_abo_WeekDaysOff{Wednesday}:=True
			//$_abo_WeekDaysOff{Sunday}:=True
		...
		If ($_bo_Customise)
			//DatePicker SET MIN DATE ($_t_FormName;Add to date(Current date;0;0;-100))  //min date is current date minus 15 days
			DatePicker SET MAX DATE($_t_FormName; Add to date:C393(Current date:C33; 1; 0; 0))  //max date is current date plus 1 year
		...
		If ($_bo_Customise)
			DatePicker SET WEEK FIRST DAY($_t_FormName; Sunday:K10:19)  // lets start the week on wednesday (why not)
			
		End if 
		
End case 
ERR_MethodTrackerReturn("OBJ ACC_ImportStatement.oPopupDate"; $_t_oldMethodName)
