...
If (False:C215)
	//Project Method Name:      StockLev_SelP
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_AndLoaded; $_bo_NoSearch; $_bo_OpenWindow; $_bo_SessionWindowOwner; lSTK_bo_FFLoaded; SM_bo_SessionWindow; WIN_bo_TrackerInited)
	...
	C_LONGINT:C283(cSIS; SM_al_NextElement; SM_l_QueryMode; SM_l_QuerySearchID; SM_l_ReturnQueryID; SM_l_SearchNumber; SRCH_l_And; SRCH_l_Not; SRCH_l_or; VItemNum; vPu)
...
$_t_oldMethodName:=ERR_MethodTracker("StockLev_SelP")
//StockLev_SelP
$_bo_OpenWindow:=False:C215

Sel_Beginning(False:C215; ->[STOCK_LEVELS:58])
$_l_CurrentRow:=1
$_l_QueryID:=0
$_l_Mode:=0
...
While ((vPu=1) & ((SRCH_l_And=1) | (SRCH_l_or=1) | (SRCH_l_Not=1))) | ($_l_CurrentRow<=$_l_ElementCount)
	...
	If ($_l_Mode=0)
		If ($_l_WhilesCounter=1)
			Sel_Beginning2(False:C215; ->[STOCK_LEVELS:58])
		End if 
	Else 
		Sel_Beginning2(False:C215; ->[STOCK_LEVELS:58])
	End if 
	...
	If ($_l_QueryID=0)
		...
		If (Not:C34(SM_bo_SessionWindow))
			...
			Open_AnyTypeWindow(272; 425; 4; Term_StoWT("Find Stock Levels"))
		...
		Else 
			...
			If ($_bo_SessionWindowOwner)
				$_bo_OpenWindow:=True:C214
			End if 
			SET WINDOW TITLE:C213(Term_StoWT("Find Stock Levels"))
		End if 
		DIALOG:C40([STOCK_LEVELS:58]; "dStockLev_Sel")
	...
	Else 
		...
		If ($_l_QueryID<0)
			...
			$_l_offset:=0
			SM_l_ReturnQueryID:=0
			If (Not:C34(lSTK_bo_FFLoaded))
				SM_SetUpFFArray(->[STOCK_LEVELS:58])
				lSTK_bo_FFLoaded:=True:C214
			End if 
		...
		If ([x_QUERY_PARAMETERS:134]X_ContextID:3=$_l_TableNumber)
			...
			If ($_l_Mode=2)  // edit
				...
				If (SM_l_QueryMode=2) & (OK=1)
					...
					SET BLOB SIZE:C606(SM_blb_SearchBlob; 0)
					If (SM_l_ReturnQueryID=0)  // not a linked query
						SM_SearchVarsToBlob(->SM_blb_SearchBlob; ->VProdCode; ->VType; ->Vanalysis; ->ACC_t_CurrencyCode; ->Vlayer; ->VamountF; ->VamountT; ->vmarginF; ->vmarginT; ->cSIS; ->cNAA; ->cNAL)
						SM_SearchVarsToBlob(->SM_blb_SearchBlob; ->SRCH_l_And; ->SRCH_l_or; ->SRCH_l_Not)
	...
	Else 
		Sel_S(->[STOCK_LEVELS:58]; ->[STOCK_LEVELS:58]Product_Code:1; vProdCode)
		Sel_S(->[STOCK_LEVELS:58]; ->[STOCK_LEVELS:58]Stock_Type:2; vType)
		Sel_S(->[STOCK_LEVELS:58]; ->[STOCK_LEVELS:58]Analysis_Code:7; vAnalysis)
		Sel_SCurr(->[STOCK_LEVELS:58]; ->[STOCK_LEVELS:58]Currency_Code:8; ACC_t_CurrencyCode)
		Sel_SLay(->[STOCK_LEVELS:58]; ->[STOCK_LEVELS:58]Layer_Code:9; vLayer)
		Sel_SIS(->[STOCK_LEVELS:58])
		Sel_SSNum(->[STOCK_LEVELS:58]; ->[STOCK_LEVELS:58]Free_Level:3; vAmountF; vAmountT)
		Sel_SSNum(->[STOCK_LEVELS:58]; ->[STOCK_LEVELS:58]Allocated_Level:4; vMarginF; vMarginT)
		Master_UseSS(->[STOCK_LEVELS:58])
		...
		Master2(->[STOCK_LEVELS:58])
		Big_One(->[STOCK_LEVELS:58])
	End if 
End while 
Sel_Ending(->[STOCK_LEVELS:58])
If ($_bo_OpenWindow)
	CLOSE WINDOW:C154
...
End if 
ERR_MethodTrackerReturn("StockLev_SelP"; $_t_oldMethodName)
