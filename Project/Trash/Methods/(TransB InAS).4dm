...
If (False:C215)
	//Project Method Name:      TransB InAS
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("TransB InAS")
Menu_Record("TransB InAS"; "Switch Access")
	...
	If (vAccessSwNo<4)
		If (In_ButtChkMan(->[TRANSACTION_BATCHES:30]; "1"))
			...
			If ((Locked:C147([TRANSACTION_BATCHES:30])) | (Read only state:C362([TRANSACTION_BATCHES:30])))
				DIALOG:C40([TRANSACTION_BATCHES:30]; "TransB In13")
			Else 
...
End if 
ERR_MethodTrackerReturn("TransB InAS"; $_t_oldMethodName)