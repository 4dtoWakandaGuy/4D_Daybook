...
If (True:C214)
	C_BOOLEAN:C305($_bo_OnScreenAlert; $2)
	C_LONGINT:C283($_l_OK)
	C_POINTER:C301($1; $3)
	C_TEXT:C284($_t_CompanyCode; $_t_ContactCode; $_t_oldMethodName; $_t_OrderCode)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Orders_InLPEvCh")
	...
	If ($_bo_OnScreenAlert)
		...
		Else 
			...
			If (($_t_OrderCode#"") & (Character code:C91(Substring:C12($_t_OrderCode; Length:C16($_t_OrderCode); 1))#64))
				QUERY:C277([ORDERS:24]; [ORDERS:24]Order_Code:3=$_t_OrderCode)
				$_l_OK:=DB_DeletionControl(->[ORDERS:24]; "Orders_InLPecCh")
				If ($_l_OK=1)
...
Else 
	USE NAMED SELECTION:C332("$Sel")
End if 
ERR_MethodTrackerReturn("Orders_InLPEvCh"; $_t_oldMethodName)
