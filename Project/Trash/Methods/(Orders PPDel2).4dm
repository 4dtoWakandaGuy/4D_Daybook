...
If (False:C215)
	//Project Method Name:      Orders PPDel2
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305($_bo_UseISO; $2; SR_bo_Validated; STK_bo_AllocTypesKnown; STK_bo_Create; STK_T_PrintExisting)
	C_DATE:C307(<>DB_d_CurrentDate; $_d_DeliveryDateOLD; $_d_NewDeliveryDate; STK_d_DeliveryDate; vDelDate; vDoDateF)
	...
	C_LONGINT:C283($_l_OrderItemRow; $_l_OrderItemsIndex; $_l_PartDeliveryIndex; $_l_Printed; $_l_ReprintRow; $_l_TableRow; ORD_l_CB_Delivery; vAdd; xNext)
	C_POINTER:C301($_ptr_HighlightSet; $_ptr_ListBoxSetup; BUT_p_ButtonStringPointer)
	C_REAL:C285($_r_Allocated; $_r_AlreadySelected; $_r_Available; $_r_Sum2; $_r_ToAllocate)
	...
	C_TEXT:C284($3; DB_t_CurrentFormUsage; DB_t_CurrentFormUsage2; DB_t_CurrentFormUsage3; STK_T_DeliveryOrder; STK_T_DeliveryReference)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Orders PPDel2"; Form event code:C388)
											...
											If (Size of array:C274($_at_DelANDAllocateMovementTypes)=0)
												...
												Else 
													...
													If (OK=1)
														$_bo_Continue:=False:C215
													Else 
														//Take the non allocated items out of the delivery
														For ($_l_AllocateIndex; 1; Size of array:C274($_al_NonAllocatedItems))
															If (OK=0)
																$_bo_Continue:=False:C215
																$_l_AllocateIndex:=999999
													...
													Else 
														...
														If (OK=1)
															If ($_d_NewDeliveryDate#!00-00-00!)
																$_bo_Continue:=True:C214
																STK_T_PrintExisting:=False:C215
																STK_T_DeliveryReference:=""
																STK_T_DeliveryOrder:=""
												...
												For ($_l_AllocationIndex; 1; Size of array:C274($_at_OrderCodes))
													...
													If ($_bo_Print)
														If (OK=1) & ($_bo_Selected)
															...
															If (Not:C34(STK_T_PrintExisting))
															...
															If (OK=1)
																If (Transact_SttLck(->[ORDER_ITEMS:25]Item_Number:27))
																	Gen_PPMulti(->[ORDERS:24]; "Delivery Note"; "Orders Out"; "Delivery"; "Delivery Note"; "Orders_PPDelL2"; 0; ->[ORDERS:24]State:15; "Delivery"; [ORDERS:24]Analysis_Code:33)
						...
						If ($_t_SelectionType#"")
							Case of 
								...
								: ($_t_SelectionType="Date")  //-select a date(actually selected a delivery reference on that date if mulitple")
									...
									End if 
									If ($_bo_Continue)
										If (STK_T_PrintExisting=False:C215)
							...
							If ($_bo_Continue)
								...
								Else 
									...
									If (OK=1) & ((STK_T_DeliveryReference#"") | (STK_d_DeliveryDate#!00-00-00!))
										...
										$_l_Printed:=Gen_PPSingle(->[ORDERS:24]; "Delivery Note"; "Orders Out"; "Delivery"; "Delivery Note"; "Orders_PPDelL2"; 0; ->[ORDERS:24]State:15; "Delivery"; [ORDERS:24]Analysis_Code:33; False:C215; $_t_DocumentName)  // 05/04/040 pb
...
End if 
vAdd:=$_l_AddMode2
ERR_MethodTrackerReturn("Orders PPDel2"; $_t_oldMethodName)
