...
If ($_l_TableNumberRow>0)
	If (<>SYS_al_TableUniqueField{$_l_TableNumber}#0)
		...
		If ($_l_CustomFieldRow>0)
			If (Count parameters:C259=3)
				...
				Case of 
					: (Type:C295($_ptr_Field->)=9) | (Type:C295($_ptr_Field->)=1) | (Type:C295($_ptr_Field->)=8)
						$_t_Source:=Str LeadZeros(String:C10($_l_TableNumber); 3)+String:C10($_ptr_Field->)
					: (Type:C295($_ptr_Field->)=4)
						$_t_Source:=Str LeadZeros(String:C10($_l_TableNumber); 3)+String:C10($_ptr_Field->)
					: (Type:C295($_ptr_Field->)=11)
						$_t_Source:=Str LeadZeros(String:C10($_l_TableNumber); 3)+Time string:C180($_ptr_Field->)
					: (Type:C295($_ptr_Field->)=6)
						If ($_ptr_Field->)
							$_t_Source:=Str LeadZeros(String:C10($_l_TableNumber); 3)+"TRUE"
						Else 
							$_t_Source:=Str LeadZeros(String:C10($_l_TableNumber); 3)+"FALSE"
						End if 
						
					: (Type:C295($_ptr_Field->)=0) | (Type:C295($_ptr_Field->)=2)
						$_t_Source:=Str LeadZeros(String:C10(<>DB_al_TableNums{$_l_TableNumberRow}); 3)+$_ptr_Field->
					Else 
			...
			Else 
				$_t_Source:=Str LeadZeros(String:C10($_l_TableNumber); 3)+$4
			End if 
