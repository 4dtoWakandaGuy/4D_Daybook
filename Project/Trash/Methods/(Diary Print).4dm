...
If (False:C215)
	//Project Method Name:      Diary Print
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("Diary Print")
//Diary Print
If ((DB_GetModuleSettingByNUM(Module_WPManager))>1)
	If ([DIARY:12]Document_Code:15#"")
		...
		Else 
			...
			CUT NAMED SELECTION:C334([DIARY:12]; "Diary Print")
			GOTO RECORD:C242([DIARY:12]; $_l_RecordNumber)
			vNo:=1
			<>NoStart:=1
			DBI_MenuDisplayRecords("DiaryMailing")
			
			USE NAMED SELECTION:C332("Diary Print")
			//  POP RECORD([DIARY])
...
End if 
ERR_MethodTrackerReturn("Diary Print"; $_t_oldMethodName)
