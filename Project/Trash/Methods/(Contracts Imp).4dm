...
If (False:C215)
	//Project Method Name:      Contracts Imp
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("Contracts Imp")
//Contracts Import
	...
	If (OK=1)
		...
		If (OK=1)
			...
			If (Records in selection:C76([CONTRACTS:17])>0)
				Gen_Confirm("Do you want to View the Contracts imported?"; "Yes"; "No")
				If (OK=1)
					DB_t_CurrentFormUsage:=""
					vNo:=Records in selection:C76([CONTRACTS:17])
					Open_Pro_Window($_t_WindowTitle; 0; 1; ->[CONTRACTS:17]; "Contracts Out")  //NG may 2004 added form name
					FS_SetFormSort(WIN_t_CurrentOutputform; Table:C252(->[CONTRACTS:17]))
					
					WIn_SetFormSize(1; ->[CONTRACTS:17]; "Contacts Out")
...
Process_End
ERR_MethodTrackerReturn("Contracts Imp"; $_t_oldMethodName)
