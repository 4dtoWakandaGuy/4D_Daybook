...
If (True:C214)
	...
	C_LONGINT:C283(<>SYS_l_CancelProcess; <>SYS_l_LoggedInaccount; $_l_OK; $_l_WindowReferenceRow; ch5; ch6; ch7; ch8; MMLetterClass; vP; vSNo)
	C_LONGINT:C283(WIN_l_CurrentWinRef)
	C_POINTER:C301($1)
	C_REAL:C285(vDiscountF; vSubtotal; vValueF; vValueT; vVAT)
	C_TEXT:C284($_t_oldMethodName; $_t_SSC; vArea; vLocation; vProdCode; vProdName; vTitle1)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("MAsk MS")
...
If (Not:C34((MMLetterClass=4D Write Email) | (MMLetterClass=4D Write Email Template) | (MMLetterClass=Text Email) | (MMLetterClass=Text Email Template)))
	...
	If ((vP=1) & (ch5=1))
		...
		If (Records in table:C83([MAILSORT_B:78])>500)
			...
			DIALOG:C40([DOCUMENTS:7]; "dAsk MS")
			CLOSE WINDOW:C154
			If ($_bo_Unload)
				...
				If (<>SYS_l_CancelProcess#Current process:C322)
					...
					If (OK=1)
						...
						If (OK=1)
							...
							FORM SET OUTPUT:C54([MAILSORT_MAIL:83]; "Line Listing")
...
Else 
	OK:=1
End if 
ERR_MethodTrackerReturn("MAsk MS"; $_t_oldMethodName)
