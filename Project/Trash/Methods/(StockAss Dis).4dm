...
If (False:C215)
	//Project Method Name:      StockAss Dis
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("StockAss Dis")
	...
	If (In_Allowed($_t_MovementTypeName; ->[STOCK_MOVEMENTS:40]))
		...
		If (Records in selection:C76([MOVEMENT_TYPES:60])>0)
			...
			While ((OK=1) & (DB_t_CallOnCloseorSave#"Close"))
				...
				vSNo:=0
				FORM SET INPUT:C55([STOCK_MOVEMENT_ITEMS:27]; "StockAss Dis13")
				WIN_t_CurrentInputForm:="StockAss Dis13"
...
End if 
Process_End
ERR_MethodTrackerReturn("StockAss Dis"; $_t_oldMethodName)
