...
If (True:C214)
	...
	C_LONGINT:C283(ModuleNumber; PAL_BUTTON_10; PAL_BUTTON_7; PAL_BUTTON_8; PAL_BUTTON_9; SD2_l_CallingProcess; SD2_l_TableNum; vContr)
...
Case of 
	: ($_l_Event=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		WS_AutoscreenSize(1; 800; 800; Table name:C256(->[INVOICES:39])+"_"+"Invoices_In13")
		...
		If ([INVOICES:39]State:10=-3)
			DB_SetInputFormMenu(Table:C252(->[INVOICES:39]); "Invoices_InP13")
		Else 
			
			DB_SetInputFormMenu(Table:C252(->[INVOICES:39]); "Invoices_In13")
		End if 
	...
	: (($_l_Event=On Activate:K2:9) | (vFromIn))
		If ([INVOICES:39]State:10=-3)
			//DB_SetInputFormMenu (Table(->[INVOICES]);"Invoices_InP13")
		Else 
			
			//DB_SetInputFormMenu (Table(->[INVOICES]);"Invoices_In13")
		End if 
		OpenHelp(Table:C252(->[INVOICES:39]); WIN_t_CurrentInputForm)
		If (Records in selection:C76([INVOICES:39])<=1)
			OBJECT SET VISIBLE:C603(PAL_BUTTON_8; False:C215)
	...
	: ($_l_Event=On Outside Call:K2:11)
		...
		Case of 
			...
			: (DB_l_ButtonClickedFunction=DB_GetButtonFunction("Plus")) | (DB_l_ButtonClickedFunction=DB_GetButtonFunction("Minus")) | (DB_l_ButtonClickedFunction=DB_GetButtonFunction("Open"))
				...
				inButtSubHandle(FORM Get current page:C276; ->[INVOICES:39]; "Invoices_In13")
