...
If (False:C215)
	//Project Method Name:      StockAss Movt
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("StockAss Movt")
				...
				If ($_r_Quantity<$_r_Required)  //If not all available at this level
					...
					If (($_l_StockAllocationLevel<$_r_StockAllocationTotalCost) & ($_l_StockAllocationLevel<10))
						...
						If (Records in selection:C76([COMPONENTS:86])>0)
							$_r_Number:=StockAss Movt($_r_Required-$l; $_at_ProductCodes{$_l_ComponentIndex}; $_l_StockAllocationLevel+1; $_r_StockAllocationTotalCost; $_t_StockMovementType; $_t_AnalysisCode; $_t_CurrencyCode; $_t_LayerCode; $_d_Date; $_t_CallCode)
...
Else 
	$0:=0
End if 
ERR_MethodTrackerReturn("StockAss Movt"; $_t_oldMethodName)
