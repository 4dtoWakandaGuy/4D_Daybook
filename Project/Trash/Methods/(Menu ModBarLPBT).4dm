...
If ((DB_GetModuleSettingByNUM(1))=5)
	Get_Picture FROM LIBRARY(20220; <>PictLogoPal)
Else 
	Get_Picture FROM LIBRARY(20210; <>PictLogoPal)
End if 

If ((DB_GetModuleSettingByNUM(1))=5)  //Silver or Gold ? button
	Get_Picture FROM LIBRARY(19798; <>PictIconPal)
Else 
	Get_Picture FROM LIBRARY(19799; <>PictIconPal)
End if 
		...
		If ((DB_GetModuleSettingByNUM(1))=5)  //Silver
			Case of 
				: (<>MOD_al_ModuleBarNum{$_l_ModuleRow}=1)  //Silver
					Get_Picture FROM LIBRARY(Menu_ModChkIcon(19702); $_pic_ModuleIcon)
					<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
				: (<>MOD_al_ModuleBarNum{$_l_ModuleRow}=30)  //Silver Plus
					Get_Picture FROM LIBRARY(Menu_ModChkIcon(19730); $_pic_ModuleIcon)
					<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
				: (<>MOD_al_ModuleBarNum{$_l_ModuleRow}=3)  //SilverWP - use the icon style
					Get_Picture FROM LIBRARY(Menu_ModChkIcon(19803); $_pic_ModuleIcon)
					<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
				: (<>MOD_al_ModuleBarNum{$_l_ModuleRow}=9)  //Silver JC
					Get_Picture FROM LIBRARY(Menu_ModChkIcon(19809); $_pic_ModuleIcon)
					<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
				: (<>MOD_al_ModuleBarNum{$_l_ModuleRow}=<>Mod_l_MaxModules)  //DM
					Get_Picture FROM LIBRARY(Menu_ModChkIcon(19837); $_pic_ModuleIcon)
					<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
			End case 
		Else 
			Case of 
				: ((<>MOD_al_ModuleBarNum{$_l_ModuleRow}=12))  //Agency
					If (<>ModuleIcons)
						Get_Picture FROM LIBRARY($_l_Result+10; $_pic_ModuleIcon)
						<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
					Else 
						Get_Picture FROM LIBRARY($_l_Result+9; $_pic_ModuleIcon)
						<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
					End if 
				: ((<>MOD_al_ModuleBarNum{$_l_ModuleRow}=32) | (<>MOD_al_ModuleBarNum{$_l_ModuleRow}=43))  //Telephony & Internet = Remote Access
					Get_Picture FROM LIBRARY(Menu_ModChkIcon($_l_Result+21); $_pic_ModuleIcon)
					<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
				Else 
					Get_Picture FROM LIBRARY(Menu_ModChkIcon($_l_Result+<>MOD_al_ModuleBarNum{$_l_ModuleRow}); $_pic_ModuleIcon)
					<>PictIconPal:=<>PictIconPal+$_pic_ModuleIcon
			End case 
		End if 
