...
If (False:C215)
	//Project Method Name:      ScrnSav Proc
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("ScrnSav Proc")
//ScrnSav Procedure
If (<>MenuProc=0)
	<>MenuProc:=1
End if 
...
If (<>SYS_bo_CurrentWindowModal=False:C215)
	...
	If ((<>DB_t_Password#"") & (OK=1) & (DB_GetModuleSettingByNUM(1)#5) & (<>PER_bo_ScrnSaveId))
		...
		<>DB_t_Password:=""
		DIALOG:C40([COMPANIES:2]; "dAsk Id")
		CLOSE WINDOW:C154
	...
	End if 
End if 
ERR_MethodTrackerReturn("ScrnSav Proc"; $_t_oldMethodName)
