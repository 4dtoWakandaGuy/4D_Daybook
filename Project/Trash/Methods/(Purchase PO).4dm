...
If (False:C215)
	//Project Method Name:      Purchase PO
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("Purchase PO")
//Project Method Purchase PO 26/3/2(ID 5995-5926)
...
Menu_Record("Purchase PO"; "Copy a Purchase Order")
	...
	If (Records in selection:C76([PURCHASE_ORDERS:57])>0)
		If (Records in selection:C76([PURCHASE_ORDERS:57])>1)
			If (Count parameters:C259=0)
				...
				//OUTPUT FORM([PURCHASE ORDERS];"Purch Ord Out")
				//WIN_t_CurrentOutputform:="Purch Ord Out"
...
If (Count parameters:C259=0)
	...
	vTot:=1
End if 
ERR_MethodTrackerReturn("Purchase PO"; $_t_oldMethodName)
