...
If (False:C215)
	//Project Method Name:      Open Flo Window
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_LONGINT:C283(<>PER_l_CurrentPersFunctions; <>SYS_l_WindowTop; <>SYS_l_Windowx; <>SYS_l_Windowy; <>WIN_MINLEFT; $_l_AbsoluteLeft; $_l_CountWindows; $_l_DockLB; $_l_DockLL; $_l_DockLR; $_l_DockP)
...
$_t_oldMethodName:=ERR_MethodTracker("Open Flo Window")
//Open_Pro_Window
...
If (vProc>0)
	...
	If (Not:C34(<>PER_l_CurrentPersFunctions ?? 3)) & (Not:C34(<>PER_l_CurrentPersFunctions ?? 2))  //counteract Palettes being permanently off
		If ($_l_AbsoluteLeft=0)
			$_l_WindowLeft:=$_l_WindowLeft-55
			$_l_WindowRight:=$_l_WindowRight-55
		End if 
		If (<>PER_l_CurrentPersFunctions<2)
	...
	If (($_l_ScreenHeight>480) | ((Not:C34(<>PER_l_CurrentPersFunctions ?? 2))))  //have room to go down if no Modules
	...
	If ($_l_ScreenWIdth>622)
		...
		$_l_EmptyIDPosition:=Abs:C99($_l_EmptyIDPosition)
	End if 
	$_bo_bar:=(<>PER_l_CurrentPersFunctions ?? 2)
...
End if 
GET WINDOW RECT:C443(PRC_l_WindowLeft; PRC_l_WindowTop; PRC_l_WindowRight; PRC_l_WindowBottom)
ERR_MethodTrackerReturn("Open Flo Window"; $_t_oldMethodName)