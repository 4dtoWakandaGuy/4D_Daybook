...
If (False:C215)
	//Project Method Name:      Jobs WIPAdj
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("Jobs WIPAdj")
...
$_t_oldMethodName:=ERR_MethodTracker("Jobs WIPAdj")
//Jobs WIPAdj
Menu_Record("Jobs WIPAdj"; "WIP Adjustment")
If (Not:C34(Read only state:C362([JOBS:26])))
	If ([JOBS:26]Use_WIP:47)
		If ((<>WIP_t_TRPTrans#"") | (<>WIP_t_CIPTRANS#""))
			If (Jobs_WIPCheck)
				...
				DIALOG:C40([JOBS:26]; "dAsk WIPAdj")
				CLOSE WINDOW:C154
				$_l_OK:=OK
				OK:=1
	...
	End if 
End if 
ERR_MethodTrackerReturn("Jobs WIPAdj"; $_t_oldMethodName)