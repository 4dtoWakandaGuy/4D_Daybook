...
If (True:C214)
	...
	C_LONGINT:C283(cSIS; SM_al_NextElement; SM_l_QueryMode; SM_l_QuerySearchID; SM_l_ReturnQueryID; SM_l_SearchNumber; SRCH_l_And; SRCH_l_Not; SRCH_l_or; VClassID; vJ)
	C_LONGINT:C283(WIN_l_CurrentWinRef)
	C_POINTER:C301($2; $3)
	C_REAL:C285($_r_AmountFrom; $_r_AmountTo; $_r_invF; $_r_invT; VamountF; VamountT; VinvF; vInvT)
	C_TEXT:C284($_t_oldMethodName; $_t_WindowTitleOLD; ACC_t_CurrencyCode; Vanalysis; vClass; VCompCode; VcompName; VContCOde; VcontrCode; VForename; VJob)
	C_TEXT:C284(VJobCode; Vlayer; VorderNo; VprodCode; Vsales; VSELPREV; Vstage; Vsurname; VType)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Jobs_SelP")
//Jobs_SelP
$_bo_OpenWindow:=False:C215



Sel_Beginning(False:C215; ->[JOBS:26])
$_l_CurrentRow:=1
$_l_QueryID:=0
$_l_Mode:=0
...
End if 
$_l_WhilesCounter:=0
While ((vJ=1) & ((SRCH_l_And=1) | (SRCH_l_or=1) | (SRCH_l_Not=1))) | ($_l_CurrentRow<=$_l_ElementCount)
	...
	If ($_l_Mode=0)
		If ($_l_WhilesCounter=1)
			Sel_Beginning2(False:C215; ->[JOBS:26])
		End if 
	Else 
		Sel_Beginning2(False:C215; ->[JOBS:26])
	End if 
	...
	If ($_l_QueryID=0)
		...
		If (Not:C34(SM_bo_SessionWindow))
			...
			Open_AnyTypeWindow(322; 528; 4; "Find Jobs")
		...
		Else 
			...
			If ($_bo_SessionWindowOwner)
				$_bo_OpenWindow:=True:C214
			End if 
			SET WINDOW TITLE:C213("Find Jobs")
		End if 
		DIALOG:C40([JOBS:26]; "dJobs_Sel")
		...
		If (SM_l_QueryMode=1) & (OK=1)
			...
			If (SM_l_ReturnQueryID=0)  // not a linked query
				...
				SM_SearchVarsToBlob(->SM_blb_SearchBlob; ->VJobCode; ->VJob; ->VCompCode; ->VcompName; ->VContCOde; ->VForename; ->Vsurname; ->VcontrCode; ->Vsales; ->VType; ->VorderNo; ->Vstage; ->VsDateF; ->VsDateT)
				SM_SearchVarsToBlob(->SM_blb_SearchBlob; ->SRCH_d_EntryDateFrom; ->SRCH_d_EntryDateTo; ->VRDateF; ->VRDateT; ->VamountF; ->VamountT; ->VinvF; ->vInvT; ->VprodCode; ->Vanalysis; ->ACC_t_CurrencyCode; ->Vlayer; ->SYS_at_RecordStateNames; ->MOD_al_ModuleAccessStatus; ->cSIS; ->cNAA; ->cNAL; ->vClass; ->VClassID)
			...
			$_l_offset:=BLOB size:C605([x_QUERY_PARAMETERS:134]X_SearchValues:5)
	...
	Else 
		Sel_S(->[JOBS:26]; ->[JOBS:26]Company_Code:3; vCompCode)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Contact_Code:4; vContCode)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Job_Code:1; vJobCode)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Job_Name:2; vJob)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Stage_Code:18; vStage)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Job_Type:21; vType)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Person:19; vSales)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Project_Code:27; vContrCode)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Analysis_Code:24; vAnalysis)
		Sel_S(->[JOBS:26]; ->[JOBS:26]Your_Order_Number:5; vOrderNo)
		Sel_SSNum(->[JOBS:26]; ->[JOBS:26]x_RecordClass:50; VClassID; VClassID)
		Sel_SCurr(->[JOBS:26]; ->[JOBS:26]Currency_Code:25; ACC_t_CurrencyCode)
		Sel_SLay(->[JOBS:26]; ->[JOBS:26]Layer_Code:26; vLayer)
		Sel_SIDate(->[JOBS:26]; ->[JOBS:26]Start_Date:6; vSDateF; vSDateT)
		Sel_State(->[JOBS:26]; ->[JOBS:26]State:9)
		Sel_SIS(->[JOBS:26])
		Sel_SSDate(->[JOBS:26]; ->[JOBS:26]End_Date:7; SRCH_d_EntryDateFrom; SRCH_d_EntryDateTo)
		Sel_SSDate(->[JOBS:26]; ->[JOBS:26]Required_Date:8; vRDateF; vRDateT)
		Sel_SSNum(->[JOBS:26]; ->[JOBS:26]AS_Quoted_Amount:11; vAmountF; vAmountT)
		Sel_SSNum(->[JOBS:26]; ->[JOBS:26]AS_Invoice_Including_Tax:13; vInvF; vInvT)
		...
		If (vProdCode#"")
			...
			Load_ManyFast(->[ORDER_ITEMS:25]; ->[ORDER_ITEMS:25]Job_Code:34; "Master2"; ->[JOBS:26]Job_Code:1; ->[JOBS:26])
		End if 
		If (vCompName#"")
			Comp_SelPOther(->[JOBS:26]; ->[JOBS:26]Company_Code:3; "Master2"; vCompName)
		End if 
		If ((vForename#"") | (vSurname#""))
			Cont_SelPOther(->[JOBS:26]; ->[JOBS:26]Contact_Code:4; "Master2"; vForename; vSurname)
		End if 
...
End if 
ERR_MethodTrackerReturn("Jobs_SelP"; $_t_oldMethodName)