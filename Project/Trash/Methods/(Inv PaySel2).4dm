...
If (False:C215)
	//Project Method Name:      Inv PaySel2
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_BOOLEAN:C305(<>PurchPayRem; <>SI_bo_AllocationValidationDisab; $_bo_AnalisDeposit; $_bo_Confirm; $_bo_Continue; $_bo_EndTransaction; $_bo_IsFromOrder; $_bo_IsOK; $_bo_NoCC; $_bo_OK; $_bo_ReportExists)
	C_BOOLEAN:C305($_bo_Selection; $_bo_SinSID; $_bo_Unlocked; $1; $5; $6; ACC_bo_CurrencyWriteOff; CCM_bo_CV2Retry; CCM_bo_DisableAVSTEMP; DB_bo_RecordModified; PAY_bo_CreateAuths)
	C_BOOLEAN:C305(PRT_bo_AskPerIteration; PRT_bo_AutoIteration; PRT_bo_ToPdf; PRT_bo_ToScreenPreview; vMod3; WIN_bo_TrackerInited)
	...
	C_LONGINT:C283(i; INV_l_CCID; PAY_l_CARDID; PAY_l_TransactionID; PRT_Count; r1; r2; Vadd; vSNo; WIN_l_CurrentWinRef)
	...
	C_REAL:C285($_r_VATCSAmount; $2; v31; v61; vPay2; vPayment; vTotal)
	...
	C_TEXT:C284(vBank; vChequeNo; vCompCode; vCompName; vCountry; vDesc; vLayer; vTitle; vTitle1; vTitle2)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Inv PaySel2")
...
$_t_oldMethodName:=ERR_MethodTracker("INV PaySel2")  //Inv PaySel2 - see also Invoices_ImpR
...
If (Records in selection:C76([TRANSACTION_TYPES:31])=1)
						...
						For ($_l_InvoicesIndex; 1; $_l_RecordsInSelection)
							...
							If ($_r_AmountRow>0)
								...
								If (OK=1)
									...
									//TRACE
									For ($_at_InvoiceNumber; $_l_invoices2Record; $_l_RecordsInSelection)
										//While ((Not(End selection([INVOICES]))) & (OK=1) & ($_t_CompanyCode=[INVOICES]Company Code))
										If ($_at_InvoiceNumber<=$_l_RecordsInSelection)
											
											USE NAMED SELECTION:C332("$ToPost")
											GOTO SELECTED RECORD:C245([INVOICES:39]; $_at_InvoiceNumber)
											If ($_t_CompanyCode=[INVOICES:39]Company_Code:2)
												
												If (Check_Locked(->[INVOICES:39]; 1))
													If (GEN_ar_Value{$_at_InvoiceNumber}#0)  //here is an error
														
														If ($_al_TestedNumbers{$_at_InvoiceNumber}=0)
															
															$_al_TestedNumbers{$_at_InvoiceNumber}:=$_at_InvoiceNumber
															If (([INVOICES:39]Invoice_Number:1="*@") & ([INVOICES:39]Total_Invoiced:5=0))  //& ([INVOICES]Total Due#0))`made negative deposits go wrong
																
																[INVOICES:39]Total_Due:7:=Round:C94(([INVOICES:39]Total_Due:7-GEN_ar_Value{$_at_InvoiceNumber}); 2)
																...
																[INVOICES_ALLOCATIONS:162]Amount:2:=GEN_ar_Value{$_at_InvoiceNumber}
																...
																$_ar_AllocationAmount{$_l_PElement}:=GEN_ar_Value{$_at_InvoiceNumber}
																//$INP:=$INP+[INVOICES]Invoice_Number+Char(9)
																//$IND:=$IND+String([INVOICES]Invoice_Date)+Char(9)
																//$INA:=$INA+String(GEN_ar_Value{$_at_InvoiceNumber})+Char(9)
																...
																If (Application type:C494=4D Remote mode:K5:5)
																	...
																	SET PROCESS VARIABLE:C370(-1; <>SI_bo_AllocationValidationDisab; <>SI_bo_AllocationValidationDisab)
																End if 
																INV_AllocationCreate([INVOICES:39]Invoice_Number:1; $_t_InvoiceNumber; Vdate; GEN_ar_Value{$_at_InvoiceNumber}; ""; [PERIODS:33]Period_Code:1)
																...
																Else 
																	...
																	[TRANSACTION_BATCH_ITEMS:155]Amount:6:=Gen_Round((0-GEN_ar_Value{$_at_InvoiceNumber}); 2; 2)
																	[TRANSACTION_BATCH_ITEMS:155]Total:9:=Gen_Round((0-GEN_ar_Value{$_at_InvoiceNumber}); 2; 2)
																	If ([INVOICES:39]Analysis_Code:17#"")
																...
																End if 
															Else 
																$_r_Amount:=GEN_ar_Value{$_at_InvoiceNumber}
																...
																If ([INVOICES:39]Cash_TAX:41)
																	If ([INVOICES:39]Cash_TAX_Due:40#0)  //GEN_ar_Value{i}
																		...
																		If (((GEN_ar_Value{$_at_InvoiceNumber}>=[INVOICES:39]Total_Due:7) & ([INVOICES:39]Total_Due:7>=0)) | ((GEN_ar_Value{$_at_InvoiceNumber}<=[INVOICES:39]Total_Due:7) & ([INVOICES:39]Total_Due:7<0)))
																			$_r_VATCSAmount:=[INVOICES:39]Cash_TAX_Due:40
																			[INVOICES:39]Cash_TAX_Due:40:=0
																		Else 
																			$_r_VATCSAmount:=Gen_Round(([INVOICES:39]Cash_TAX_Due:40*(GEN_ar_Value{$_at_InvoiceNumber}/[INVOICES:39]Total_Due:7)); 2; 2)
																...
																End if 
																[INVOICES:39]Total_Paid:6:=Gen_Round(([INVOICES:39]Total_Paid:6+GEN_ar_Value{$_at_InvoiceNumber}); 2; 2)
																[INVOICES:39]Total_Due:7:=Gen_Round(([INVOICES:39]Total_Due:7-GEN_ar_Value{$_at_InvoiceNumber}); 2; 2)
																CREATE RECORD:C68([INVOICES_ALLOCATIONS:162])
																[INVOICES_ALLOCATIONS:162]Company_Code:7:=[INVOICES:39]Company_Code:2
																[INVOICES_ALLOCATIONS:162]INVOICES_CODE:6:=[INVOICES:39]Invoice_Number:1
																[INVOICES_ALLOCATIONS:162]ID:5:=AA_GetNextID(->[INVOICES_ALLOCATIONS:162]ID:5)
												...
												Else 
													...
													OK:=0
												End if 
												$_l_Invoices2Temp:=$_at_InvoiceNumber
											Else 
												$_l_Invoices2Temp:=$_at_InvoiceNumber
												$_at_InvoiceNumber:=$_l_RecordsInSelection
											End if 
										Else 
											$_l_Invoices2Temp:=$_at_InvoiceNumber
											$_at_InvoiceNumber:=$_l_RecordsInSelection
							...
							If (OK=0)
								...
								Else 
									...
									For ($_l_Index; 1; Records in selection:C76([INVOICES:39]))
										...
										$_bo_Continue:=Gen_PPSingle_2(->[INVOICES:39]; ->[INVOICES_ALLOCATIONS:162]; "Receipt Rem"; "Alloc SubRec"; "Invoices Out"; DB_t_CurrentFormUsage; "Inv Remittance"; "INV_ReceiptFormMethod"; 0; ->[INVOICES:39]State:10; "Receipt Rem"; [INVOICES:39]Analysis_Code:17; False:C215; $_l_Index)
	...
	End if 
End if 
ERR_MethodTrackerReturn("INV PaySel2"; $_t_oldMethodName)