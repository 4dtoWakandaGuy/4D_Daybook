...
If (False:C215)
	//Project Method Name:      JobPers_SelP
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_POINTER:C301($2; $3)
	C_TEXT:C284($_t_oldMethodName; $_t_WindowTitleOLD; VJobCode; vPerson; VSELPREV; Vstage)
End if 

//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("JobPers_SelP")
$_bo_OpenWindow:=False:C215

//JobPers_SelP
Sel_Beginning(False:C215; ->[JOB PERSONNEL:48])
$_l_CurrentRow:=1
$_l_QueryID:=0
$_l_Mode:=0
...
While ((vJ=1) & ((SRCH_l_And=1) | (SRCH_l_or=1) | (SRCH_l_Not=1))) | ($_l_CurrentRow<=$_l_ElementCount)
	...
	If (Count parameters:C259>=2)
		...
		Case of 
			: ($1<0)
				$_l_Mode:=1  // vDont search we are building
	...
	If ($_l_Mode=0)
		If ($_l_WhilesCounter=1)
			Sel_Beginning2(False:C215; ->[JOB PERSONNEL:48])
		End if 
	Else 
		Sel_Beginning2(False:C215; ->[JOB PERSONNEL:48])
	End if 
	...
	If ($_l_QueryID=0)
		...
		If (Not:C34(SM_bo_SessionWindow))
			...
			Open_AnyTypeWindow(277; 428; 4; "Find Job Personnel")
		...
		Else 
			...
			If ($_bo_SessionWindowOwner)
				$_bo_OpenWindow:=True:C214
			End if 
			SET WINDOW TITLE:C213("Find Job Personnel")
		End if 
		DIALOG:C40([JOB PERSONNEL:48]; "dJobPers_Sel")
	...
	Else 
		Sel_S(->[JOB PERSONNEL:48]; ->[JOB PERSONNEL:48]Person:3; vPerson)
		Sel_S(->[JOB PERSONNEL:48]; ->[JOB PERSONNEL:48]Job_Code:1; vJobCode)
		Sel_S(->[JOB PERSONNEL:48]; ->[JOB PERSONNEL:48]Stage_Code:2; vStage)
		Sel_SIS(->[JOB PERSONNEL:48])
		Big_One(->[JOB PERSONNEL:48])
	End if 
End while 
Sel_Ending(->[JOB PERSONNEL:48])
If ($_bo_OpenWindow)
	CLOSE WINDOW:C154
...
End if 
ERR_MethodTrackerReturn("JobPers_SelP"; $_t_oldMethodName)