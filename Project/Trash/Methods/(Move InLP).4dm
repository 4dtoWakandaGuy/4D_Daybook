...
If (False:C215)
	//Project Method Name:      Move InLP
	//------------------ Method Notes ------------------
	
	//------------------ Revision Control ----------------
	//Date Created: 09/12/2009 00:13`Method: Move InLP
...
If (True:C214)
	...
	ARRAY REAL:C219($_ar_2DLongintData; 0)
	...
	C_BOOLEAN:C305($_bo_ReadOnly; DB_bo_NoLoad; DB_bo_RecordModified; PAL_bo_ButtonSubFunction; STK_bo_AddItem; STK_bo_DataPointer; STK_bo_LineTrackerActive; STOCK_AutoCancel; vFromIn)
	...
	C_LONGINT:C283(iOK; IP_l_Message; LB_l_ColumnNumber; LB_l_RowNumber; ModuleNumber; PAL_BUTTON_10; PAL_BUTTON_4; PAL_BUTTON_5; PAL_BUTTON_7; PAL_BUTTON_8; PAL_BUTTON_9)
	C_LONGINT:C283(STK_l_DataPointer; STK_l_DupID; STK_l_NewStockItemID; STK_l_REcallID; STOCK_l_CopyProcess)
	C_PICTURE:C286(STK_pic_ModeStock)
	C_POINTER:C301($_ptr_ArrayFieldPointers; $_ptr_ArrayOfArrayNames; $_ptr_ArrayofArrayPointers; $_ptr_ArrayOfColumFormats; $_Ptr_ArrayofColumnFormulas; $_ptr_ArrayReferences; $_ptr_EnterabilityArray; $_ptr_Field; $_ptr_LBTable; $_Ptr_ThisColumnArray)
	C_REAL:C285(STK_r_BreakQuantity; STK_R_DataPointer; vTot)
	...
	C_TEXT:C284(PAL_BUTTON; STK_t_CurrentStockType; STK_t_DataPointer; vButtDisMov; vCompName; vName; vType; WIN_t_CurrentInputForm)
	C_TIME:C306(STK_ti_DataPointer)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("Move InLP")
//Move InLP
...
Case of 
	...
	: ($_l_FormEvent=On Load:K2:1) & (Not:C34(DB_bo_NoLoad))
		...
		If ($_l_MaxWidth>800)
			$_l_MaxWidth:=800
		End if 
		WS_AutoscreenSize(1; 450; $_l_MaxWidth; Table name:C256(->[STOCK_MOVEMENTS:40])+"_"+"Movements_In13")
		...
		DB_SetInputFormMenu(Table:C252(->[STOCK_MOVEMENTS:40]); "Movements_In13")
	...
	: (($_l_FormEvent=On Activate:K2:9) | (vFromIn))
		...
		End if 
		OpenHelp(Table:C252(->[STOCK_MOVEMENTS:40]); "Movements_In13")
		INT_SetInput(Table:C252(->[STOCK_MOVEMENTS:40]); WIN_t_CurrentInputForm)
		SD2_SetDiaryPopup
		If (Records in selection:C76([STOCK_MOVEMENTS:40])<=1)
			OBJECT SET VISIBLE:C603(PAL_BUTTON_8; False:C215)
								...
								If (BLOB size:C605(STK_BL_DataBlob)>$_l_offset)
									...
									Case of 
										...
										: (Type:C295($_ptr_Field->)=Is longint:K8:6)
											...
											STK_l_DataPointer:=0
											$_l_offset:=DB_Blob_to_Data(->STK_l_DataPointer; ->STK_BL_DataBlob; $_l_offset; True:C214)
											$_al_LongintData{$_l_index}{$_l_RecordCount}:=STK_l_DataPointer
	...
	: ($_l_FormEvent=On Deactivate:K2:10)
		...
		//PAUSE PROCESS(vProc)
End case 
ERR_MethodTrackerReturn("Move InLP"; $_t_oldMethodName)
