...
If (Diary_FileL)  // modified NG June 2004
	...
	If (In_Allowed($_t_WindowTitle; ->[DIARY:12]))
		...
		vAdd:=1
		OK:=1
		Open_Pro_Window($_t_WindowTitle; 0; 2; ->[DIARY:12]; "Diary_InL13"+Documents_4DWriteAccessType(False:C215))  //NG May 2004 added form
		While ((OK=1) & (DB_t_CallOnCloseorSave#"Close"))
			FORM SET INPUT:C55([DIARY:12]; "Diary_InL13"+Documents_4DWriteAccessType(False:C215))  //NG April 2004 removed reference to ◊screen
			ADD RECORD:C56([DIARY:12]; *)
			ADD TO SET:C119([DIARY:12]; "DMaster")
			Gen_InOne
		End while 
		Close_ProWin(Table name:C256(->[DIARY:12])+"_"+"Diary_InL13"+Documents_4DWriteAccessType(False:C215))
		If (Count parameters:C259>=1)
