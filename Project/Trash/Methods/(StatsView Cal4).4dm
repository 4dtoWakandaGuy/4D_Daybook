...
If (False:C215)
	//Project Method Name:      StatsView Cal4
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("StatsView Cal4")
//StatsView Calc Col 4
Load_Fields

$_l_ArraySize:=Size of array:C274(aSV1)
If ((SV_at_FieldName4=1) | (bd12=1))
	For ($_l_CurrentRow; 1; $_l_ArraySize)
		aSV4{$_l_CurrentRow}:=0
	End for 
	vSVT4:=0
Else 
	<>SYS_l_CancelProcess:=0
	If ((SV_at_FieldName4#1) & ($_l_ArraySize>0))
		vSVT4:=0
		...
		Else 
			...
			Else 
				...
				If ($_l_FIeldNumberColumn>0)
					...
					COPY NAMED SELECTION:C331(vFilePtr->; "$SelCal4")
					$_l_CurrentRow:=1
...
ERR_MethodTrackerReturn("StatsView Cal4"; $_t_oldMethodName)