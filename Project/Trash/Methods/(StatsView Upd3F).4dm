...
If (False:C215)
	//Project Method Name:      StatsView_Upd3F
	//------------------ Method Notes ------------------
...
If (True:C214)
	...
	C_REAL:C285($_r_Sum; vSVT3)
	C_TEXT:C284($_t_FurtherField; $_t_oldMethodName; $_t_TableNumberString; $FFType)
End if 
//Code Starts Here

$_t_oldMethodName:=ERR_MethodTracker("StatsView_Upd3F")
//StatsView_Upd3F - FurthFld
Load_Fields
...
$_t_FurtherField:=Substring:C12(SV_at_FieldName3{SV_at_FieldName3}; 4; 31)
	...
	Case of 
		...
		: (SV_at_CalcOption3=3)
			If (Records in selection:C76([CUSTOM_FIELDS:98])>0)
				aSV3{$1}:=Max:C3([CUSTOM_FIELDS:98]Real_or_Integer:5)
			Else 
				aSV3{$1}:=0
			End if 
			vSVT3:=vSVT3+aSV3{$1}
		: (SV_at_CalcOption3=4)
			If (Records in selection:C76([CUSTOM_FIELDS:98])>0)
				aSV3{$1}:=Min:C4([CUSTOM_FIELDS:98]Real_or_Integer:5)
			Else 
				aSV3{$1}:=0
			End if 
			vSVT3:=vSVT3+aSV3{$1}
...
End if 
ERR_MethodTrackerReturn("StatsView_Upd3F"; $_t_oldMethodName)
