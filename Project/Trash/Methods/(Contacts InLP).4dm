...
If (True:C214)
	...
	C_LONGINT:C283(DB_l_WindowCurrentModule; iCancel; iOK; ModuleNumber; PAL_BUTTON_10; PAL_BUTTON_7; PAL_BUTTON_8; PAL_BUTTON_9; SD2_l_CallingProcess; vAccessSwNo; vNo2)
...
Case of 
	...
	: ($_l_Event=On Activate:K2:9)
		DB_SetInputFormMenu(Table:C252(->[CONTACTS:1]); "Contacts_InM13")
		OpenHelp(Table:C252(->[CONTACTS:1]); "Contacts_InM13")
		If ($_bo_ContactButtons)
			If (Size of array:C274(PAL_POPUP_5)>=2)
				...
				PAL_POPUP_5{2}:="Further Analysis"
				$_t_PopupItemName:=PAL_POPUP_5{2}
				In_ButtChkDis(->vButtDisCon; "Contact"; $_t_PopupItemName)
			Else 
				ARRAY TEXT:C222(PAL_POPUP_5; 2)
				PAL_POPUP_5{1}:="Contact"
				PAL_POPUP_5{2}:="Further Analysis"
				In_ButtChkDis(->vButtDisCon; "Contact"; PAL_POPUP_5{2})
			End if 
		Else 
			If (vAccessSwNo=-1)
				vAccessSwNo:=0
