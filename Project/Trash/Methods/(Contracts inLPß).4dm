...
Case of 
	...
	: (CONT_al_ViewTableToTable{CONT_at_ViewTabsLabels}=Table:C252(->[ORDERS:24]))  //Contract shedule items
		...
		If (Size of array:C274(SVS_aptr_ListboxSetup)=0) | (CONT_l_ViewTabsSelectedOLD#CONT_at_ViewTabsLabels)
			CONT_l_ViewTabsSelectedOLD:=CONT_at_ViewTabsLabels
			LBi_ArrDefFill(->SVS_aptr_ListboxSetup; ->SVS_lb_ItemsSetup; ->[ORDERS:24]Order_Code:3; ->[CONTRACTS:17]Contract_Code:3; "411111000"; "Orders_In13"; 9; "Order Records"; "Orders"; ""; $_bo_NewLevel; $_l_ReuseLevel)
		Else 
	...
	: (CONT_al_ViewTableToTable{CONT_at_ViewTabsLabels}=Table:C252(->[CUSTOM_FIELDS:98]))
		...
		If (Size of array:C274(SVS_aptr_ListboxSetup)=0) | (CONT_l_ViewTabsSelectedOLD#CONT_at_ViewTabsLabels)
			...
			LBi_ArrDefFill(->SVS_aptr_ListboxSetup; ->SVS_lb_ItemsSetup; ->[ORDER_ITEMS:25]Item_Number:27; ->[CONTRACTS:17]Contract_Code:3; "411111126"; "Items in13"; 9; "Products"; "Product"; ""; $_bo_NewLevel; $_l_ReuseLevel)  // todo: this needs checking!
	...
	: (CONT_al_ViewTableToTable{CONT_at_ViewTabsLabels}=Table:C252(->[PURCHASE_ORDERS:57]))  //PO's relating to Order items charges in relation to the contract
		...
		If (Size of array:C274(SVS_aptr_ListboxSetup)=0) | (CONT_l_ViewTabsSelectedOLD#CONT_at_ViewTabsLabels)
			CONT_l_ViewTabsSelectedOLD:=CONT_at_ViewTabsLabels
			LBi_ArrDefFill(->SVS_aptr_ListboxSetup; ->SVS_lb_ItemsSetup; ->[PURCHASE_ORDERS:57]Purchase_Order_Number:2; ->[CONTRACTS:17]Contract_Code:3; $_t_FlagReference; "Purch Ord In13"; 5; "Purchases Orders"; "Purchase order"; ""; $_bo_NewLevel; $_l_ReuseLevel)
		Else 
			LBi_Prefs11(->SVS_aptr_ListboxSetup)  //reset the unique fields or page-turning won't work
		End if 
		LBi_LoadSetup(->SVS_aptr_ListboxSetup; "B"; CONT_al_ViewTabsSettings{CONT_at_ViewTabsLabels})
	: (CONT_al_ViewTableToTable{CONT_at_ViewTabsLabels}=Table:C252(->[SERVICE_CALLS:20]))  //service calls for the contract
		...
		If (Size of array:C274(SVS_aptr_ListboxSetup)=0) | (CONT_l_ViewTabsSelectedOLD#CONT_at_ViewTabsLabels)
			CONT_l_ViewTabsSelectedOLD:=CONT_at_ViewTabsLabels
			LBi_ArrDefFill(->SVS_aptr_ListboxSetup; ->SVS_lb_ItemsSetup; ->[SERVICE_CALLS:20]Call_Code:4; ->[CONTRACTS:17]Contract_Code:3; $_t_FlagReference; "Calls_In13"; 2; "Service Calls"; "Service Call"; ""; $_bo_NewLevel; $_l_ReuseLevel)
