...
If (False:C215)
	//Project Method Name: Object Name:      DB_OutputForm.oAccDateFrom
	//------------------ Method Notes ------------------
...
Case of 
	: ($_l_FormEvent=On After Keystroke:K2:26)
		...
		If (Character code:C91($sE)#Backspace:K15:36)
			...
			If ($Pos>0)
				ACC_at_PeriodFromCode{0}:=ACC_at_PeriodFromCode{$Pos}
				$Len2:=Length:C16(ACC_at_PeriodFromCode{0})
				HIGHLIGHT TEXT:C210(ACC_at_PeriodFromCode{0}; $Len+1; $Len2+1)
				LAY_b_FiltersActive:=True:C214
				Lay_b_UpdateFilters:=True:C214
				$pos:=Find in array:C230(ACC_at_PeriodFromCode; $CurrentText)
				If ($Pos>0)
					Lay_b_UpdateFilters:=False:C215
			...
			Else 
				ACC_at_PeriodFromCode{0}:=Substring:C12($CurrentText; 1; $Len-1)
