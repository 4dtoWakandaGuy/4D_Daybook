	...
	If (Records in selection:C76([CURRENT_STOCK:62])#0)
		...
		If ((Records in selection:C76([CURRENT_STOCK:62])>1) | ([STOCK_MOVEMENT_ITEMS:27]Serial_Number:4=""))
			...
			If (OK=1)
				...
				//OUTPUT FORM([CURRENT STOCK];"StockCurr Out")
				...
				If (Records in selection:C76([STOCK_MOVEMENTS:40])#0)
					...
					If (Records in selection:C76([STOCK_MOVEMENTS:40])>1)
						...
						If (OK=1)
							...
							//INPUT FORM([STOCK MOVEMENTS];"Movements_In13")  `NG April 2004 Removed ◊Screen
							//WIN_t_CurrentInputForm:="Movements_In13"
							//OUTPUT FORM([STOCK MOVEMENTS];"Movements Out")
							//WIN_t_CurrentOutputform:="Movements Out"
							//v`Layout:="Select"
