...
DIALOG:C40([ORDERS:24]; "dAsk GBReport")
CLOSE WINDOW:C154
$_l_OK:=OK
OK:=1
		...
		If (ch0=1)
			...
			If (OK=1)
				...
				If (OK=1)
					...
					//PAGE SETUP([ORDERS];"GB AdEnq Hdr")
					// PRINT SETTINGS
					If ($_l_OK>=0)
								...
								If (Records in selection:C76([ORDERS:24])>0)
									...
									If (($_l_PrintPosition<3) & ($_l_PrintPosition#0))
										...
										While ($_l_PrintPosition<3)
											...
											//   PRINT LAYOUT([ORDERS];"GB AdEnq Hdr")
											//  FORM FEED
									...
									Print form:C5([ORDERS:24]; "GB AdEnq Hdr")
									If (ch3=1)
										PAGE BREAK:C6  //(*)
									End if 
									//*************END
									...
									While ((Not:C34(End selection:C36([ORDERS:24]))) & (<>SYS_l_CancelProcess#Current process:C322))
										...
										End if 
										//****** START
										Print form:C5([ORDERS:24]; "GB AdEnq Label")
										If (ch3=1)
											PAGE BREAK:C6  //(*;>)
										End if 
										//****** END
			...
			If (OK=1)
				...
				If (OK=1)
					...
					If (OK=1)
						...
						PRINT_SetSIZE("A4"; "P")
						//PAGE SETUP([ORDERS];"GB Enq Label")
					Else 
								...
								If (Records in selection:C76([ORDERS:24])>0)
									...
									While ((Not:C34(End selection:C36([ORDERS:24]))) & (<>SYS_l_CancelProcess#Current process:C322))
										...
										If (DB_t_CurrentFormUsage2="1")
											Print form:C5([ORDERS:24]; "GB Enq Label")
											If (ch3=1)
												PAGE BREAK:C6
											End if 
										Else 
