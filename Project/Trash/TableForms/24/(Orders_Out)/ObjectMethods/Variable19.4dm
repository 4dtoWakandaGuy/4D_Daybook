...
If (False:C215)
	//Form Name:      ORDERS.Orders_Out.Variable19
	//------------------ Method Notes ------------------
...
$_t_oldMethodName:=ERR_MethodTracker("OBJ [ORDERS].Orders_Out.Variable19"; Form event code:C388)
...
Case of 
		...
		If (cNAL=0)
			...
			If ((Character code:C91($_t_Keystroke)#Backspace:K15:36) & (Length:C16($_t_EditedText)>0))
				If (Character code:C91($_t_EditedText[[Length:C16($_t_EditedText)]])#64)
					$_l_ArrayRow:=Find in array:C230(ORD_at_SalesInitials; $_t_EditedText)
					$_l_EditedTextLength:=Length:C16($_t_EditedText)
					If ($_l_ArrayRow<0)
						$_l_ArrayRow:=Find in array:C230(ORD_at_SalesNames; $_t_EditedText)
						If ($_l_ArrayRow<0)
							For ($_l_Index; 1; Size of array:C274(ORD_at_SalesInitials))
								If (Position:C15($_t_EditedText; ORD_at_SalesInitials{$_l_Index})=1)
									$_l_ArrayRow:=$_l_Index
									$_l_Index:=99999
								End if 
							End for 
							If ($_l_ArrayRow<0)
								For ($_l_Index; 1; Size of array:C274(ORD_at_SalesNames))
									If (Position:C15($_t_EditedText; ORD_at_SalesNames{$_l_Index})=1)
										$_l_ArrayRow:=$_l_Index
										$_l_Index:=99999
									End if 
								End for 
					...
					End if 
					If ($_l_ArrayRow>0)
						Self:C308->:=ORD_at_SalesInitials{$_l_ArrayRow}
	...
	: ($_l_event=On Data Change:K2:15)
		//$_t_oldMethodName:=ERR_MethodTracker ("OBJ [ORDERS].Orders_Out.Variable19";Form event)
		If (cNAL=0)
			Check_MinorNC(Self:C308; ""; ->[PERSONNEL:11]; ->[PERSONNEL:11]Initials:1; ->[PERSONNEL:11]Name:2; "Person")
		End if 
		If (Not:C34(ORD_bo_UnfilteredSetKnown))
			//Remember the current selection
			ORD_l_SetID:=0
...
End case 
ERR_MethodTrackerReturn("OBJ [ORDERS].Orders_Out.Variable19"; $_t_oldMethodName)
$_t_oldMethodName:=ERR_MethodTracker("OBJ [ORDERS].Orders_Out.Variable19"; Form event code:C388)
$_l_event:=Form event code:C388
Case of 
	: ($_l_event=On After Keystroke:K2:26)
